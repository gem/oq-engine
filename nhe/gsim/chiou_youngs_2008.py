...
from nhe.gsim.base import GMPE
from nhe import const
from nhe.imt import PGA, PGV, SA


#: Coefficients table describing constants values for different intensity
#: measure types and different spectral acceleration periods. First column
#: contains spectral acceleration periods or strings "pga" and "pgv" and
#: thus define IMT. Other columns define coefficients values for respective
#: IMT. Table is constructed from values in tables 1, 2 and 3 (pages 197,
#: 198 and 199).
COEFFS = """\
...
def _make_coeffs_dict(sa_damping):
    """
    Parse coefficients table and return dictionary mapping intensity measure
    type objects to dictionary of corresponding coefficients values.
    """
    global COEFFS
    table = COEFFS.splitlines()
