...
class TestSmoothingUtils(unittest.TestCase):
    ...
    def test_get_even_magnitude_completeness(self):
        ...
        comp_table = np.array([[1990.0, 4.0], [1960.0, 4.5], [1900.0, 4.8]])
        ...
        np.testing.assert_array_almost_equal(
            expected_table,
            utils.get_even_magnitude_completeness(comp_table, self.catalogue)[
                0
            ],
        )

        # Common case - only one value
        comp_table = np.array([[1990.0, 4.0]])
        np.testing.assert_array_almost_equal(
            np.array([[1990.0, 4.0]]),
            utils.get_even_magnitude_completeness(comp_table, self.catalogue)[
                0
    ...
    def test_get_adjustment_factor(self):
        ...
        self.assertAlmostEqual(
            tfact,
            _get_adjustment(
                4.2, 1995.0, comp_table[0, 1], comp_table[:, 0], tfact
            ),
        )
        ...
        self.assertAlmostEqual(
            tfact,
            _get_adjustment(
                4.7, 1985.0, comp_table[0, 1], comp_table[:, 0], tfact
            ),
        )
        ...
        self.assertFalse(
            _get_adjustment(
                3.8, 1990.0, comp_table[0, 1], comp_table[:, 0], tfact
            )
        )
        ...
        self.assertFalse(
            _get_adjustment(
                4.8, 1950.0, comp_table[0, 1], comp_table[:, 0], tfact
            )
        )

        # Good case when only one completeness period is needed
        comp_table = np.array([[1960.0, 4.5]])
        self.assertAlmostEqual(
            1.0,
            _get_adjustment(
                5.0, 1990.0, comp_table[0, 1], comp_table[:, 0], tfact
            ),
        )
        # Bad case when only one completeness period is needed
        self.assertFalse(
            _get_adjustment(
                4.0, 1990.0, comp_table[0, 1], comp_table[:, 0], tfact
            )
        )
    ...
    def test_weichert_factor(self):
        ...
        beta = 0.8 * np.log(10.0)
        end_year = 2006.0
        comp_table = np.array(
            [[1933.0, 4.0], [1900.0, 5.0], [1850.0, 6.0], [1850.0, 7.0]]
        )
        self.assertAlmostEqual(
            0.0124319686,
            utils.get_weichert_factor(
                beta, comp_table[:, 1], comp_table[:, 0], end_year
            )[0],
        )

        # Test 2: Single value of completeness
        comp_table = np.array([[1960.0, 4.0]])
        self.assertAlmostEqual(
            1.0 / (2006.0 - 1960.0 + 1.0),
            utils.get_weichert_factor(
                beta, comp_table[:, 1], comp_table[:, 0], end_year
            )[0],
        )
...
class TestSmoothedSeismicity(unittest.TestCase):
    ...
    def test_instantiation(self):
        ...
        self.grid_limits = Grid.make_from_list(
            [35.0, 40.0, 0.5, 40.0, 45.0, 0.5, 0.0, 40.0, 20.0]
        )
        expected_dict = {
            "beta": None,
            "bval": None,
        ...
        expected_dict["beta"] = np.log(10.0)
        for key in expected_dict:
            self.assertAlmostEqual(
                self.model.__dict__[key], expected_dict[key]
            )
    ...
    def test_get_2d_grid(self):
        ...
        mags = 5.0 * np.ones(6)
        years = 2000.0 * np.ones(6)
        expected_result = np.zeros(100, dtype=int)
        expected_result[[9, 28, 46, 64, 82, 90]] = 1
        np.testing.assert_array_almost_equal(
            expected_result,
            self.model.create_2D_grid_simple(
                lons, lats, years, mags, comp_table
            ),
        )
    ...
    def test_get_3d_grid(self):
        ...
        self.catalogue.data["longitude"] = np.hstack(
            [np.arange(35.0, 41.0, 1.0), np.arange(35.0, 41.0, 1.0)]
        )
        self.catalogue.data["latitude"] = np.hstack(
            [np.arange(40.0, 46.0, 1.0), np.arange(40.0, 46.0, 1.0)]
        )
        self.catalogue.data["depth"] = np.hstack(
            [10.0 * np.ones(6), 30.0 * np.ones(6)]
        )
        self.catalogue.data["magnitude"] = 4.5 * np.ones(12)
        self.catalogue.data["year"] = 1990.0 * np.ones(12)

        # Case 1 - one depth layer
        self.grid_limits = Grid.make_from_list(
            [35.0, 40.0, 0.5, 40.0, 45.0, 0.5, 0.0, 40.0, 40.0]
        )
        self.model = SmoothedSeismicity(self.grid_limits, bvalue=1.0)
        [gx, gy] = np.meshgrid(
            np.arange(35.25, 40.0, 0.5), np.arange(40.25, 45.0, 0.5)
        )
        ...
        expected_count[
            [9, 28, 46, 64, 82, 90, 109, 128, 146, 164, 182, 190]
        ] = 1.0
        expected_result[:, -1] = expected_count
        expected_result[:, 2] = np.hstack(
            [10.0 * np.ones(100), 30.0 * np.ones(100)]
        )
        self.model.create_3D_grid(self.catalogue, comp_table)
        np.testing.assert_array_almost_equal(expected_result, self.model.data)
    ...
    def test_csv_writer(self):
        ...
        self.grid_limits = [35.0, 40.0, 0.5, 40.0, 45.0, 0.5, 0.0, 40.0, 20.0]
        self.model = SmoothedSeismicity(self.grid_limits, bvalue=1.0)
        self.model.data = np.array(
            [[1.0, 1.0, 10.0, 4.0, 4.0, 1.0], [2.0, 2.0, 20.0, 8.0, 8.0, 1.0]]
        )
    ...
    def test_analysis_Frankel_comparison(self):
        ...
        config = {"Length_Limit": 3.0, "BandWidth": 50.0, "increment": 0.1}
        ...
        np.genfromtxt(os.path.join(BASE_PATH, FRANKEL_OUTPUT_FILE))
        # Run analysis
        ...
        self.assertTrue(
            fabs(np.sum(output_data[:, -1]) - np.sum(output_data[:, -2])) < 1.0
        )
        self.assertTrue(fabs(np.sum(output_data[:, -1]) - 390.0) < 1.0)
