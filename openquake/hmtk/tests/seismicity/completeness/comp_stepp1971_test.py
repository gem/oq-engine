...
class TestBilinearResiduals(unittest.TestCase):
    ...
    def setUp(self):
        self.xdata = np.arange(0.0, 11.0, 1.0)
        self.params = [2.0, -1.0, 5.0, 1.0]
        self.ydata = np.zeros(11, dtype=float)
        for ival in range(0, 11):
            self.ydata[ival] = piecewise_linear_scalar(
                self.params, self.xdata[ival]
            )
    ...
    def test_bilinear_residuals_stepp(self):
        ...
        self.assertAlmostEqual(
            np.sum(offset1**2.0),
            get_bilinear_residuals_stepp(
                self.params[1:], self.xdata, y_offset1, self.params[0]
            ),
        )
        ...
        self.assertAlmostEqual(
            0.0,
            get_bilinear_residuals_stepp(
                self.params[1:], self.xdata, y_offset0, self.params[0]
            ),
        )
...
class TestSteppCompleteness(unittest.TestCase):
    ...
    def test_stepp_get_windows(self):
        ...
        decimal_years = np.array([1950.0, 1970.0, 1990.0, 2010.0])
        self.config["time_bin"] = 20.0
        start_year_1, time_bins_1 = self.process._get_time_limits_from_config(
            self.config, decimal_years
        )
        self.assertAlmostEqual(start_year_1, 1950.0)
        self.assertTrue(
            np.allclose(time_bins_1, np.array([1990.0, 1970.0, 1950.0]))
        )
        self.assertAlmostEqual(self.process.end_year, 2010.0)
        # Test 2: Time step larger than catalogue duration
        decimal_years = np.array([2000.0, 2010.0])
        with self.assertRaises(ValueError) as ae:
            _, _ = self.process._get_time_limits_from_config(
                self.config, decimal_years
            )
            self.assertEqual(
                str(ae.exception),
        ...
        decimal_years = np.array([1950.0, 2010.0])
        self.config["time_bin"] = [2010.0, 1990.0, 1970.0, 1950]
        start_year_1, time_bins_1 = self.process._get_time_limits_from_config(
            self.config, decimal_years
        )
        self.assertAlmostEqual(start_year_1, 1950.0)
        self.assertTrue(
            np.allclose(
                time_bins_1, np.array([2010.0, 1990.0, 1970.0, 1950.0])
            )
        )
        self.assertAlmostEqual(self.process.end_year, 2010.0)
        # Test 4: Incorrect time input as bins
        self.config["time_bin"] = [2010.0, 1970.0, 1990.0, 1950]
        with self.assertRaises(ValueError) as ae:
            _, _ = self.process._get_time_limits_from_config(
                self.config, decimal_years
            )
    ...
    def test_stepp_get_magnitudes(self):
        ...
        with self.assertRaises(ValueError) as ae:
            ...
            self.assertEqual(
                str(ae.exception), "Bin width greater than magnitude range!"
            )
    ...
    def test_stepp_count_magnitudes(self):
        ...
        self.assertTrue(
            np.allclose(n_years, np.array([40.0, 30.0, 20.0, 10.0]))
        )
    ...
    def test_fit_bilinear_function(self):
        ...
        self.xdata = np.arange(0.0, 21.0, 1.0)
        ...
        self.assertAlmostEqual(
            np.log10(comp_time), np.log10(100000219.8145678), places=3
        )
        self.assertAlmostEqual(gradient, -1.000, places=3)
        expected_model_line = 10.0 ** (-0.5 * self.xdata + 10.0)
        for ival in range(0, len(model_line)):
            self.assertAlmostEqual(
                np.log10(model_line[ival]),
    ...
    def test_get_completeness_points(self):
        ...
        n_years = np.hstack([1.0, 5.0, np.arange(10.0, 110.0, 10.0)])
        ...
        for ival in range(0, number_values):
            yvals_1[ival] = 10.0 ** piecewise_linear_scalar(
                params1, np.log10(n_years[ival])
            )
            yvals_2[ival] = 10.0 ** piecewise_linear_scalar(
                params2, np.log10(n_years[ival])
            )
            yvals_3[ival] = 10.0 ** piecewise_linear_scalar(
                params3, np.log10(n_years[ival])
            )

        test_sigma = np.column_stack([yvals_1, yvals_2, yvals_3])
        (ntime, nmags) = np.shape(test_sigma)
        completeness_time, gradients, _ = self.process.get_completeness_points(
            n_years, test_sigma, nmags, ntime
        )
    ...
    def test_complete_stepp_analysis_basic(self):
        ...
        expected_completeness_table = np.array(
            [
                [1990.0, 4.0],
                [1962.0, 4.5],
                [1959.0, 5.0],
