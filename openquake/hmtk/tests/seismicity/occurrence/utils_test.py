...
class RecurrenceTableTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        filename = os.path.join(
            self.BASE_DATA_PATH, "completeness_test_cat.csv"
        )
        ...
        # Read initial dataset
        filename = os.path.join(
            self.BASE_DATA_PATH, "recurrence_test_cat_B.csv"
        )
        test_data = np.genfromtxt(filename, delimiter=",", skip_header=1)
        # Create the catalogue A
        ...
        filename = os.path.join(
            self.BASE_DATA_PATH, "recurrence_table_test_A.csv"
        )
        self.true_tableA = np.genfromtxt(filename, delimiter=",")

        # Read the verification table A
        filename = os.path.join(
            self.BASE_DATA_PATH, "recurrence_table_test_B.csv"
        )
    ...
    def test_recurrence_table_A(self):
        ...
        self.assertTrue(
            np.allclose(
                self.true_tableA,
                rec_utils.recurrence_table(
                    self.catalogueA.data["magnitude"],
                    magnitude_interval,
    ...
    def test_recurrence_table_B(self):
        ...
        self.assertTrue(
            np.allclose(
                self.true_tableB,
                rec_utils.recurrence_table(
                    self.catalogueB.data["magnitude"],
                    magnitude_interval,
    ...
    def test_input_checks_raise_error(self):
        ...
        self.assertRaises(
            ValueError,
            rec_utils.input_checks,
            catalogue,
            config,
            fake_completeness_table,
        )
    ...
    def test_generate_magnitudes(self):
        ...
        for _ in range(0, 100):
            ...
            cat = Catalogue.make_from_dict(
                {"magnitude": mags, "year": np.zeros(len(mags), dtype=int)}
            )
...
class TestCompletenessCounts(unittest.TestCase):
    ...
    def test_completeness_counts(self):
        ...
        self.assertEqual(
            self.catalogue.get_number_events(), int(np.sum(n_obs))
        )
