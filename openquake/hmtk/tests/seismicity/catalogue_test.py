...
class CatalogueTestCase(unittest.TestCase):
    ...
    def test_load_from_array(self):
        ...
        np.testing.assert_allclose(
            cat.data["magnitude"], self.data_array[:, 1]
        )
        np.testing.assert_allclose(
            cat.data["year"], self.data_array[:, 0].astype(int)
        )
    ...
    def test_get_decimal_time(self):
        ...
        expected_dec_time = decimal_time(
            time_dict["year"],
            time_dict["month"],
            time_dict["day"],
            time_dict["hour"],
        ...
        np.testing.assert_array_almost_equal(
            expected_dec_time, cat.get_decimal_time()
        )
    ...
    def test_hypocentres_as_mesh(self):
        ...
        cat = Catalogue()
        cat.data["longitude"] = np.array([2.0, 3.0])
        cat.data["latitude"] = np.array([2.0, 3.0])
        cat.data["depth"] = np.array([2.0, 3.0])
    ...
    def test_hypocentres_to_cartesian(self):
        ...
        cat.data["latitude"] = np.array([2.0, 3.0])
        cat.data["depth"] = np.array([2.0, 3.0])
        expected_data = spherical_to_cartesian(
            cat.data["longitude"], cat.data["latitude"], cat.data["depth"]
        )
...
class TestGetDistributions(unittest.TestCase):
    ...
    def test_depth_distribution_no_depth_error(self):
        ...
        depth_bins = np.arange(0.0, 60.0, 10.0)
        self.catalogue.data["depth"] = np.array([])
        with self.assertRaises(ValueError) as ae:
            self.catalogue.get_depth_distribution(depth_bins)
        self.assertEqual(str(ae.exception), "Depths missing in catalogue")

    def test_depth_distribution_simple(self):
        # Tests the calculation of the depth histogram with no uncertainties
        # Without normalisation
        self.catalogue.data["depth"] = np.arange(5.0, 50.0, 5.0)
        depth_bins = np.arange(0.0, 60.0, 10.0)
        expected_array = np.array([1.0, 2.0, 2.0, 2.0, 2.0])
        np.testing.assert_array_almost_equal(
            expected_array, self.catalogue.get_depth_distribution(depth_bins)
        )
        # With normalisation
        np.testing.assert_array_almost_equal(
            expected_array / np.sum(expected_array),
            self.catalogue.get_depth_distribution(
    ...
    def test_depth_distribution_uncertainties(self):
        ...
        depth_bins = np.arange(-10.0, 70.0, 10.0)
        expected_array = np.array([0.0, 1.5, 2.0, 2.0, 2.0, 1.5, 0.0])
        hist_array = (
            self.catalogue.get_depth_distribution(depth_bins, bootstrap=1000)
            / 1000
        )
        diff = np.fabs(hist_array - expected_array).max()
        self.assertTrue(diff <= 0.052)
        # With normalisation
        expected_array = np.array([0.0, 0.16, 0.22, 0.22, 0.22, 0.16, 0.01])
        hist_array = self.catalogue.get_depth_distribution(
            depth_bins, normalisation=True, bootstrap=1000
        )
        diff = np.fabs(hist_array - expected_array).max()
...
class TestMagnitudeDepthDistribution(unittest.TestCase):
    ...
    def setUp(self):
        self.catalogue = Catalogue()
        x, y = np.meshgrid(
            np.arange(5.0, 50.0, 10.0), np.arange(5.5, 9.0, 1.0)
        )
        nx, ny = np.shape(x)
    ...
    def test_distribution_no_uncertainties(self):
        ...
        # Without normalisation
        depth_bins = np.arange(0.0, 60.0, 10.0)
        mag_bins = np.arange(5.0, 10.0, 1.0)
        expected_array = np.ones(
            [len(mag_bins) - 1, len(depth_bins) - 1], dtype=float
        )
        np.testing.assert_array_almost_equal(
            expected_array,
    ...
    def test_depth_to_pmf_default(self):
        ...
        depth_bins = np.array([0.0, 5.0, 10.0, 15.0])
        output_pmf = self.catalogue.get_depth_pmf(
            depth_bins, default_depth=10.0
        )
    ...
    def test_mag_depth_distribution_uncertainties(self):
        ...
        np.testing.assert_array_almost_equal(
            test_array[:, 0], np.zeros(4, dtype=float)
        )
...
class TestMagnitudeTimeDistribution(unittest.TestCase):
    ...
    def setUp(self):
        self.catalogue = Catalogue()
        x, y = np.meshgrid(
            np.arange(1915.0, 2010.0, 10.0), np.arange(5.5, 9.0, 1.0)
        )
        nx, ny = np.shape(x)
        self.catalogue.data["magnitude"] = (y.reshape([nx * ny, 1])).flatten()
    ...
    def test_magnitude_time_distribution_no_uncertainties(self):
        # Tests the magnitude-depth distribution without uncertainties
        mag_range = np.arange(5.0, 10.0, 1.0)
        time_range = np.arange(1910.0, 2020.0, 10.0)
        # Without normalisation
        expected_array = np.ones(
