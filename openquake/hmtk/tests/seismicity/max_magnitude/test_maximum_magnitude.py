...
BASE_DATA_PATH = os.path.join(
    os.path.dirname(__file__), "./../completeness/data"
)
...
class MmaxTestCase(unittest.TestCase):
    ...
    def test_get_magnitude_vector_properties(self):
        ...
        # minimum magnitude
        neq, mmin = _get_magnitude_vector_properties(
            test_catalogue, self.config
        )
    ...
    def test_cumulative_moment(self):
        ...
        self.assertAlmostEqual(
            ...
            self.model.cumulative_moment(
                self.catalogue.data["year"], self.catalogue.data["magnitude"]
            ),
            1,
        )
...
class TestKijkoSellevolBayes(unittest.TestCase):
    ...
    def test_ksb_intfunc(self):
        ...
        # Bad case neq = 0.
        self.assertAlmostEqual(
            self.model._ksb_intfunc(mval, 0.0, mmin, pval0, qval0), 1.0
        )
    ...
    def _get_pval_qval(self, bval, sigma_b):
        ...
        sigma_beta = sigma_b * np.log(10.0)
        pval = beta / (sigma_beta**2.0)
        qval = (beta / sigma_beta) ** 2.0
        return pval, qval
    ...
    def test_kijko_npg_intfunc_simps(self):
        ...
        self.assertAlmostEqual(
            ...
            self.model._kijko_npg_intfunc_simps(
                mvals, test_mag, np.max(test_mag), h_fact, 100.0
            ),
        )
