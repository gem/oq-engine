...
AL83_FIG2_MODEL = np.genfromtxt(
    os.path.join(BASE_DATA_PATH, "anderson_luco_arbitrary_results.dat")
)
AL83_INC_DATA = np.genfromtxt(
    os.path.join(BASE_DATA_PATH, "anderson_luco_arbitrary_incremental.dat")
)
        ...
        for iloc, mag in enumerate(self.magnitudes):
            self.assertAlmostEqual(
                ...
                self.model.cumulative_value(
                    moment_slip, self.mmax, mag, self.bbar, self.dbar
                ),
                7,
            )
...
class TestAndersonLucoArbitrary(unittest.TestCase):
    ...
    def test_get_mfd(self):
        ...
        for iloc, model_type in enumerate(["First", "Second", "Third"]):
            ...
            self.model.setUp(self.config)
            self.model.get_mmax(self.config, self.msr, 0.0, 30000.0)
            test_output = self.model.get_mfd(20.0, 30000.0)
            print(AL83_INC_DATA[:, iloc], test_output[2])
            np.testing.assert_array_almost_equal(
                AL83_INC_DATA[:, iloc], test_output[2]
            )
            ...
            self.model.setUp(self.config)
            self.model.get_mmax(self.config, self.msr, 0.0, 30000.0)
            self.model.get_mfd(20.0, 30000.0)
