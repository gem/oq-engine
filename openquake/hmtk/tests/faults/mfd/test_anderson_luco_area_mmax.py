...
MAGNITUDES = np.arange(5.0, 8.1, 0.1)

BASE_DATA_PATH = os.path.join(os.path.dirname(__file__), "data")
BUN07_FIG1 = np.genfromtxt(
    os.path.join(BASE_DATA_PATH, "anderson_luco_area_mmax_results.dat")
)
AL83_AREA_MMAX_INC = np.genfromtxt(
    os.path.join(BASE_DATA_PATH, "anderson_luco_area_mmax_incremental.dat")
)
...
class TestType1Recurrence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mmax = 8.0
        self.bbar = 1.0 * log(10.0)
        self.dbar = 1.5 * log(10.0)
        self.beta = None
    ...
    def test_recurrence_model_type1(self):
        ...
        self.beta = np.sqrt(
            (1e-5 * (10.0**16.05)) / ((30 * 1e10) * (60.0 * 1e5))
        )
        expected_results = BUN07_FIG1[:, 1]
        for iloc, mag in enumerate(self.magnitudes):
            self.assertAlmostEqual(
                expected_results[iloc],
                self.model.cumulative_value(
                    1.0, self.mmax, mag, self.bbar, self.dbar, self.beta
                ),
                7,
            )
...
class TestType2Recurrence(unittest.TestCase):
    ...
    def test_recurrence_model_type1(self):
        # Tests the recurrence function In all cases if bbar > dbar
        # (1.5) then models will fail!  No comparison figures found in
        # Anderson & Luco (1983) - but a comparison figure is found in
        # Bungum (2007)
        # Tests 1 - master case - reproduces the Model 2
        # line of Figure 1 in Bungum (2007) slip = 1 mm/yr,
        # shear_modulus = 30 GPa, fault width = 60 km,
        # disp_length_ratio =1E-5
        self.beta = np.sqrt(
            (1e-5 * (10.0**16.05)) / ((30 * 1e10) * (60.0 * 1e5))
        )
...
class TestType3Recurrence(unittest.TestCase):
    ...
    def test_recurrence_model_type1(self):
        ...
        # Tests 1 - master case - reproduces the Model 3 line of
        # Figure 1 in Bungum (2007) slip = 1 mm/yr, shear_modulus = 30
        # GPa, fault width = 60 km, disp_length_ratio =1E-5
        self.beta = np.sqrt(
            (1e-5 * (10.0**16.05)) / ((30 * 1e10) * (60.0 * 1e5))
        )
...
class TestAndersonLucoArbitrary(unittest.TestCase):
    """
    Tests the Anderson & Luco Arbitrary models :class:
    `openquake.hmtk.faults.mfd.anderson_luco_arbitrary.AndersonLucoArbitrary`
    """
    ...
    def test_get_mfd(self):
        ...
        for iloc, model_type in enumerate(["First", "Second", "Third"]):
            ...
            np.testing.assert_array_almost_equal(
                AL83_AREA_MMAX_INC[:, iloc], test_output[2]
            )
