...
class TestStrainCsvReader(unittest.TestCase):
    ...
    def test_check_invalid_longitude(self):
        ...
        """
        self.model = ReadStrainCsv("some_random_file")
        self.model.strain.data = {"longitude": np.array([30.0, 180.0, 270.0])}
        self.model._check_invalid_longitudes()
        np.testing.assert_array_almost_equal(
            self.model.strain.data["longitude"], np.array([30.0, 180.0, -90.0])
        )
    ...
    def test_read_file(self):
        ...
        np.testing.assert_array_almost_equal(
            exp_latitude, strain.data["latitude"]
        )
        ...
        np.testing.assert_array_almost_equal(
            np.log10(str_2nd_inv[4:]), np.log10(strain.data["2nd_inv"][4:])
        )
...
class TestStrainCsvWriter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model.data = dict(
            [
                ("longitude", np.array([30.0, 30.0, 30.0])),
                ("latitude", np.array([30.0, 30.0, 30.0])),
                ("exx", np.array([1.0, 2.0, 3.0])),
                ("eyy", np.array([1.0, 2.0, 3.0])),
                ("exy", np.array([1.0, 2.0, 3.0])),
            ]
        )
        self.filename = None
    ...
    def test_slice_rates_to_data_as_array(self):
        ...
        """
        self.writer = WriteStrainCsv("some_random_file")
        self.model.seismicity_rate = np.array(
        ...
        np.testing.assert_array_almost_equal(
            self.model.data["5.500"], np.array([1.0, 1.0, 1.0])
        )

        np.testing.assert_array_almost_equal(
            self.model.data["6.600"], np.array([2.0, 2.0, 2.0])
        )
