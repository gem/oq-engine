...
class TestGeodeticStrain(unittest.TestCase):
    ...
    def test_secondary_strain_data_input_error(self):
        ...
        self.assertEqual(
            str(ae.exception), "Strain data not input or incorrectly formatted"
        )
        ...
        self.assertEqual(
            str(ae.exception), "Essential strain information exy missing!"
        )
    ...
    def test_secondary_strain_data_with_input(self):
        ...
        self.data = {
            "longitude": np.array([10.0, 20.0, 30.0]),
            "latitude": np.array([10.0, 20.0, 30.0]),
            "exx": 1e-9 * np.array([100.0, 10.0, 1.0]),
            "eyy": 1e-9 * np.array([50.0, 5.0, 0.5]),
            "exy": 1e-9 * np.array([10.0, 1.0, 0.1]),
        }
        self.model.get_secondary_strain_data(self.data)
