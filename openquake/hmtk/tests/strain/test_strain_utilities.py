...
TAPER_FUNCTION_DATA = np.genfromtxt(
    os.path.join(BASE_DATA_PATH, "taper_function_log_data.txt"), delimiter=","
)
...
class TestStrainUtils(unittest.TestCase):
    ...
    def test_moment_magnitude_function(self):
        ...
        self.assertAlmostEqual(
            6.0, moment_magnitude_function(moment_function(6.0))
        )
    ...
    def test_calculate_taper_function(self):
        ...
        np.testing.assert_array_almost_equal(
            TAPER_FUNCTION_DATA, np.log10(obs_data)
        )
