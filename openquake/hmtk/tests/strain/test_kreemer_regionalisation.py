...
from openquake.hmtk.strain.regionalisation.kreemer_regionalisation import (
    _build_kreemer_cell,
    KreemerRegionalisation,
)

BASE_DATA_PATH = os.path.join(os.path.dirname(__file__), "strain_data")
KREEMER_2POLY_FILE = "kreemer_2poly_sample.txt"
KREEMER_2REG_FILE = os.path.join(
    BASE_DATA_PATH, "kreemer_2poly_sample_2types.txt"
)
KREEMER_POLY_SAMPLE = getlines(
    os.path.join(BASE_DATA_PATH, KREEMER_2POLY_FILE)
)
...
class TestBuildKreemerCell(unittest.TestCase):
    ...
    def test_build_kreemer_polygon(self):
        ...
        np.testing.assert_array_almost_equal(
            expected_output_1, _build_kreemer_cell(self.data, 0)
        )

        np.testing.assert_array_almost_equal(
            expected_output_2, _build_kreemer_cell(self.data, 6)
        )
...
class TestKreemerRegionalisation(unittest.TestCase):
    ...
    def test_point_in_tectonic_region(self):
        ...
        np.testing.assert_array_almost_equal(
            ...
            np.array([0.0, 1.0, 1.0, 1.0, 1.0, 0.0, 0.0]),
        )
        ...
        expected_output = [
            {
                "area": 1.0,
                "cell": None,
                "lat_lims": np.array([-66.0, -65.5]),
                "long_lims": np.array([179.4, 180.0]),
                "region_type": "R",
            },
            {
                "area": 1.0,
                "cell": None,
                "lat_lims": np.array([-66.0, -65.5]),
                "long_lims": np.array([-180.0, -179.4]),
                "region_type": "R",
            },
        ]
    ...
    def test_full_regionalisation_workflow(self):
        ...
        np.testing.assert_array_equal(
            self.model.data["region"], np.array([b"R", b"C", b"IPL"])
        )
        np.testing.assert_array_equal(
            self.model.data["area"], np.array([1.0, 5.0, np.nan])
        )
