...
class ReadStrainCsv(object):
    ...
    def read_data(self, scaling_factor=1e-9, strain_headers=None):
        ...
        for key in self.strain.data.keys():
            if "region" in key:
                self.strain.data[key] = np.array(
                    self.strain.data[key], dtype="S13"
                )
            else:
    ...
    def _check_invalid_longitudes(self):
        ...
        if np.any(idlon):
            self.strain.data["longitude"][idlon] = (
                self.strain.data["longitude"][idlon] - 360.0
            )
...
class WriteStrainCsv(object):
    ...
    def write_file(self, strain, scaling_factor=1e-9):
        ...
        writer = csv.DictWriter(outfile, fieldnames=output_variables)
        writer.writeheader()
    ...
    def slice_rates_to_data(self, strain):
        ...
        cond = isinstance(strain.target_magnitudes, np.ndarray) or isinstance(
            strain.target_magnitudes, list
        )
        if cond:
        ...
        assert (
            np.shape(strain.seismicity_rate)[0]
