...
@tasks.oqtask
def insert_into_gmf_agg(job_id, gmf_set_id):
    """
    ...
    WHERE a.gmf_set_id=b.id AND gmf_set_id=%d
    GROUP BY gmf_collection_id, imt, sa_damping, sa_period, location;
    '''
    curs = db.connections['reslt_writer'].cursor()
    with db.transaction.commit_on_success(using='reslt_writer'):
        curs.execute(insert_query % gmf_set_id)
        curs.execute('DELETE FROM hzrdr.gmf WHERE gmf_set_id=%d' % gmf_set_id)
...
def insert_into_gmf_agg_arg_gen(job):
    ...
    for output in job.output_set.filter(output_type='gmf'):
        for gmfset in output.gmfcollection.gmfset_set.filter():
            yield job.id, gmfset.id
