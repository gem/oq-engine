...
@tasks.oqtask
def gmfs(job_id, sites, rupture, gmf_id, task_no, task_seed, realizations):
    """
    A celery task wrapper function around :func:`compute_gmfs`.
    See :func:`compute_gmfs` for parameter definitions.
    """
    numpy.random.seed(task_seed)
    compute_gmfs(job_id, sites, rupture, gmf_id, task_no, realizations)


def compute_gmfs(job_id, sites, rupture, gmf_id, task_no, realizations):
    """
    ...
    with EnginePerformanceMonitor('saving gmfs', job_id, gmfs):
        save_gmf(gmf_id, task_no, gmf, sites)


@transaction.commit_on_success(using='job_init')
def save_gmf(gmf_id, task_no, gmf_dict, sites):
    """
    ...
    def task_arg_gen(self, block_size):
        ...
        for task_no, sites in enumerate(blocks):
            ...
            yield (self.job.id, models.SiteCollection(sites),
                   self.rupture, self.gmf.id, task_no, task_seed,
