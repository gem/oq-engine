...
class _GmfsPerSES(collections.Sequence):
    """
    A sequence-like object storing all the GMFs generated by the
    ruptures in a given SES.
    """
    def __init__(self, gmfs, ses):
        self._gmfs = gmfs
        self._ses = ses
        self.investigation_time = ses.investigation_time
        self.stochastic_event_set_id = ses.stochastic_event_set_id
...
class GmfCollection(djm.Model):
    ...
    # NB: uses the helper view gmf_output
    ...
    def get_gmfs_per_ses(self, location=None, orderby=False):
        ...
        if children:  # complete logic tree
            ...
            if all_gmfs:
                yield _GmfsPerSES(all_gmfs, ses)
            return
        # leaf of the tree
    ...
    for imt, sa_period, sa_damping in imts:
        ...
        for gmf in GmfScenario.objects.filter(
            ...
            for i, gmv in enumerate(gmf.gmvs):  # i is the realization index
                nodes[i].append(_GroundMotionFieldNode(gmv, loc.x, loc.y))
