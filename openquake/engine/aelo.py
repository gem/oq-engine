...
def get_params_from(inputs, mosaic_dir, exclude=(), ini=None):
    ...
    # NB: or the description is passed explicitly and the siteid does not need
    # to be a valid custom_site_id, or it is generated from the siteid, which
    # must be a valid custom_site_id in that case
    params['description'] = inputs.get(
        'description', f'AELO for {inputs["siteid"]}')
    ...
    # NB: aelo_run in the WebUI will replace params['siteid'] with a valid
    # custom_site_id generated by get_custom_site_id
