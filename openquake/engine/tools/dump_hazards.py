...
class HazardDumper(object):
    ...
    def __init__(self, conn, format='text'):
        self.conn = conn
        ...
        out_dir = tempfile.mkdtemp(prefix='hazard_calculation-')
    ...
    def dump(self, hazard_calculation_id):
        ...
        return tardir(self.dest, 'hc-%s' % hazard_calculation_id)


# this is used in the test
def hazard_restore(tar):
    "Restore a tar file generated by the HazardDumper with shell commands"
    cmds = [
        'cd /tmp',
        'tar xf %s' % tar,
        'cd %s',
        'bash RESTORE.sh',
        'psql openquake -f RESTORE.sql',
        'rm -rf %s'
    ]
    for cmd in cmds:
        os.system(cmd)
