...
def convolution(
    ...
    **kwargs
    ...
    for iset, (sset, uset) in enumerate(zip(ssets, usets)):
        ...
        if uset:
            # 'sset' is the set of IDs of the sources to be processed
            # 'uset' is the set of IDs for the uncertainties
            # 'an01' is an Analysis object
            # 'rlzgroups' is a dictionary with keys as tuples containing the
            #   uncertainty ID and the associated source ID and with values a
            #   list of lists each one containing the IDs of the correlated
            #   realizations. The cardinality of the list of lists must be the
            #   same for all the sources.
            # 'res' is an integer with the resolution for the histogram
            # 'imt' is a string representing the IMT
            # 'atype' the calculation type
        ...
        # Update the final distribution by convolving the two distributions.
        if iset == 0:
            acc = h
        else:
            acc *= h
