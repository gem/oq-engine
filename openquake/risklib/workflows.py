...
class RiskModel(collections.Mapping):
    ...
    def gen_outputs(self, getters):
        """
        Yield the output generated by each getter and loss type for each
        workflow.
        """
        for imt, taxonomies in self.get_imt_taxonomies():
            for getter in getters:
                for taxonomy, assets, hazards, epsilons in getter.get_all(
                        imt, taxonomies):
                    if not assets:
                        logging.debug('No assets for IMT=%s, taxonomy=%s',
                                      imt, taxonomy)
                        continue
...
class HazardGetter(object):
    ...
    :param site_ids: IDs of a hazard sites
    :param hazards: hazard data on that sites
    :param assets: assets on that sites
    """
    def __init__(self, site_ids, hazard_by_site, assets_by_site):
    ...
    def get_all(self, imt, taxonomies):
        """
        Yield taxonomy, assets, hazards, epsilons
        """
        haz = [h[imt] for h in self.hazards]
        for taxonomy in taxonomies:
            assets, hazards, epsilons = [], [], []
            for asset, hazard in zip(self.assets, haz):
