...
class RiskModel(collections.Mapping):
    ...
    def expand(self, hazards, rlzs_assoc):
        """
        :param hazards: a list of hazards per site, as dictionaries
        :param rlzs_assoc: how to combine the hazards
        :returns: a dictionary with the hazards by realization
        """
        hazards_by_rlz = AccumDict()  # rlz -> hazards
        for h in hazards:
            items = rlzs_assoc.combine(h).iteritems()
            hazards_by_rlz += {rlz: [val] for rlz, val in items}
        return hazards_by_rlz
    ...
    def gen_output(self, riskinput, rlzs_assoc):
        ...
        for taxonomy in riskinput.taxonomies:
            ...
            if not assets:
                continue
            hazards_by_rlz = self.expand(hazards_[taxonomy], rlzs_assoc)
            ...
            for loss_type in workflow.loss_types:
                ...
                for rlz in rlzs_assoc.realizations:
                    results.append(
                        workflow(loss_type, assets, hazards_by_rlz[rlz],
                                 epsilons))
