...
class Workflow(object):
    ...
    def compute(self, loss_type, assets, hazards, epsilons):
        if isinstance(hazards[0], AccumDict):
...
class RiskModel(collections.Mapping):
    ...
    def gen_outputs(self, riskinputs, rlzs_assoc=None):
        """
        ...
        """
        if rlzs_assoc is None:  # scenario case, single realization
            for riskinput in riskinputs:
                out = self.gen_output(riskinput)
                for key in sorted(out):
                    yield out[key]

        else:  # classical and event based calculations
            dic = groupby(riskinputs, operator.attrgetter('trtgsim'))
            out = {}
            for key in dic:
                for riskinput in dic[key]:
                    for out in self.gen_output(riskinput):
                        yield out
            wf = functools.partial(workflow, loss_type, assets,
                                   epsilons=epsilons)
            result = hazards.combine().apply(wf)
            yield loss_type, [result[rlz]
    ...
    def gen_output(self, riskinput, rlzs_assoc):
        ...
        for taxonomy in riskinput.taxonomies:
            ...
            if not assets:
                continue
            all_hazards = self.expand(hazards_[taxonomy], rlzs_assoc)
            ...
            for loss_type in workflow.loss_types:
                ...
                for hazards in all_hazards:
                    results.append(
                        workflow(loss_type, assets, hazards, epsilons))
                output[taxonomy, loss_type] = results
        return output
