...
"""
Module :mod:`openquake.pfd.scalerel.base` defines abstract base
...
def check_args(func, expected):
    """Validate that ``func`` has the ``expected`` positional argument names.

    Parameters
    ----------
    func : callable
        The function or method whose signature is checked.
    expected : list of str
        The exact ordered list of positional argument names required.

    Raises
    ------
    SyntaxError
        If ``func``'s positional arguments differ from ``expected``.
    """
