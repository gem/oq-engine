...
class Region(object):
    ...
    def sites(self):
        """
        Return the set of sites representing the center of
        the cells generated by using the given cell size
        and boundaries.
        """
...
class Grid(object):
    ...
    def __init__(self, region, cell_size):
        ...
        self.lleft_corner = self.region.lower_left_corner
    ...
    def site_inside(self, site):
        ...
        if not self.cell_centers:
            self.cell_centers = self.region.sites
        ...
        #
        # all the points with 2.25 < longitude <= 2.3
        # didn't have a corresponding grid point because the last cell
        # finished at longitude = 2.25. To avoid this, we include
        # now additional columns and rows, but the cell center points
        # are outside the region (for example (2.5, 1.0)).
        #
        # Since this code is used ALSO to get the grid points from
        # the sites that represent the center of the cells, this
        # is necessary in order to avoid the `Point outside the
        # region error`. This code needs a major refactoring.
        if site in self.cell_centers:
            return True
    ...
    def _latitude_to_row(self, latitude):
        ...
        latitude_offset = math.fabs(
            latitude - self.lleft_corner.latitude)
    ...
    def _row_to_latitude(self, row):
        ...
        return self.lleft_corner.latitude + ((row) * self.cell_size)
    ...
    def _longitude_to_column(self, longitude):
        ...
        longitude_offset = longitude - self.lleft_corner.longitude
    ...
    def _column_to_longitude(self, column):
        ...
        return self.lleft_corner.longitude + ((column) * self.cell_size)
