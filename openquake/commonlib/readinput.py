...
def get_source_models(oqparam):
    """
    Build all the source models generated by the logic tree and yield
    ...
    for i, (sm, weight, smpath) in enumerate(distinct(source_model_lt)):
...
def get_filtered_source_models(oqparam, sitecol):
    ...
    Yield :class:`openquake.commonlib.readinput.SourceModel` tuples,
    unless the filtered source model does not contain any source.
