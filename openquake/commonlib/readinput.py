...
class SourceModelFactory(object):
    def __init__(self, oqparam, gsim_lt, source_model_lt, monitor,
        ...
        self.grp_id = 0
        self.source_ids = set()
        self.mags = AccumDict(accum=set())  # TRT -> mags
    ...
    def apply_uncertainties(self, sm, sources, idx, dic):
        """
...
def get_source_models(oqparam, gsim_lt, source_model_lt, monitor,
    ...
    make_sm = SourceModelFactory(oqparam, gsim_lt, source_model_lt, monitor,
                                 in_memory, srcfilter)
    ...
    for sm in source_model_lt.gen_source_models(gsim_lt):
        make_sm.apply_uncertainties(sm, sources, idx, dic)
        yield sm
    if make_sm.mags and monitor.hdf5:
        mags_by_trt = {trt: sorted(ms) for trt, ms in make_sm.mags.items()}
