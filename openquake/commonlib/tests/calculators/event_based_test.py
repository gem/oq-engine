...
class EventBasedTestCase(CalculatorTestCase):
    ...
    def test_case_2(self):
        out = self.run_calc(case_2.__file__, 'job.ini',
                            exports='csv', concurrent_tasks=0)
        self.assertEqualFiles(
            'expected/0-SadighEtAl1997.csv',
            out['0-SadighEtAl1997.csv'])
    ...
    def test_case_5(self):
        ...
        out = self.run_calc(case_5.__file__, 'job.ini',
                            exports='csv', concurrent_tasks=0)
    ...
    def test_case_6(self):
        ...
        out = self.run_calc(case_6.__file__, 'job.ini',
                            exports='csv', concurrent_tasks=0)
        for fname in expected:
            self.assertEqualFiles('expected/%s' % fname, out[fname])
    ...
    def test_case_12(self):
        expected = [
            '0-SadighEtAl1997.csv',
            '1-BooreAtkinson2008.csv',
            'hazard_curve-smltp_b1-gsimltp_b1_b2-ltr_0.csv'
        ]
        out = self.run_calc(case_12.__file__, 'job.ini',
                            exports='csv', concurrent_tasks=0)
        for fname in expected:
            self.assertEqualFiles('expected/%s' % fname, out[fname])
    ...
    def test_case_13(self):
        expected = [
            '0-BooreAtkinson2008.csv',
            'hazard_curve-smltp_b1-gsimltp_b1-ltr_0.csv',
        ]
        out = self.run_calc(case_13.__file__, 'job.ini',
                            exports='csv', concurrent_tasks=0)
        for fname in expected:
            self.assertEqualFiles('expected/%s' % fname, out[fname])
