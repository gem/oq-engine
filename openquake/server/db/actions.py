    ...
    if len(outputs) > 0:
        ...
        for i, o in enumerate(outs):
            if not full and i >= 10:
                out += ' ... | %d additional output(s)' % (len(outs) - 10)
                truncated = True
                break
            out += '%4d | %s' % (o.id, o.display_name)
        if truncated:
            out += ('Some outputs where not shown. You can see the full list '
...
def get_log(db, job_id):
    ...
    for log in logs:
        ...
        out += '[%s #%d %s] %s' % (time, job_id, log.level, log.message)
    return out
