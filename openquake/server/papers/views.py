...
from openquake.baselib import writers, hdf5
...
FNAME = "inputs//papers_ses.hdf5" # Datastore (the hdf5 containing the SES ruptures)
TWO24 = 2 ** 24
...
def get_job_ctx(rup_id,
    ...
    [job] = engine.create_jobs([job_dict])
...
def run_scenario_calc_from_ses_rupture_ext(fname,
         rup_id,
         gmm_lt,
         site_model,
         imts,
         ...
         consequence,
         day_or_night,
         hazard_only=False): # By default compute risk too)
    """
...
def run_scenario_calc_from_ses_rupture(rup_id):
    dstore, rup_geojson = run_scenario_calc_from_ses_rupture_ext(
        fname=FNAME,
        rup_id=rup_id,
        gmm_lt=GMM_LT,
