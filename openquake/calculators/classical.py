...
class ClassicalCalculator(base.HazardCalculator):
    ...
    def execute(self):
        ...
        self.rlzs_by_gsim_list = self.submit_tasks(smap)
        rlzs_by_g = []
        for rlzs_by_gsim in self.rlzs_by_gsim_list:
    ...
    def post_execute(self, pmap_by_key):
        ...
        if nr:  # few sites, log the number of ruptures per magnitude
            logging.info('%s', nr)
        oq = self.oqparam
        slice_by_g = getters.get_slice_by_g(self.rlzs_by_gsim_list)
        ...
        with self.monitor('saving probability maps'):
            for key, pmap in pmap_by_key.items():
                if isinstance(key, str):  # disagg_by_src
                    ...
                    rlzs_by_gsim = self.rlzs_by_gsim_list[pmap.gidx]
