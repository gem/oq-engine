...
def plot_governing_mce(dstore, site_idx=0, update_dstore=False):
    ...
    DLL = mce_df['DLL']
    ax1.plot(T[2:], DLL[2:], 'kx', markersize=8, label='DLL', linewidth=1,
             linestyle='-')
    ax1.plot(T[2:], prob_mce[2:], 'bX', markersize=8, label='Probabilisitc MCE',
             linewidth=1, linestyle='-')
    upperlim = max(max(prob_mce), max(mce), max(det_mce), max(DLL))
    ax1.plot(T[2:], det_mce[2:], 'c^', markersize=8, label='Deterministic MCE',
             linewidth=1, linestyle='-')
    ax1.set_ylim([0.01, upperlim + 0.2])
    ax1.plot(T[2:], mce[2:], 'r', label='Governing MCE', linewidth=4, linestyle=':')
    ax1.grid('both')
    ...
    ax1.set_xlim([min(T[2:]), max(T)])

    plt.rcParams.update({'font.size': 15})
    ax2.plot(T[2:], DLL[2:], 'kx', markersize=8, label='DLL', linewidth=1,
             linestyle='-')
    ax2.plot(T[2:], prob_mce[2:], 'bX', markersize=8, label='Probabilisitc MCE',
             linewidth=1, linestyle='-')
    ax2.plot(T[2:], det_mce[2:], 'c^', markersize=8, label='Deterministic MCE',
             linewidth=1, linestyle='-')
    plt.ylim([0.01, upperlim + 0.2])
    ax2.plot(T[2:], mce[2:], 'r', label='Governing MCE', linewidth=4, linestyle=':')
    ax2.grid('both')
