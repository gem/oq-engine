...
def view_params(token, dstore):
    ...
    params = ['calculation_mode', 'number_of_logic_tree_samples',
              ...
              'within_event_correlation_model', 'minimum_intensity',
...
def view_branches(token, dstore):
    ...
    if full_lt.extra_lt is not None:
        for k, v in full_lt.extra_lt.shortener.items():
            tbl.append((k, v, k))
...
def view_rlz(token, dstore):
    ...
    for brid in rlz.sm_lt_path:
        br = smlt.branches.get(brid)
        tbl.append((bset.uncertainty_type, br.value if br else ''))
        if br and br.bset:
            bset = br.bset
...
def view_branchsets(token, dstore):
    ...
    if flt.extra_lt is not None:
        rows.append((len(rows),
                     '<amplificationModel(%d)>' % flt.extra_lt.xR))
