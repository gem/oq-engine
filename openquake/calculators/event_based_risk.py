...
class EventBasedRiskCalculator(base.RiskCalculator):
    ...
    def pre_execute(self):
        ...
        for rlz in self.rlzs_assoc.realizations:
            self.asset_loss_table[rlz.ordinal] = self.datastore.create_dset(
                'asset_loss_table/%04d' % rlz.ordinal, ela_dt)
            self.agg_loss_table[rlz.ordinal] = self.datastore.create_dset(
                'agg_loss_table/%04d' % rlz.ordinal, self.elt_dt)
    ...
    def post_execute(self, result):
        ...
        for i, rlz in enumerate(self.realizations):
            elt = self.datastore['asset_loss_table/%04d' % i]
            alt = self.datastore['agg_loss_table/%04d' % i]
