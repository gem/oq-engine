...
from openquake.qa_tests_data.classical import (
    case_01, case_02, case_03, case_04, case_12, case_18,
    case_22, case_23, case_24, case_25, case_26, case_27,
    case_29, case_32, case_33, case_34, case_35, case_37,
    case_38, case_40, case_41, case_42, case_43, case_44,
    case_47, case_48, case_49, case_50, case_51, case_53,
    case_54, case_55, case_57, case_60, case_61, case_62,
    case_63, case_64, case_65, case_66, case_67, case_69,
    case_70, case_72, case_74, case_75, case_76, case_77,
    case_78, case_80, case_81, case_82, case_83, case_84,
    case_85, case_86, case_87,
)
...
class ClassicalTestCase(CalculatorTestCase):
    ...
    def test_case_01(self):
        self.assert_curves_ok(
            ["hazard_curve-PGA.csv", "hazard_curve-SA(0.1).csv"], case_01.__file__
        )
    ...
    def test_sa_period_too_big(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            self.run_calc(
                case_01.__file__, "job.ini", intensity_measure_types_and_levels=imtls
            )
        self.assertEqual(
    ...
    def test_case_12(self):
        # test Modified GMPE
        self.assert_curves_ok(
            ["hazard_curve-smltp_b1-gsimltp_b1_b2.csv"], case_12.__file__
        )

    def test_case_18(self):  # GMPEtable, PointMSR, 3 hypodepths
        self.run_calc(case_18.__file__, "job.ini", calculation_mode="preclassical")
    ...
    def test_case_22(self):
        ...
        with mock.patch.dict(os.environ, {"OQ_DISTRIBUTE": "no"}), mock.patch.dict(
            config.memory, {"pmap_max_gb": 1e-5}
    ...
    def test_case_22_bis(self):
        ...
        # NB: requires disabling the parallelization otherwise the
        # workers would read the real custom_tmp and not the mocked one
        with mock.patch.dict(config.memory, {"pmap_max_gb": 1e-5}), mock.patch.dict(
            config.directory, {"custom_tmp": ""}
    ...
    def test_case_23(self):  # filtering away on TRT
        self.assert_curves_ok(["hazard_curve.csv"], case_23.__file__, delta=1e-5)
    ...
    def test_case_24(self):  # UHS
        ...
        self.assert_curves_ok(
            [
                "hazard_curve-PGA.csv",
                "hazard_curve-PGV.csv",
                "hazard_curve-SA(0.025).csv",
                "hazard_curve-SA(0.05).csv",
                "hazard_curve-SA(0.1).csv",
                "hazard_curve-SA(0.2).csv",
                "hazard_curve-SA(0.5).csv",
                "hazard_curve-SA(1.0).csv",
                "hazard_curve-SA(2.0).csv",
                "hazard_uhs.csv",
            ],
            case_24.__file__,
            delta=1e-3,
    ...
    def test_case_25(self):  # negative depths
        self.assert_curves_ok(
            ["hazard_curve-smltp_b1-gsimltp_b1.csv"], case_25.__file__
        )

    def test_case_26(self):  # split YoungsCoppersmith1985MFD
        self.assert_curves_ok(
            ["hazard_curve-rlz-000.csv"], case_26.__file__, delta=2e-5
        )

    def test_case_27(self):  # Nankai mutex model
        self.assert_curves_ok(["hazard_curve.csv"], case_27.__file__, delta=1e-5)
    ...
    def test_case_34(self):
        # spectral averaging
        self.assert_curves_ok(["hazard_curve-mean-AvgSA.csv"], case_34.__file__)
    ...
    def test_case_35(self):
        # cluster
        self.assert_curves_ok(["hazard_curve-rlz-000-PGA.csv"], case_35.__file__)
    ...
    def test_case_38(self):
        ...
        self.assert_curves_ok(
            ["hazard_curve-mean-PGA.csv", "hazard_uhs-mean.csv"], case_38.__file__
        )
    ...
    def test_case_40(self):
        # NGA East
        self.assert_curves_ok(
            ["hazard_curve-mean-PGV.csv", "hazard_map-mean.csv"], case_40.__file__
        )
    ...
    def test_case_41(self):
        ...
        self.assert_curves_ok(
            ["hazard_curve-mean-PGA.csv", "hazard_curve-mean-SA(1.0).csv"],
            case_41.__file__,
        )
    ...
    def test_case_42(self):
        ...
        self.assert_curves_ok(
            ["hazard_curve-mean-PGA.csv", "hazard_map-mean-PGA.csv"], case_42.__file__
        )

        # check pandas readability of hmaps-stats
        df = self.calc.datastore.read_df(
            "hmaps-stats", "site_id", dict(imt="PGA", stat="mean")
        )
    ...
    def test_case_43(self):
        ...
        hc_id = str(self.calc.datastore.calc_id)
        self.run_calc(case_43.__file__, "job.ini", hazard_calculation_id=hc_id)
    ...
    def test_case_44(self):
        ...
        # jupyter notebook
        self.assert_curves_ok(
            ["hc-shift-hypo-PGA.csv"], case_44.__file__, shift_hypo="true"
        )
        self.assert_curves_ok(
            ["hazard_curve-mean-PGA.csv"], case_44.__file__, shift_hypo="false"
        )

    def test_case_47(self):
        # Mixture Model for Sigma using PEER (2018) Test Case 2.5b
        self.assert_curves_ok(
            ["hazard_curve-rlz-000-PGA.csv", "hazard_curve-rlz-001-PGA.csv"],
            case_47.__file__,
            delta=1e-5,
        )
    ...
    def test_case_48(self):
        ...
        aac(dst[0], expect[:, 0], atol=0.05)  # site 0
        aac(dst[1], expect[:, 1], atol=0.05)  # site 1
        ...
        dst = get_dists(self.calc.datastore)
        aac(dst[0], approx[:, 0], atol=0.05)  # site 0
        aac(dst[1], approx[:, 1], atol=0.05)  # site 1
    ...
    def test_case_50(self):
        # serious test of amplification + uhs
        self.assert_curves_ok(
            ["hcurves-PGA.csv", "hcurves-SA(1.0).csv", "uhs.csv"],
            case_50.__file__,
            delta=1e-5,
        )
    ...
    def test_case_54(self):
        ...
        with self.assertRaises(InvalidFile) as ctx:
            self.run_calc(case_54.__file__, "job.ini", individual_rlzs="false")
    ...
    def test_case_57(self):
        ...
        self.assertIn(
            "Missing site_model_file specifying the parameter region",
            str(ctx.exception),
        )
    ...
    def test_case_65(self):
        ...
        self.run_calc(case_65.__file__, "job.ini", calculation_mode="preclassical")
        hc_id = str(self.calc.datastore.calc_id)
    ...
    def test_case_66(self):
        ...
        self.run_calc(case_66.__file__, "job1.ini", calculation_mode="preclassical")
        self.assertEqual(self.calc.sitecol.vs30, [810.0])
    ...
    def test_case_76(self):
        ...
        if not config.directory.custom_tmp:
            ...
            for g, gsim in enumerate(gsims):
                ...
                delta = 0.0003 if sys.platform == "darwin" else 1e-5
