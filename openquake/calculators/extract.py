...
def extract_sources(dstore, what):
    ...
    oknames = [name for name in info.dtype.names  # avoid pickle issues
               if name not in ('source_id', 'rt_ids')]
...
def extract_mfd(dstore, what):
    ...
    rups = dstore['ruptures']['rt_id', 'mag', 'n_occ']
    mags = sorted(numpy.unique(rups['mag']))
    magidx = {mag: idx for idx, mag in enumerate(mags)}
    num_groups = rups['rt_id'].max() + 1
    frequencies = numpy.zeros((len(mags), num_groups), float)
    for rt_id, mag, n_occ in rups:
        if kind_mean:
            dd[mag] += n_occ * weights[rt_id % n] / duration
        if kind_by_group:
            frequencies[magidx[mag], rt_id] += n_occ / duration
    dic['magnitudes'] = numpy.array(mags)
    if kind_mean:
        dic['mean_frequency'] = numpy.array([dd[mag] for mag in mags])
    if kind_by_group:
        for rt_id, freqs in enumerate(frequencies.T):
            dic['grp-%02d_frequency' % rt_id] = freqs
