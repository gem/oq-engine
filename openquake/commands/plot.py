...
def plot(imt, calc_id=-1, other_id=None, site=0, webapi=False):
    ...
    if x2 is None:
        stats.pop('mean')
        mean_curves = x1.get('hcurves?kind=mean&imt=%s&site_id=%d' %
                             (imt, site))
        others = {
        ...
        mean1 = x1.get('hcurves?kind=mean&imt=%s?site_id=%d' % (imt, site))
        mean2 = x2.get('hcurves?kind=mean&imt=%s?site_id=%d' % (imt, site))
