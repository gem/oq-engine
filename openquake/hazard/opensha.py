...
class ClassicalMixin(BasePSHAMixin):
    ...
    def serialize_hazard_curve_of_realization(self, sites, realization):
        ...
        key_template = kvs.tokens.hazard_curve_poes_key(self.id, realization, '%s')
    ...
    @property
    def quantiles(self):
        return [float(x) for x in self['QUANTILE_LEVELS'].split()]

    @property
    def desired_poes(self):
        return [float(x) for x in self['POES_HAZARD_MAPS'].split()]
