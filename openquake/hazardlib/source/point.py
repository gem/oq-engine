...
from openquake.baselib.performance import Monitor, compile, numba
...
@numba.njit()
def _array_rec(inp, clon, clat, cdep):
    ...
    for rec in inp:
        ...
        for i, az in enumerate(azimuths):
            array[:2, i] = geodetic.point_at(clon, clat, az, hor_dist)
        array[2, 0:2] = cdep - half_height
        array[2, 2:4] = cdep + half_height
        yield array, strike, dip, clon, clat, cdep
...
def _get_surfaces(inp, hypo, shift_hypo=False):
    ...
    """
    out = []
    for array, strike, dip, lon, lat, dep in _array_rec(
            inp, hypo.x, hypo.y, hypo.z):
        surface = PlanarSurface.from_array(array, strike, dip)  # shape (3, 4)
        surface.hc = Point(lon, lat, dep) if shift_hypo else hypo
        out.append(surface)
    return out
