...
# -*- coding: utf-8 -*-
"""
CanadaSHM6 GMMs for Stable Crust
"""
import numpy as np
import os
...
class CanSHM6_StableCrust_AA13(GMPETable):
    """
    ...
    References:
        ...
        [CanadaSHM6 Open-file Reference]
    ...
    def __init__(self, submodel):
        ...
        PATH = os.path.join(os.path.dirname(__file__), 'tables',
                            'AA13', 'ENA_%s_cl450.hdf5' % submodel)

        super().__init__(gmpe_table=PATH)

    def defined_imts(self):
    ...
    def get_mean_and_stddevs(self, sctx, rctx, dctx, imt, stddev_types):
        """
        Returns the mean and standard deviations (modified from GMPETable to
                                                  include a site-term)
        """
    ...
    def site_term(self, sctx, rctx, dctx, dists, imt, stddev_types):
        ...
        PGA450 = self._get_mean(imls_pga, dctx, dists)  # in units of g
        ...
        PGA760_ref[sctx.vs30 > 450.] = 0.1  # fixed PGA of 0.1g used for > 450
...
class CanSHM6_StableCrust_NGAEast(GMPETable):
    """
    ...
    def site_term(self, sctx, rctx, dctx, dists, imt, stddev_types):
        ...
        B = self.BA08_BC17(imt, PGArock, dctx, rctx, sctx)

        # site term from Model H.
        if imt != PGA() and imt != PGV():
            lin = self.Hea19_L1_linear(imt, sctx.vs30)
            nl = self.Fnl_USGS(imt, sctx.vs30, PGArock)
            H = lin + nl
        else:
            H = [-999.]*len(B)
    ...
    def Hea19_L1_linear(self, imt, vs30):
        """
    ...
    def Fnl_USGS(self, imt, vs30, PGArock):
        """
    ...
    def BA08_BC17(self, imt, PGArock, dctx, rctx, sctx):
        ...
        self.Repi_from_Rrup(dctx, rctx.mag, 10.0)
        ...
        C = self.BA08.COEFFS_SOIL_RESPONSE[imt]
        nl = self.BA08._get_site_amplification_non_linear(vs30[lte760],
                                                          PGA760, C)
        lin = self.BA08._get_site_amplification_linear(vs30[lte760], C)
        F[lte760] += nl + lin
    ...
    def interp(self, x, x0, x1, y0, y1):
        """
        Helper function for fast simple 1D interpolation
        """

        return y0 + (x-x0)*((y1-y0)/(x1-x0))

    def Repi_from_Rrup(self, dctx, mag, h):
        """
    ...
    # arrays of factors from BC17 for PGA for
    # mag = [3., 4., 5., 6., 7., 8.] and rrup = [2., 2.49, 3.11, 3.88, 4.83,
    # 6.03, 7.51, 9.37, 11.68, 14.56, 18.16, 22.64, 28.22, 35.19, 43.87, 54.7,
    # 68.2, 85.04, 106.02, 132.19, 164.81, 205.49, 256.21, 319.44, 398.28,
    # 496.58, 619.14, 771.94, 962.46, 1200.]
