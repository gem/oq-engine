...
def get_distance_term(self, C, ctx, imt):
    """
    ...
    if self.kind == 'regional':
        c3 = get_distance_coefficients_3(
            self.att, self.c3, self.delta_c3_epsilon, C, imt, ctx)
    else: 
         c3 = get_distance_coefficients(
             self.kind, self.c3, self.c3_epsilon, C, imt, ctx)
    f_r = (C["c1"] + C["c2"] * (ctx.mag - CONSTANTS["Mref"])) * np.log(
        rval / rref_val) + (c3 * (rval - rref_val) / 100.)
    return f_r
...
class KothaEtAl2020(GMPE):
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            fp = get_distance_term(self, C, ctx, imt)
