...
class LanzanoEtAl2016_RJB(GMPE):
    ...
    def _compute_distance(self, rup, dists, C, sites):
        ...
        #PEA = np.zeros_like(sites.vs30)
        #PEA[sites.lat - LATref >= 0] = 1
        R = np.sqrt(dists.rjb**2 + C['h']**2)

        dist_term = (diff >=0) * (C['c11'] + C['c21'] * (rup.mag - Mr)) * (R<=Rh) * np.log10(R/Rh) +\
                    (diff >=0) * (C['c12'] + C['c22'] * (rup.mag - Mr)) * (R> Rh) * np.log10(R/Rh) +\
                    (diff < 0) * (C['c13'] + C['c23'] * (rup.mag - Mr)) * (R<=Rh) * np.log10(R/Rh) +\
                    (diff < 0) * (C['c14'] + C['c24'] * (rup.mag - Mr)) * (R> Rh) * np.log10(R/Rh)
