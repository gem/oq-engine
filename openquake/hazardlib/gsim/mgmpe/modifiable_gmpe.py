...
def horiz_comp_to_geom_mean(self, ctx, imt):
        ...
        if imt==PGA():
            ...
            if horcom == 'RotD50' or horcom == 'GREATER_OF_TWO_HORIZONTAL':
                ...
                if T<=0.15:
                    ...
                    conv_median=C[0]+(C[1]-C[0])*np.log10(T/0.15)/np.log10(0.8/0.15);
                    conv_sigma=C[2]+(C[3]-C[2])*np.log10(T/0.15)/np.log10(0.8/0.15);
                Rstd=C[4]
    ...
    std = (total_stddev**2-conv_sigma**2)/Rstd**2
