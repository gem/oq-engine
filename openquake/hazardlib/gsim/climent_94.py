...
import numpy as np# standard acceleration of gravity in m/s**2
...
class ClimentEtAl1994(GMPE):
    ...
    NORSAR report: "Spectral strong motion attenuation in Central 
    America", NORSAR Technical Report No. 2-17, 46 pp.
    The original formulation predict peak ground acceleration (m/s*s and
    5% damped pseudo-relative velocity (PSV) in m/s for the largest 
    component of horizontal ground motion (see Summary pag.1).
    Spectral acceleration (SA) values are obtained from velocity response
    spectrum  (PSV) values using the following formula ::
        SA = PSV * (2 * pi/ T)    
    """
    ...
    def _compute_term_3_4(self, dists,C):
        ...
        The predicted values are truncated by distance below 6 km
        (see third paragraph in pag. 22)
    ...
    def _get_stddevs(self, C, stddev_types, num_sites):
        ...
        stddevs = [np.zeros(num_sites) + C['SigmaB'] for _ in stddev_types]
        return stddevs
    ...
    COEFFS = CoeffsTable(sa_damping=5, table="""\
     IMT      c1       c2        c3        c4        c5       SigmaB
     pga     -1.6870   0.5530   -0.5370   -0.00302   0.3270   0.750
     0.025   -7.2140   0.5530   -0.5370   -0.00302   0.3270   0.750
     0.050   -5.4870   0.4470   -0.5500   -0.00246   0.3090   0.780
     0.100   -4.7260   0.4830   -0.5810   -0.00199   0.3810   0.800
     0.200   -4.8760   0.6420   -0.6420   -0.00156   0.4700   0.820
     0.500   -5.8620   0.9170   -0.7260   -0.00107   0.5660   0.820
     1.000   -6.7440   1.0810   -0.7560   -0.00077   0.5880   0.820
     2.000   -7.3480   1.1280   -0.7280   -0.00053   0.5360   0.790
     4.000   -7.4410   1.0070   -0.6010   -0.00040   0.4960   0.730
    """)
