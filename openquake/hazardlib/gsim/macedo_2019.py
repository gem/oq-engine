...
def get_mean_conditional_arias_intensity(
    ...
    return (
        C["c1"]
        + C["c2"] * np.log(ctx.vs30)
        + C["c3"] * ctx.mag
        + C["c4"] * np.log(mean_gms["PGA"])
        + C["c5"] * np.log(mean_gms["SA(1.0)"])
    )


def get_stddev_component(
    C: Dict, sig_ia_cond: float, sig_pga: float, sig_sa1: float, rho: float
) -> float:
...
class MacedoEtAl2019SInter(ConditionalGMPE):
    ...
    def compute(
        ...
        for m, imt in enumerate(imts):
            mean[m] = get_mean_conditional_arias_intensity(C, ctx, imt, mean_gms)
