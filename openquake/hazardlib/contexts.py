...
class ContextMaker(object):
    ...
    def make_ctxs(self, ruptures, sites, src_id):
        """
        :param ruptures:
...
class PmapMaker(object):
    ...
    def gen_ctxs(self, rups, sites, srcid):
        """
        :yields: context objects (to save memory)
        """
        with self.ctx_mon:
    ...
    def _make_src_indep(self):
        ...
        for srcs, sites in srcs_sites:
            ...
            self._update_pmap(self.gen_ctxs(rups, sites, src_id))
    ...
    def _make_src_mutex(self):
        for src, indices in self.srcfilter.filter(self.group):
            ...
            self._update_pmap(self.gen_ctxs(rups, sites, src.id), pmap)
            p = pmap
            if self.rup_indep:
                p = ~p
            p *= src.mutex_weight
