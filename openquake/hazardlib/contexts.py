...
class PmapMaker():
    ...
    def _get3(self, rup, mdist):
        # return sctx, dctx, pnes of shape (N, M, G)
        with self.ctx_mon:
        ...
        with self.poe_mon:
            return r_sites, dctx, self._make_pnes(rup, mean_std)
    ...
    def make(self):
        ...
        """
        fewsites = len(self.s_sites.complete) <= self.max_sites_disagg
        ...
        for rups, sites, mdist in self._gen_rups_sites():
            ...
            for rup in rups:
                try:
                    sctx, dctx, pnes = self._get3(rup, mdist)
                ...
                if fewsites:  # store rupdata
                    rupdata.add(rup, self.src.id, sctx, dctx)
        poemap.nrups = nrups
