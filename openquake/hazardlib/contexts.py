...
class ContextMaker(object):
    ...
    def __init__(self, trt, gsims, param=None, monitor=Monitor()):
        ...
        if 'rup_occ_file' in param:
            logging.warning("rup occurrence rate file detected in cmaker init")
            self.rup_occ_file = param['rup_occ_file']
        else:
            logging.warning("NO rup occurrence rate file detected in cmaker init")
    ...
    def gen_ctxs(self, ruptures, sites, src_id, rup_occurrence_adjustments=None, 
                 mon=Monitor()):
        """
        :param ruptures:
        ...
        for i, rup in enumerate(ruptures):
            # adjust for occurrence as aftershocks (only parametric rups)
            if (hasattr(rup, 'occurrence_rate') and
                rup_occurrence_adjustments is not None):
                try:
                    rup.occurrence_rate += rup_occurrence_adjustments[i]
                except:
                    pass
...
class PmapMaker(object):
    ...
    def _gen_ctxs(self, rups, sites, srcid, rup_occ_rate_adjustments=None):
        # yield context objects to save memory
        ctxs = self.cmaker.gen_ctxs(rups, sites, srcid, 
                                    rup_occ_rate_adjustments, self.ctx_mon)
    ...
    def _get_source_rup_rate_adjustments(self, rup_occ_rate_adjs, src):
        source_id_split = src.source_id.split(':')[0]
        src_rup_occ_rate_adj = rup_occ_rate_adjs.loc[
                rup_occ_rate_adjs.source == source_id_split,
                'rate'
            ].values
        
        if len(src_rup_occ_rate_adj) == 0:
            logging.warn(f"source {src.source_id} zero len")
        
        if numpy.mean(src_rup_occ_rate_adj) == 0:
            logging.warn(f"source {src.source_id} zero mean")

        return src_rup_occ_rate_adj

    def _make_src_indep(self):
        # sources with the same ID
        
        if hasattr(self.cmaker, 'rup_occ_file'):
            logging.warn("reading rup rate file")
            import pandas as pd
            rup_occ_rate_adjs = pd.read_csv(self.cmaker.rup_occ_file)
        else:
            logging.warn("not reading rup rate file")
            rup_occ_rate_adjs = None

        for src, sites in self.srcfilter.split(self.group):
            if rup_occ_rate_adjs is not None:
                rup_occ_rate_adjustments = self._get_source_rup_rate_adjustments(
                    rup_occ_rate_adjs, src)
            else:
                rup_occ_rate_adjustments = None
            if self.fewsites:
