...
import collections
import numpy
from openquake.baselib import __version__, InvalidFile, hdf5, general
from openquake.hazardlib import (
    calc, geo, gsim, mfd, scalerel, source, const, correlation, imt, pmf, site,
    tom, nrml, near_fault, sourceconverter, gsim_lt, contexts)
...
Input = collections.namedtuple('Input', 'model sitecol gsim_lt cmakerdict')
...
def _get_sitecol(hparams, req_site_params):
    ...
    return site.SiteCollection.from_points(
        mesh.lons, mesh.lats, mesh.depths, sm or hparams, req_site_params)
...
def _rupture_groups(ebruptures):
    ...
    for trt, ebrs in itertools.groupby(ebruptures, bytrt):
        rup_groups.append(sourceconverter.SourceGroup(trt, ebrs))
    return rup_groups
...
def read_input(hparams):
    ...
    - if source_model_file "investigation_time" is mandatory
    - if rupture_model_file "number_of_ground_motion_fields" and "ses_seed"
      are mandatory
    ...
    if rmfname and rmfname.endswith('.xml'):
        for grp in groups:
            for ebr in grp:
                ebr.n_occ = ngmfs * len(rlzs_by_gsim)
