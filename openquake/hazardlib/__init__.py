...
def get_smlt(hparams, sourceID=""):
    ...
    if "source_model_logic_tree" in hparams["inputs"]:
        args = (
            hparams["inputs"]["source_model_logic_tree"],
...
class Input(object):
    ...
    def __init__(self, hparams, extra, read_all=True):
        ...
        hparams.setdefault("rupture_mesh_spacing", 5.0),
        hparams.setdefault("complex_fault_mesh_spacing", 5.0)
        hparams.setdefault("width_of_mfd_bin", 1.0),
        hparams.setdefault("area_source_discretization", 10.0)
        ...
        self.sitecol = _get_sitecol(hparams, self.full_lt.gsim_lt.req_site_params)
    ...
    def get_groups_cmakers(self):
        ...
        if smfname:
            ...
            src = nrml.get(
                self.srcstring,
                c.investigation_time,
                c.rupture_mesh_spacing,
                c.width_of_mfd_bin,
        ...
        for grp_id, sg in enumerate(groups):
            ...
            for src in sg:
                ...
                src.smweight = 1.0 / num_rlzs
                idx += 1
                if hasattr(src, "trt_smrs"):
                    trt_smrs.append(src.trt_smrs)
                else:  # scenario
                    trt_smrs.append([grp_id])
        self.oq.mags_by_trt = {trt: sorted(mags) for trt, mags in mags_by_trt.items()}
        if rmfname:
