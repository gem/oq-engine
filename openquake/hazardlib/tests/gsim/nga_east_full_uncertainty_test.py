...
from openquake.hazardlib.gsim.nga_east import DarraghEtAl2015NGAEast1CCP
...
QUANTILES = {"low": 0.05, "central": 0.5, "high": 0.95}
...
class NGAEastUncertaintyTestCase(unittest.TestCase):
    ...
    def test_all_nga_east_uncertainty(self):
        """
        The file list (NGA_EAST_SIGMA_FILES) describes every permutation of
        the aleatory uncertainty model, each permutation is tested with
        the loop.
        """
        for filestem in NGA_EAST_SIGMA_FILES:
            # Break filestem into model configuration
            config_params = [key.lower() for key in filestem.split("_")]
