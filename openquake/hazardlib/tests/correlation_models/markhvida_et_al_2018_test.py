...
def test_covariance_is_symmetric_positive_definite():
    ...
    imts = [SA(period) for period in (0.01, 0.1, 1.0, 5.0)]
    model = MarkhvidaEtAl2018()
...
def test_rejects_invalid_distances(distances, message):
    with pytest.raises(ValueError, match=message):
        MarkhvidaEtAl2018().correlation_block(distances, [SA(1.0)])
