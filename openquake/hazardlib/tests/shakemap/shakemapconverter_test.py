...
class ShakemapConverterTestCase(unittest.TestCase):
    def __init__(self, *args, **kw):
        ...
        self.parser = ShakemapParser()

    def test_pga(self):
        imt_dt = numpy.dtype([('PGA', F32)])
        array = self.parser._get_shakemap_array(gettemp(example_pga))
        n = 3  # number of sites
    ...
    def test_sa(self):
        ...
        array = self.parser._get_shakemap_array(gettemp(example_sa))
        n = 4  # number of sites
        self.assertEqual(len(array), n)
        self.assertEqual(array.dtype.names,
                         ('lon', 'lat', 'vs30', 'val', 'std'))
    ...
    def test_ghorka(self):
        ...
        array = self.parser._get_shakemap_array(grid_file, uncertainty_file)
