...
class NearFault(object):

    def get_xyz_from_ll(self, projected, reference):
        """
    ...
    def get_plane_equation(self, p0, p1, p2, reference):
        '''
        ...
        p0_xyz = self.get_xyz_from_ll(p0, reference)
        p1_xyz = self.get_xyz_from_ll(p1, reference)
        p2_xyz = self.get_xyz_from_ll(p2, reference)
    ...
    def projection_pp(self, site, normal, dist_to_plane, reference):
        '''
        ...
        [site_x, site_y, site_z] = self.get_xyz_from_ll(site, reference)
    ...
    def average_s_rad(self, site, hypocenter, reference, pp,
        ...
        site_xyz = self.get_xyz_from_ll(site, reference)
        ...
        hyp_xyz = self.get_xyz_from_ll(hypocenter, reference)
        ...
        v = pp - hyp_xyz
        phi = self.vectors2angle(u, v) - np.deg2rad(delta_slip)
    ...
    def isochone_ratio(self, e, rd, r_hyp):
        """
    ...
    def directp(self, node0, node1, node2, node3, hypocenter, reference, pp):
        """
        ...
        node0_xyz = self.get_xyz_from_ll(node0, reference)
        node1_xyz = self.get_xyz_from_ll(node1, reference)
        node2_xyz = self.get_xyz_from_ll(node2, reference)
        node3_xyz = self.get_xyz_from_ll(node3, reference)
        hypocenter_xyz = self.get_xyz_from_ll(hypocenter, reference)
