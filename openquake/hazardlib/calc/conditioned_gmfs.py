...
class ConditionedGmfComputer(GmfComputer):
    ...
    def _compute(self, ms, m, imt, gsim, intra_eps, idxs, rng):
        E = len(idxs)
        mu_Y, cov_WY_WY, cov_BY_BY = ms  # mea, tau, phi with shapes (N,1),(N,N),(N,N)
        ...
        if self.cmaker.truncation_level <= 1E-9:
            ...
            gmf = exp(arr, imt.string != "MMI").T
        return gmf  # shape (N, E)
