...
def poe_map(src, s_sites, imtls, cmaker, trunclevel, ctx_mon, pne_mons,
    ...
    try:
        for rup, weight in rupture_weight_pairs(src):
            ...
            if bbs:  # update bounding boxes) for disaggregation
                _update_bbs(bbs, sctx.sites, dctx.rjb, rup)
...
def _update_bbs(bbs, sites, jb_dists, rup):
    # update bounding boxes for the disaggregation, this is much faster than
    # the other operations, so it is not monitored
    if not bbs:
        return
    sids = set(sites.sids)
    closest_points = rup.surface.get_closest_points(sites.mesh)
