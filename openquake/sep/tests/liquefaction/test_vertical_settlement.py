...
class TestHazusVerticalSettlement(unittest.TestCase):
    ...
    def test_hazus_vertical_settlement_list_m(self):
        v_settle = hazus_vertical_settlement(
            self.all_liq_types, return_unit="m"
        )
        np.testing.assert_array_almost_equal(
            v_settle, np.array([0.3048, 0.1524, 0.0508, 0.0254, 0.0, 0.0])
        )

    def test_hazus_vertical_settlement_list_in(self):
        v_settle = hazus_vertical_settlement(
            self.all_liq_types, return_unit="in"
        )
        np.testing.assert_array_almost_equal(
            v_settle, np.array([12.0, 6.0, 2.0, 1.0, 0.0, 0.0])
        )
