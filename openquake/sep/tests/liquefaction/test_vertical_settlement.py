...
class TestHazusVerticalSettlement(unittest.TestCase):
    ...
    #     v_settle = hazus_vertical_settlement('vh', mag=self.mag, pga=self.pga[0], return_unit='m')
    #     np.testing.assert_almost_equal(v_settle, 0.0722051)

    # def test_hazus_vertical_settlement_single_in(self):
    #     v_settle = hazus_vertical_settlement('vh', mag=self.mag, pga=self.pga[0], return_unit='in')
