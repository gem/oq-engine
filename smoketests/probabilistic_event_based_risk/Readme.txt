In this demonstration loss curves are computed based on probabilistic seismic hazard, using a stochastic event set-based approach. 
...
About 50 realizations of the seismicity history, each with a 50 years time span, are used in this demo and the spatial correlation of the ground motion is taken into account. 

In addition to the loss curves for each asset that are produced (making a total of 423 loss curves), also a loss curve representing the aggregated losses from all assets is computed.  
