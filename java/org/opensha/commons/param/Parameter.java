...
ParameterAPI<E>, java.io.Serializable
{
	...
	public void setDefaultValue( E defaultValue ) throws ConstraintException {
		...
		if ( !isAllowed( defaultValue ) ) {
			throw new ConstraintException( getName() + ": setDefaultValue(): Value is not allowed: " + defaultValue.toString() );
		}
	...
	public void unableToSetValue( E value ) throws ConstraintException {
		...
		org.opensha.commons.param.event.ParameterChangeFailEvent event =
			new org.opensha.commons.param.event.ParameterChangeFailEvent(this,
	...
	public void firePropertyChangeFailed( org.opensha.commons.param.event.ParameterChangeFailEvent event ) {
		...
		for ( int i = 0; i < vector.size(); i++ ) {
			org.opensha.commons.param.event.ParameterChangeFailListener listener = ( org.opensha.commons.param.event.ParameterChangeFailListener ) vector.get( i );
	...
	public void firePropertyChange( ParameterChangeEvent event ) {
		...
		for ( int i = 0; i < vector.size(); i++ ) {
			org.opensha.commons.param.event.ParameterChangeListener listener = ( org.opensha.commons.param.event.ParameterChangeListener ) vector.get( i );
