 ...
 * only one discrete value is allowed
 * <p>
 * 
 * <b>Description:</b> A Integer Constraint represents a range of allowed values
 * between a min and max integer value, inclusive. The main purpose of this
 * class is to call isAllowed() which will return true if the value is withing
 * the range. Null values may or may not be allowed. See the ParameterConstraint
 * javadocs for further documentation.
 * <p>
 * 
 * Note: It is up to the programmer using this class to ensure that the min
 * value is less than the max value. As an enhancement to this class setting min
 * and max could be validated that min is not greater than max.
 * <p>
 * 
...
public class IntegerConstraint extends ParameterConstraint<Integer> {
    ...
    /**
     * Constructor that sets the constraints during instantiation. Sets the min
     * and max values allowed in this constraint. No checks are performed that
     * min and max are consistant with each other.
     * <P>
     * 
     * @param min
    ...
    /**
     * Checks if the passed in value is within the min and max, inclusive of the
     * end points. First the value is chekced if it's null and null values are
     * allowed. Then it checks the passed in object is a Integer. If the
    ...
    /**
     * Checks if the passed in value is within the min and max, inclusive of the
     * end points. First the value is checked if it's null and null values are
     * allowed. If the constraint min and max values are null, true is returned,
     * else the value is compared against the min and max values. If any of
     * these checks fails false is returned. Otherwise true is returned.
     * 
     * @param obj
     *            The object to check if allowed.
    ...
    /**
     * Returns the classname of the constraint, and the min & max as a debug
     * string
     */
