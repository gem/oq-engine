 ...
 * ConstrainedIntegetParameters which recall have a minimum and maximum allowed
 * values. The widget is an IntegerTextField so that only integers can be typed
 * in. When hitting <enter> or moving the mouse away from the IntegerTextField,
 * the value will change back to the original if the new number is outside the
 * constraints range. The constraints also appear as a tool tip when you hold
 * the mouse cursor over the IntegerTextField.
 * <p>
 * 
...
public class ConstrainedIntegerParameterEditor extends IntegerParameterEditor {
    ...
    /**
     * Constructor that sets the parameter that it edits. Only calls the super()
     * function.
     */
    ...
    /**
     * Calls the super().;setFunction() and uses the constraints to set the
     * JTextField tooltip to show the constraint values.
     */
    ...
    /**
     * This is where the IntegerTextField for the Constraint IntegerParameter It
     * checks if the min and max constraint value are same then change the font
     * and size of the valueEditor and widgetPanel is defined and configured.
     */
    protected void addWidget() {
        ...
        if (constraint.getMax().doubleValue() == constraint.getMin()
                .doubleValue()) {
            if (valueEditor != null) {
    ...
    protected IntegerConstraint getConstraint() {
        ...
        if (ParamUtils.isWarningParameterAPI(model)) {
            constraint =
                    (IntegerConstraint) ((WarningParameterAPI) model)
    ...
    private void setToolTipText() {
        IntegerConstraint constraint = getConstraint();
        valueEditor.setToolTipText("Min = " + constraint.getMin().toString()
                + "; Max = " + constraint.getMax().toString());
    }
