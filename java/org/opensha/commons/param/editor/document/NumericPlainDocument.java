...
/**
 * <b>Title:</b> NumericPlainDocument
 * <p>
 * 
 * <b>Description:</b> Model ( or data) associated with an Numeric Text Field.
 * The insertString() function is called whenever data is being entered into the
 * text field. This is where the text field is checked to make sure only numeric
 * valid charachters are being added.
 * <p>
 * 
 * This is an extention of the Model View Controller (MVC) design pattern that
 * all Java Swing elements are built upon. For example, the Java class
 * JTextField contains a PlainDocument model that actually contains the text of
 * the JTextField. This class simply replaces the JTextField PlainDocument with
 * this document. Then as a user types in text into the textfield, this class
 * instance is consulted to see if they are valid characters the user is typing.
 * <p>
 * 
 * You don't have to know the details on how this class works in order to use
 * it. To make text field that uses this document model simply extend JTextField
 * and overide the method createDefaultModel() by creating an instance of you
 * subclass of Plain Document.
 * 
 * <code>
 ...
 * <p>
 * 
 * Note: SWR: This class was implemented with java JDK 1.3. In the new java JDK
 * 1.4 there is a much simpler way to do this. Now you can create a Formatter
 * (Decimal, Date, etc. ) and simply pass the formatter to a Standard
 * JTextField. No subclasses to make.
 * <p>
 * 
...
public class NumericPlainDocument extends PlainDocument {
    ...
    /**
     * Format of this Decimal. Determines what are valid non-digit characters
     * such as decimal point.
     */
    ...
    public interface InsertErrorListener {
        ...
        public void insertFailed(NumericPlainDocument numericplaindocument,
                int i, String string, AttributeSet attributeset);
    ...
    public NumericPlainDocument(AbstractDocument.Content content,
        ...
        try {
            ...
            throw new IllegalArgumentException(S
                    + "Initial context not a valid number");
        }
    ...
    /**
     * Returns the Deciaml format. Decimal strings can have different format,
     * which in effect determines what are the valid characters for thousands
    ...
    /**
     * Helper function that converts the String representation model data into a
     * Number, the superclass of a Double.
     */
    public Number getNumberValue() throws ParseException {
        ...
        try {
            ...
            if (parsePos.getIndex() != this.getLength())
                throw new ParseException(S + "Not a valid number: " + context,
                        0);
    ...
    /**
     * Method called to add data to the text field. Typically used when users
     * type in text, but may be called by back end process if needed. Throws
     * errors if the string is not valid decimal characters.
     */
    public void insertString(int offset, String str, AttributeSet a)
        ...
        if (str != null && str.length() != 0) {
            ...
            String targetString =
                    content.getString(0, offset) + str
            ...
            do {
                if (gotPositive == true || gotNegative == true) {
                    ...
                    if (targetString.endsWith(suffix) == false) {
                        ...
                        for (i = suffixLength - 1; i > 0; i--) {
                            if (targetString.regionMatches(length - i, suffix,
                                    0, i)) {
                                targetString =
                ...
                if (endIndex != length
                        ...
                        && (positivePrefixLen <= 0
                                ...
                                || length > positivePrefixLen || !targetString
                                .regionMatches(0, positivePrefix, 0, length))

                        && (negativePrefixLen <= 0
                                || endIndex >= negativePrefixLen
                                || length > negativePrefixLen || !targetString
                                .regionMatches(0, negativePrefix, 0, length))) {

                    char lastChar = targetString.charAt(originalLength - 1);
                    int decimalIndex = targetString.indexOf(decimalSeparator);

                    /*
                     * if ((!format.isGroupingUsed() || lastChar !=
                     * groupingSeparator || decimalIndex != -1) &&
                     * (format.isParseIntegerOnly() != false || lastChar !=
                     * decimalSeparator || decimalIndex != originalLength - 1))
                     * { if (errorListener != null)
                     * errorListener.insertFailed(this, offset, str, a); return;
                     * }
                     */
                }
            }
            ...
            String context = this.getText(0, this.getLength());
            if (D)
                System.out.println(S + "Current context = " + context);
            if (D)
                System.out.println(S + "Inserting " + str + " at " + offset);

            super.insertString(offset, str, a);
            context = this.getText(0, this.getLength());

            if (D)
                System.out.println(S + "Ending: New context = " + context);

        }
    }

    /**
     * Adds a listener that is notified when insertString() fails because text
    ...
    /**
     * Removes a listener that was notified when insertString() fails because
