...
/**
 * <b>Title:</b> ConstrainedStringParameterEditor
 * <p>
 * 
 * <b>Description:</b> This editor is for editing ConstrainedStringParameters.
 * Recall a ConstrainedStringParameter contains a list of the only allowed
 * values. Therefore this editor presents a picklist of those allowed values,
 * instead of a JTextField or subclass.
 * <p>
 * 
...
public class ConstrainedStringParameterEditor extends ParameterEditor implements
        ItemListener {
     ...
     * StringConstraint. Then the constraints are checked that there is at least
     * one. If any of these fails an error is thrown.
     * <P>
     * 
     ...
     * else a picklist of values to choose from are presented to the user. A
     * tooltip is given to the name label if model info is available.
     */
    ...
    /**
     * The parameter is checked that it is a DoubleDiscreteParameter, and the
     * constraint is checked that it is a DoubleDiscreteConstraint. Then the
     * constraints are checked that there is at least one. If any of these fails
     * an error is thrown.
     */
    protected void verifyModel(ParameterAPI model) throws ConstraintException {
        ...
        if (model == null) {
            throw new NullPointerException(S
                    + "Input Parameter model cannot be null");
        }

        if (!(model instanceof StringParameter))
            throw new ConstraintException(S
                    + "Input model parameter must be a StringParameter.");

        ParameterConstraintAPI constraint = model.getConstraint();

        if (!(constraint instanceof StringConstraint))
            throw new ConstraintException(S
                    + "Input model constraints must be a StringConstraint.");

        int numConstriants = ((StringConstraint) constraint).size();
        if (numConstriants < 1)
            throw new ConstraintException(
                    S
                            + "There are no constraints present, unable to build editor selection list.");
    ...
    protected void setWidgetObject(String name, Object obj) {
        ...
        if (D)
            System.out.println(S + "Starting: Name = " + name + ": Object = "
                    + obj.toString());

        super.setWidgetObject(name, obj);

        if ((obj != null) && (valueEditor != null)
                && (valueEditor instanceof JComboBox))
    ...
    /**
     * This is where the JComboBox picklist is defined and configured. This
     * function adds a little more intellegence in that if there is only one
     * constraint, it only adds a lable instead of a picklist. No need to give a
     * list of choices when there is only one allowed value.
     */
    protected void addWidget() {
        ...
        if (model != null) {
            ...
            StringConstraint con =
                    (StringConstraint) ((StringParameter) model)
                            .getConstraint();
     ...
     * synchronizes the model to the new GUI value. This is where the picklist
     * value is set in the ParameterAPI of this editor.
     */
    public void itemStateChanged(ItemEvent e) {
        String S = C + ": itemStateChanged(): ";
        if (D)
            System.out.println(S + "Starting: " + e.toString());

        String value = ((JComboBox) valueEditor).getSelectedItem().toString();
        if (D)
            System.out.println(S + "New Value = " + (value));
        this.setValue(value);
    ...
    /**
     * Called everytime a key is typed in the text field to validate it as a
     * valid integer character ( digits and - sign in first position ).
     */
    ...
    /**
     * Called when the user clicks on another area of the GUI outside this
     * editor panel. This synchornizes the editor text field value to the
