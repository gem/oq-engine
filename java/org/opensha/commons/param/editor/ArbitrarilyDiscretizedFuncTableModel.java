...
public class ArbitrarilyDiscretizedFuncTableModel extends AbstractTableModel {
    ...
    public void updateData(ArbitrarilyDiscretizedFunc newFunc) {
        if (!areFunctionPointsEqual(func, newFunc)) {
            if (D) {
                ...
                System.out.println("Old First: " + func.getX(0) + ", "
                        + func.getY(0));
                System.out.println("New First: " + newFunc.getX(0) + ", "
                        + newFunc.getY(0));
            }
            this.func.clear();
    ...
    public static boolean areFunctionPointsEqual(
            ArbitrarilyDiscretizedFunc func1, ArbitrarilyDiscretizedFunc func2) {
    ...
    private void doRemovePoints(int[] indexes) {
        ArbitrarilyDiscretizedFunc old =
                (ArbitrarilyDiscretizedFunc) func.deepClone();
        func.clear();
    ...
    // I have to reimplement some of it because JTable.DoubleRenderer isn't
    // visible
    class ArbitrarilyDiscretizedFuncTableCellRenderer extends
