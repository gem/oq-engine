...
/**
 * <b>Title:</b> DoubleParameterEditor
 * <p>
 * 
 * <b>Description:</b> Subclass of ParameterEditor for editing DoubleParameters.
 * The widget is an DoubleTextField so that only numbers can be typed in.
 * <p>
 * 
 * The main functionality overidden from the parent class to achive Double
 * cusomization are the setWidgetObject() and AddWidget() functions. The
 * parent's class JComponent valueEditor field becomes an NumericTextField, a
 * subclass of a JTextField. Note: We have to create a double parameter with
 * constraints if we want to reflect the constarints as the tooltip text in the
 * GUI. Because when we editor is created for that double parameter, it creates
 * a constraint double parameter and then we can change the constraint and it
 * will be reflected in the tool tip text.
 * <p>
 * 
...
public class DoubleParameterEditor extends ParameterEditor {
     ...
     * 
     * Note: When calling the super() constuctor addWidget() is called which
     * configures the NumericTextField as the editor widget.
     * <p>
     */
    ...
    private void verifyModel(ParameterAPI model) throws ConstraintException {
        ...
        if (!(model instanceof DoubleParameter))
            throw new ConstraintException(S
                    + "Input model parameter must be a DoubleParameter.");
     ...
     * valid number character ( digits, - sign in first position, etc. ).
     */
    public void keyTyped(KeyEvent e) throws NumberFormatException {
        ...
        if (focusLostProcessing == true)
            return;

        if (e.getKeyChar() == '\n') {
            keyTypeProcessing = true;
            if (D)
                System.out.println(S + "Return key typed");
            ...
            try {
                ...
                if (D)
                    System.out.println(S + "Error = " + ee.toString());

                Object obj = getValue();
                if (obj != null)
                    ((NumericTextField) valueEditor).setText(obj.toString());
                else
                    ((NumericTextField) valueEditor).setText("");
