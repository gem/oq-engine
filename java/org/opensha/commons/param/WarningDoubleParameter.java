...
/**
 * <b>Title:</b> WarningDoubleParameter
 * <p>
 * 
 * <b>Description:</b> Concrete implementation of the WarningParameterAPI
 * interface that stores a Double for it's value object. Maintains a list of
 * listeners and passes them ParameterChangeWarningEvents when the value is
 * attemted to be set beyong the warning constraints.
 * <p>
 * 
 * Typical use case is that there is one listener that also acts as the editor
 * of the value, ( such as the GUI component attempting to update the value ).
 * This listener attempts to change the value outside the warning range. The
 * listener is notified of the attemp, i.e. "warned". This listener then
 * notifies the user via a DialogBox. The user is then given the option to
 * cancel or ignore the warning and set the value. The listener will then set
 * the value ignoring the warning.
 * <p>
 * 
 * The whole reason for using a listener is that any type of situation can be
 * handled. This class doesn't need to know anything about the listener other
 * than it adheres to the ParameterChangeWarningListener interface. The listener
 * can be any class, and can be updated to any new class in the future. This
 * class never has to be changed. This means that this parameter component is
 * not tied to any specific class of editors, a guiding principle in
 ...
 * <p>
 * 
 * Note: Since this class extends from DoubleParameter it also has a second
 * absolute DoubleConstraint that can never be exceeded. It's important that the
 * programmer realizes this and ensures the warning constraints are smaller than
 * the absolute constraints when using this parameter in their program.
 * <p>
 * 
...
public class WarningDoubleParameter extends DoubleParameter implements
     ...
     * Recall the absolute constraints are still active and may still block the
     * value update.
     */
    private boolean ignoreWarning;

    /**
     * No warning constraints or absolute constraints specified. All values
     * allowed. Also sets the name of this parameter.
     */
    ...
    /**
     * No warning constraints or absolute constraints specified. All values
     * allowed. Also sets the name and units of this parameter.
     */
     ...
     */
    public WarningDoubleParameter(String name, double min, double max)
            throws ConstraintException {
        super(name, min, max);
    }
     ...
     *             Is thrown if the value is not allowed
     */
    public WarningDoubleParameter(String name, DoubleConstraint constraint)
            throws ConstraintException {
     ...
     *                thrown if the value is not allowed
     * @throws ConstraintException
     *             Is thrown if the value is not allowedallowed one. Null values
     *             are always allowed in the constructors
     */
    public WarningDoubleParameter(String name, DoubleConstraint constraint,
            String units) throws ConstraintException {
     ...
     *            Name of the parameter
     * @param value
     *            Double value of this parameter
     */
     ...
     *             Is thrown if the value is not allowed
     */
    public WarningDoubleParameter(String name, String units, Double value)
            throws ConstraintException {
     ...
     */
    public WarningDoubleParameter(String name, double min, double max,
            Double value) throws ConstraintException {
     ...
     * framework. This is where the listener list is created if null, i.e.
     * "lazy instantiation".
     * 
     * @param listener
    ...
    /**
     * Removes a ParameterChangeFailListener to the list of listeners. This
     * listener will no longer receive warning events.
     */
    ...
    public void setWarningConstraint(ParameterConstraint warningConstraint)
        ...
        // (this.warningConstraint.getName() == null) )
    ...
    /**
     * Proxy passthrough method to the DoubleConstraint to get the minimumn
     * value below which warnings will be issued.
     * 
     ...
     * ignoreWarning flag is present the warning constraints are bypassed.
     * Another condition checked is if the value is null and null values are
     * allowed.
     * 
     * @param value
     *            The new value - must be a Double
     * @throws ConstraintException
     *             Thrown if the new value is beyond the constraint levels or
     *             null values not allowed.
     * @throws WarningException
     *             Thrown if the new value is beyond the warning levels.
     */
    public synchronized void setValue(Double value) throws ConstraintException,
            WarningException {
     ...
     *             Thrown if the new value is beyond the warning levels.
     */
    public void setValueIgnoreWarning(Double value) throws ConstraintException,
            ParameterException {
        String S = C + ": setValueIgnoreWarning(): ";
        if (D)
            System.out.println(S
                    + "Setting value ignoring warning and constraint: ");
        // this.value = value;
        super.setValue(value);
    }

    /**
     * Uses the warning constraint object to determine if the new value being
     * set is within recommended range. If no Constraints are present all values
     * are recommended. The implied intention is that the warning constraint is
     * more restrictive than the absolute constraints so that if a value passes
     * the warning test, it will also pass the absolute constraint test.
     * 
     * @param obj
    ...
    public boolean isRecommended(Double obj) {
        if (warningConstraint != null)
            return warningConstraint.isAllowed(obj);
        else
            return true;
    ...
    /**
     * This is the function that notifies all listeners assigned to this
     * parameter that the warning constraints have been exceeded.
     * 
     * @param event
     ...
     * the comparing objects. This implies that the value object of both
     * parameters must be a double.
     * 
     * @param obj
    ...
    public Object clone() {
        ...
        if (value == null)
            param = new WarningDoubleParameter(name, c1, units);
        else
            param =
                    new WarningDoubleParameter(name, c1, units, new Double(
                            this.value.toString()));
        if (param == null)
            return null;
        ...
        // NOTE: The listeners are NOT cloned. They were interested in the
        // original,
