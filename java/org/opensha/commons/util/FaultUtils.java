...
/**
 * <b>Title:</b> FaultUtils
 * <p>
 * 
 * <b>Description:</b> Collection of static utilities used in conjunction with
 * strike, dip and rake angles of faults. These functions are assertion
 * functions, in that they validate the angles as valid strike, dip, and rake
 * angles, and return true or false if valid.
 * <p>
 * 
...
public final class FaultUtils {
     ...
     * <p>
     * <code>0 <= strike <= 360</code>
     * <p>
     * 
     * @param strike
     *            Angle to validate
     * @throws InvalidRangeException
     *             Thrown if not valid angle
     */
    public static void assertValidStrike(double strike)
            throws InvalidRangeException {

        if (strike < 0)
            throw new InvalidRangeException(S1
                    + "Strike angle cannot be less than zero (value = "
                    + strike + ")");
        if (strike > 360)
            throw new InvalidRangeException(S1
                    + "Strike angle cannot be greater than 360 (value = "
                    + strike + ")");
    }
    ...
    public static void assertValidDip(double dip) throws InvalidRangeException {
        ...
        if (dip < 0)
            throw new InvalidRangeException(S2
                    + "Dip angle cannot be less than zero; the value is " + dip);
        if (dip > 90)
            throw new InvalidRangeException(S2
                    + "Dip angle cannot be greater than 90; the value is "
                    + dip);
    }
    ...
    public static void assertValidSeisUpperAndLower(double upperSeis,
            double lowerSeis) throws InvalidRangeException {
