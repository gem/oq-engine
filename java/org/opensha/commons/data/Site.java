 ...
 * <p>
 * 
 * <b>Description:</b> This class hold the information about a geographical
 * site. It has been generalized by extendign a ParameterList so that any site
 * parameters can be contained within this Site object. The Site object is
 * really a specialized collection of parameters associated with a Location
 * object, i.e. latitude and longitude.
 * <p>
 * 
 ...
 * will also be provided so that one can add others if so desired.
 * <p>
 * 
 * An IntensityMeasureRalationship object will request whatever Parameter values
 * it needs from the Site object that is passed into the IMR.
 * <p>
 * 
 * Provide methods for setting all site parameters from, for example, Vs30
 * (authors must approve how this is done)?
 * <p>
 * 
 * Note that the Site class implements NamedObjectAPI. This is usefull for
 * giving a Site a name, such as "Los Angeles".
 * <p>
 * 
...
public class Site extends ParameterList implements NamedObjectAPI,
    ...
    /**
     * Returns true if the comparing site has the same name, Location and each
     * parameter exists and has the same value in each Site.
     */
     ...
     * name, Location and each parameter exists and has the same value in each
     * Site.
     */
     ...
     * <p>
     * 
     * Note: Cloning this object then calling equals() would return true. They
     * are different instances, but have the same values. Modifying one would
     * not affect the second. Equal yet disparet.
     * <p>
     */
    public Object clone() {
    ...
    public static Site fromXMLMetadata(Element siteEl,
            ArrayList<ParameterAPI> paramsToAdd) {
