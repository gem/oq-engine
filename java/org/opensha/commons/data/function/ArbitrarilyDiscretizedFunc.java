...
/**
 * <b>Title:</b> ArbitrarilyDiscretizedFunc
 * <p>
 * 
 * <b>Description:</b> This class is a sublcass implementation of a
 * DiscretizedFunc that stores the data internaly as a sorted TreeMap of
 * DataPoint2D. This subclass distinguishes itself by the fact that it assumes
 * no spacing interval along the x-axis. Consecutive points can be spread out or
 * bunched up in no predicatable order. For at least the default comparator
 * (DataPoint2DComparator), the tolerance determines whether the set() methods
 * add the point (if x value is more than tolerance away from that of all
 * existing points) or whether they replace an existing point (if within
 * tolerance). A tolerance of less than about 1e-16 is effectively about 1e-16
 * due to the numerical precision of floating point arithmetic (1.0 -
 * (1.0+1e-16) = 1.0).
 * <p>
 * 
...
public class ArbitrarilyDiscretizedFunc extends DiscretizedFunc implements
        Serializable {
     ...
     * stored in a DataPoint2D TreeMap so they are sorted on the X-Values.
     * <p>
     * 
     * This TreeMap will not allow identical DataPoint2D. A comparator and
     * equals() is used to determine equality. Since you can specify any
     * comparator you want, this ArbitrarilyDiscretizedFunc can be adopted for
     * most purposes.
     * <p>
     * 
     * Note: SWR: I had to make a special org.opensha. version of the Java
     * TreeMap and subclass DataPoint2DTreeMap to access internal objects in the
     * Java TreeMap. Java's Treemap had internal objects hidden as private, I
     * exposed them to subclasses by making them protected in
     * org.opensha.data.TreeMap. This was neccessary for index access to the
     * points in the TreeMap. Seems like a poor oversight on the part of Java.
     * <p>
     */
     ...
     * <p>
     * 
     * The passed in comparator must be an implementor of
     * DataPoint2DComparatorAPI. These comparators know they are dealing with a
     * DataPoint2D and usually only compare the x-values for sorting. Special
     * comparators may wish to sort on both the x and y values, i.e. the data
     * points are geographical locations.
     */
    public ArbitrarilyDiscretizedFunc(Comparator comparator) {
        if (!(comparator instanceof DataPoint2DComparatorAPI)) {
            throw new DataPoint2DException(
                    "Comparator must implement DataPoint2DComparatorAPI");
        }
     ...
     * default Comparator compares only x-values within tolerance to determine
     * if two points are equal.
     * <p>
     */
    public ArbitrarilyDiscretizedFunc(double toleranace) {
        DataPoint2DToleranceComparator comparator =
     ...
     * <p>
     * 
     * The default tolerance of 0 is used. This means that two x-values must be
     * exactly equal doubles to be considered equal.
     */
     ...
     * tolerance level that specifies how close two points have to be along the
     * x axis to be considered equal. A tolerance of less than about 1e-16 is
     * effectively about 1e-16 due to the numerical precision of floating point
     * arithmetic (1.0 - (1.0+1e-16) = 1.0).
     */
    ...
    public double getMinX() {
        return ((DataPoint2D) points.firstKey()).getX();
    }
    ...
    public double getMaxX() {
        return ((DataPoint2D) points.lastKey()).getX();
    }

    /**
     * Return the minimum y value along the y-axis. This value is calculated
     * every time a DataPoint2D is added to the list and cached as a variable so
     * this function returns very quickly. Slows down adding new points
     * slightly. I assume that most of the time these lists will be created
     * once, then used for plotting and in other functions, in other words more
     * lookups than inserts.
     */
    @Override
    ...
    /**
     * Returns the nth (x,y) point in the Function, else null if this index
     * point doesn't exist
     */
    @Override
    ...
    public int getIndex(DataPoint2D point) {
        return points.getIndex(point);
    }

    /** Returns the x value of a point given the index */
    @Override
    public int getXIndex(double x) {
        return points.getIndex(new DataPoint2D(x, 0.0));
    }

    /**
     * Either adds a new DataPoint, or replaces an existing one, within
     * tolerance
     */
    @Override
    ...
    /**
     * Either adds a new DataPoint, or replaces an existing one, within
     * tolerance, created from the input x and y values.
     */
    @Override
    ...
    /**
     * Returns an iterator over all datapoints in the list. Results returned in
    ...
    /**
     * Given the imput y value, finds the two sequential x values with the
     * closest y values, then calculates an interpolated x value for this y
     * value, fitted to the curve.
     * <p>
     * 
     ...
     * 
     * @param y
     *            (value for which interpolated first x value has to be found
    ...
    public double getFirstInterpolatedX(double y) {
        ...
        if (!found)
            throw new InvalidRangeException("Y Value (" + y
                    + ") must be within the range: " + getY(0) + " and "
                    + getY(max - 1));
    ...
    /**
     * Given the input y value, finds the two sequential x values with the
     * closest y values, then calculates an interpolated x value for this y
     * value, fitted to the curve. The interpolated Y value returned is in the
     * linear space but the interpolation is done in the log space. Since there
     * may be multiple y values with the same value, this function just matches
     * the first found starting at the x-min point along the x-axis.
     * 
     * @param y
     *            : Y value in the linear space coressponding to which we are
    ...
    /**
     * Given the imput x value, finds the two sequential x values with the
     * closest x values, then calculates an interpolated y value for this x
     * value, fitted to the curve.
     * 
     * @param x
     *            (value for which interpolated first y value has to be found
    ...
    public double getInterpolatedY(double x) {
        ...
        if (x > getX(max - 1) || x < getX(0))
            throw new InvalidRangeException(
                    "x Value must be within the range: " + getX(0) + " and "
                            + getX(max - 1));
     ...
     * value of x. the interpolation of the Y value is done in the log space for
     * x and y values. The Y value returned is in the linear space but the
     * interpolation is done in the log space. If both bounding y values are
     * zero, then zero is returned. If only one of the bounding y values is
     * zero, that value is converted to Double.MIN_VALUE. If the interpolated y
     * value is Double.MIN_VALUE, it is converted to 0.0.
     * 
     * @param x
     *            : X value in the linear space corresponding to which we are
     ...
     * // finds the size of the point array int max=points.size(); double
     * x1=Double.NaN; double x2=Double.NaN; //if passed parameter(x value) is
     * not within range then throw exception if(x>getX(max-1) || x<getX(0))
     * throw new
     * InvalidRangeException("x Value must be within the range: "+getX(
     * 0)+" and "+getX(max-1)); //if x value is equal to the maximum value of
     * all given X's then return the corresponding Y value if(x==getX(max-1))
     * return getY(x); //finds the X values within which the the given x value
     * lies for(int i=0;i<max-1;++i) { x1=getX(i); x2=getX(i+1); if(x>=x1 &&
     * x<=x2) break; } //finding the y values for the coressponding x values
     * double y1 = getY(x1); double y2 = getY(x2); if(y1==0 && y2==0) return 0;
     * if(y1==0) y1 = Double.MIN_VALUE; if(y2==0) y2 = Double.MIN_VALUE; double
     * logY1=Math.log(y1); double logY2=Math.log(y2); if(debug) {
     * System.out.println("tol="+this.tolerance);
     * System.out.print(x1+"\t"+x2+"\t"+x+"\t"); } x1 = Math.log(x1); x2 =
     * Math.log(x2); x = Math.log(x); //using the linear interpolation equation
     * finding the value of y for given x double y=
     * ((logY2-logY1)*(x-x1))/(x2-x1) + logY1; double expY = Math.exp(y); if
     * (expY == Double.MIN_VALUE) expY = 0.0; if(debug) {
     * System.out.println(y1+"\t"
     * +y2+"\t"+logY1+"\t"+logY2+"\t"+x1+"\t"+x2+"\t"+x+
     * "\t"+y+"\t"+Math.exp(x1)
     * +"\t"+Math.exp(x2)+"\t"+Math.exp(x)+"\t"+expY+"\t"+Double.MIN_VALUE); }
     * 
     * return expY;
     * 
     * }
    ...
    /**
     * This function interpolates the y-axis value corresponding to the given
     * value of x. the interpolation of the Y value is done in the log-y space.
    ...
    public double getInterpExterpY_inLogYDomain(double x) {
        try {
            ...
            if (x < getX(0)) {
                return Math.exp(extrapolate(getX(0), getX(1),
                        Math.log(getY(0)), Math.log(getY(1)), x));
            } else {
                int max = points.size();
    ...
    /**
     * This function returns a new copy of this list, including copies of all
     * the points. A shallow clone would only create a new DiscretizedFunc
     * instance, but would maintain a reference to the original points.
     * <p>
     * 
     ...
     * value is the same. This requires the two lists to have the same number of
     * points.
     */
    ...
    /**
     * Standard java function, usually used for debugging, prints out the state
     * of the list, such as number of points, the value of each point, etc.
     * 
     * @return
     */
    @Override
    ...
    /**
     * Almost the same as toString() but used specifically in a debugging
     * context. Formatted slightly different
     * 
     * @return
     */
     ...
     * 
     * @param function
     *            DiscretizedFuncAPI function whose Y values will the Y values
     *            of the new ArbitrarilyDiscretizedFunc.
     * @return ArbitrarilyDiscretizedFunc new ArbitrarilyDiscretizedFunc
     */
    public ArbitrarilyDiscretizedFunc
            getYY_Function(DiscretizedFuncAPI function) {

        if (getNum() != function.getNum())
            throw new InvalidRangeException(
                    "This operation cannot be performed on functions "
                            + "with different size");

        ArbitrarilyDiscretizedFunc newFunction =
    ...
    /**
     * It finds out whether the X values are within tolerance of an integer
     * value
     * 
     * @param tol
    ...
    /*
     * temp main method to investige numerical precision issues public static
     ...
     * func.set(1+3e-16,3); func.set(1+4e-16,4); func.set(1+5e-16,5);
     * func.set(1+6e-16,6); func.set(1+7e-16,7); func.set(1+8e-16,8);
     * func.set(1+9e-16,9); func.set(1+10e-16,10); Iterator it =
     * func.getPointsIterator(); DataPoint2D point; while( it.hasNext()) { point
     * = (DataPoint2D) it.next();
     ...
     * while(it.hasNext()) { points.add( (DataPoint2D)it.next() ); }
     * 
     * // get all non-log points if any it = getNonLogPointsIterator(); if( it
     * != null ) while(it.hasNext()) { points.add( (DataPoint2D)it.next() ); }
     * 
     * // clear permanent storage points.clear(); nonPositivepoints.clear();
     * 
     * // rebuild permanent storage it = points.listIterator(); if( it != null )
     * while(it.hasNext()) { set( (DataPoint2D)it.next() ); }
     * 
     * if( D ) System.out.println("rebuild: " + toDebugString()); points = null;
     * }
     * 
     * public boolean isYLog() { return yLog; } public void setYLog(boolean
     * yLog) {
     * 
     * if( yLog != this.yLog ) { this.yLog = yLog; rebuild(); } }
     * 
     * public boolean isXLog() { return xLog; } public void setXLog(boolean
     * xLog) { if( xLog != this.xLog ) { this.xLog = xLog; rebuild(); } }
     */
