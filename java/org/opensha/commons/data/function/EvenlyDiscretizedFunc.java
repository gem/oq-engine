...
/**
 * <b>Title:</b> EvenlyDiscretizedFunc
 * <p>
 * 
 ...
 * This allows replacement of values at specified indexes.
 * <p>
 * 
 * Note that the basic unit for this function framework are DataPoint2D which
 ...
 * the DataPoint2D values as needed.
 * <p>
 * 
 ...
 * calculate any x-value by index.
 * <p>
 * 
 ...
 * values for histograms. The x value is the mid-point of the histogram interval
 * <p>
 * 
 * 
...
public class EvenlyDiscretizedFunc extends DiscretizedFunc {
    ...
    /**
     * This is one of two constructor options to fully quantify the domain of
     * this list, i.e. the x-axis.
     * 
     * @param min
     *            - Starting x value
     * @param num
     ...
     * x-axis. This function clears the list of points previously in this
     * function
     * 
     * @param min
     *            - Starting x value
    ...
    public void set(double min, double max, int num) {
        ...
        if (num == 1 && min != max)
            throw new DiscretizedFuncException(
                    "min must equal max if num points = 1");
     ...
     * aywhere along the x-axis, each point needs to be examined, lookup is
     * slower the larger the dataset.
     * <p>
     * 
     * Note: An alternative would be to check for the min value every time a
     * point is inserted and store the miny value. This would only slightly slow
     * down the insert, but greatly speed up the lookup.
     * <p>
     */
    ...
    /**
     * Returns an x and y value in a DataPoint2D based on index into the
     * y-points array. The index is based along the x-axis.
     */
     ...
     * tolerance of one of the dicretized values.
     */
    public int getXIndex(double x) throws DataPoint2DException {
        ...
        if (withinTolerance(x, this.getX(i)))
            ...
            throw new DataPoint2DException(C
                    + ": set(): This point doesn't match a permitted x value.");
        /*
         * // Steve's old approach:
         * 
         * double xx = x; // Why this? double xxMin = this.minX; // Why this?
         * 
         * for( int i = 0; i < num; i++){ if( withinTolerance(xx, ( xxMin +
         * i*delta ) ) ) return i; } throw new DataPoint2DException(C +
         * ": set(): This point doesn't match a permitted x value.");
         */
    }
    ...
    /**
     * this function will throw an exception if the index is not within the
     * range of 0 to num -1
     */
    public void set(int index, double y) throws DataPoint2DException {
        if (index < 0 || index > (num - 1)) {
            throw new DataPoint2DException(
                    C
                            + ": set(): The specified index doesn't match this function domain.");
        }
        points[index] = y;
    }
    ...
    /**
     * This method can be used for generating histograms if tolerance is set
     * greater than delta. Adds to the y-value at a specified index. The
     * specified x value is the mid-point of the histogram interval.
     * 
     * this function will throw an exception if the index is not within the
     * range of 0 to num -1
     ...
     * reconstitute all the DataPoint2D x-values by index, only y values are
     * stored internally in this function type. A DataPoint2D is built for each
     * y value and added to a local ArrayList. Then the iterator of the local
    ...
    public double getInterpolatedY(double x) {
        ...
        if (x > getX(num - 1) || x < getX(0))
            throw new InvalidRangeException("x Value (" + x
                    + ") must be within the range: " + getX(0) + " and "
                    + getX(num - 1));
    ...
    /**
     * Returns a copy of this and all points in this DiscretizedFunction. A
     * copy, or clone has all values the same, but is a different java class
     * instance. That means you can change the copy without affecting the
     * original instance.
     * <p>
     * 
    ...
    public boolean equalXAndYValues(DiscretizedFuncAPI function) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (y1 == Double.NaN && y2 != Double.NaN)
                return false;
            else if (y2 == Double.NaN && y1 != Double.NaN)
                return false;
            else if (y1 != y2)
                return false;
     ...
     * x-value and y-values in list should match with that of point returns -1
