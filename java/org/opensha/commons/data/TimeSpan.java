 ...
 * <p>
 * 
 * <b>Description:</b> This object represents a start time and a duration.
 * <p>
 * 
 * The start-time is represented with a Year, Month, Day, Hour, Minute, Second,
 * and Millisecond, all of which are stored internally with IntegerParameter
 * objects. The default constraints (range of allowed values) for these
 * parameters are:
 * <p>
 * <UL>
 ...
 * differs from the 0-11 range in the java.util.GregorianCalendar object. This
 * means that what's returned from the getStartTimeMonth() method is always one
 * greater than what's obtained using
 * getStartTimeCalendar().get(Calendar.MONTH)). Keep this in mind if you use the
 * setStartTimeCalendar(), getStartTimeCalendar(), or getEndTimeCalendar
 * methods. 2) the Day and Hour fields here correspond to the DATE and
 * HOUR_OF_DAY fields, respecively in java.util.GregorianCalendar (the HOUR
 * field of GregorianCalendar goes from 0 to 11 rather than 0 to 23).
 * <p>
 * 
 * The above start-time parameter constraints can be overridden using the
 * setStartTimeConstraint() method.
 * 
 * The startTimePrecision field specifies the level of precision. For example,
 * if this is set as "Days", then one cannot set or get the Hours, Minute,
 * Second, or Millisecond parameter values (and the associated methods throw
 * exceptions). Setting the startTimePrecsion as "None" indicates that only the
 * Duration is relevant (e.g., for a Poissonian forecast). Presently one can
 * only set the startTimePrecision in the constructor, but we could relax this
 * later. If one gets a GregorianCalendar using the getStartTimeCalendar()
 * method, the fields that are not within the specified precision are set to
 * their minimum value.
 * <p>
 * 
 * Before a value is returned from any one of the getStartTime*() methods, it is
 * first confirmed that the start-time parameter settings correspond to an
 * acutual date. For example, assuming the startTimePrecision is "Months", one
 * could execute the method: setStartTime(2003,2,29). However, when they go to
 * get one of these fields (e.g., getStartYear() or getStartMonth()) an
 * exception will be thrown because there are not 29 days in Feburary (unless
 * it's a leap year). This check is made in the get* rather than set* methods to
 * allow users to finish their settings (e.g., in a GUI) before checking values.
 * 
 * The Units on the Duration field must presently be set in the constructor,
 * (this could be relaxed later). These Units are assumed when using the
 * getDuration() and setDuration(double duration) methods. If one wishes to get
 * or set the duration with other units (e.g., to avoid having to check what the
 * internal units are), they can use the setDuration(String units, double
 * duration) and getDuration(String units, double duration) methods, but note
 * that the internal "chosen" units will not have changed. The constraints on
 * the units can be set using the setDurationConstraint() method. NOTE: in
 * converting between duration units it is assumed there are 365.25 days per
 * year (correct only for durations in years that are an integer when divided by
 * four). This limitation can be fixed, but will require some thought.
 * <p>
 * 
 * Finally, one can get an end-time calendar object that corresponds to the
 * start time plus the duration (getEndTimeCalendar()). Note, that because this
 * invoves a duration unit conversion, it is assumed that there are 365.25 days
 ...
 * allowing an "Adjustable" opting for the duration units in the constructor.
 * For this case, make the units field of the durationParameter empty, and use
 * the value set in the durationUnitsParameter for the getDuration() and
 * setDuration() methods. Also add durationUnitsParameter to the list returned
 * by the getAdjustableParamsList(). This raises the question of how non-default
 * constraints can be applied if the user is changing the units (perhaps only
 * default constraints can be applied in this case). Care will be required in
 * using the getDuration() and setDuration() methods if other objects can change
 * the units (perhaps these should throw and non-usable exception).
 * <p>
 * 
 * We might need a setNonEditable() method here (e.g., to prevent ouside
 * entities from changing constraints).
 * 
 * 
 * 
 * 
 * @author Edward Field, based on an earlier version by Sid Hellman and Steven
 *         W. Rock.
 * @created March, 2003
 * @version 1.0
 */
public class TimeSpan implements ParameterChangeListener, Serializable {
    ...
    private IntegerConstraint startYearConstraint = new IntegerConstraint(0,
            Integer.MAX_VALUE);
    ...
    private IntegerConstraint startMonthConstraint = new IntegerConstraint(1,
            12);
    ...
    private IntegerConstraint startHourConstraint =
            new IntegerConstraint(0, 59);
    ...
    private IntegerConstraint startMinuteConstraint = new IntegerConstraint(0,
            59);
    ...
    private IntegerConstraint startSecondConstraint = new IntegerConstraint(0,
            Integer.MAX_VALUE);
    public final static String START_MILLISECOND = "Start Second";
    private IntegerParameter startMillisecondParam;
    private IntegerConstraint startMillisecondConstraint =
            new IntegerConstraint(0, 999);
    ...
    private final static String START_TIME_ERR =
            "Start-Time Precision Violation: ";
    ...
    private DoubleConstraint durationConstraint = new DoubleConstraint(0.0,
            Double.MAX_VALUE);
    ...
    // whenver any change is made in this timespan object, all the listeners are
    // notified
    ...
    private void initParams() {
        ...
        // Start Time Parameters
        startYearParam =
                new IntegerParameter(START_YEAR, startYearConstraint,
                        START_YEAR_DEFAULT);
        startMonthParam =
                new IntegerParameter(START_MONTH, startMonthConstraint,
                        START_MONTH_DEFAULT);
        startDayParam =
                new IntegerParameter(START_DAY, startDayConstraint,
                        START_DAY_DEFAULT);
        startHourParam =
                new IntegerParameter(START_HOUR, startHourConstraint,
                        START_HOUR_DEFAULT);
        startMinuteParam =
                new IntegerParameter(START_MINUTE, startMinuteConstraint,
                        START_MINUTE_DEFAULT);
        startSecondParam =
                new IntegerParameter(START_SECOND, startSecondConstraint,
                        START_SECOND_DEFAULT);
        startMillisecondParam =
                new IntegerParameter(START_MILLISECOND,
                        startMillisecondConstraint, START_MILLISECOND_DEFAULT);
        ...
        durationUnitsParam =
                new StringParameter(this.DURATION_UNITS,
                        durationUnitsConstraint, DURATION_UNITS_DEFAULT);

        // Duration Parameters (continuous versus discrete; only one used at any
        // one time)
        durationParam =
                new DoubleParameter(DURATION, durationConstraint,
                        DURATION_UNITS_DEFAULT, DURATION_DEFAULT);
        discreteDurationParam =
                new DoubleDiscreteParameter(DURATION, DURATION_UNITS_DEFAULT,
                        DURATION_DEFAULT);
        ...
        precisionConstraint.addString(NONE); // this one is for start-time
                                             // independent models (e.g.,
                                             // Poissonian)
        precisionConstraint.setNonEditable();
        startTimePrecisionParam =
                new StringParameter(START_TIME_PRECISION, precisionConstraint,
     ...
     * and max outside the default bounds (e.g., min and max for "Start Hour"
     * must be between 0 and 23). Note also that this method ignores the
     * start-time precision (e.g., you can set new constraints on "Start Minute"
     * even if the start-time precision has been set as "Years").
     * 
     * @param name
    ...
    public void setStartTimeConstraint(String name, int min, int max) {
        ...
        if (name.equals(START_YEAR)) {
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startYearConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
                startYearParam.setConstraint(constraint);
            else
                throw new RuntimeException(
                        "TimeSpan.setStartTimeConstraint(): max or min is outside absolute bounds for \""
                                + name + "\"");
        } else if (name.equals(START_MONTH)) {
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startMonthConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startDayConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startHourConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startMinuteConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startSecondConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            // check that min and max are within the originally defined
            // constraint (absolute constraint)
            if (startMillisecondConstraint.isAllowed(min)
                    && startYearConstraint.isAllowed(max))
            ...
            throw new RuntimeException(
                    "TimeSpan.setStartTimeConstraint(): invalid name");
    }

    /**
     * Sets the Start-Time Precision. Options are "Years", "Months", "Days",
     * "Hours", "Minutes", "Seconds" and "Milliseconds". "None" can also be set
     * if the start-time is not needed (e.g., for Poissonian models).
     * 
     * @param startTimePrecision
     */
    ...
    /**
     * This returns the start-time precision's integer equivalent (0 for NONE, 1
     * for YEARS, 2 for MONTHS, 3 for DAYS, 4 for HOURS, 5 for MINUTES, 6 for
     * SECONDS, and 7 for MILLISECONDS).
     * 
     * @return precision integer
     */
    private int getStartTimePrecInt() {
        String precisionUnitString =
                (String) startTimePrecisionParam.getValue();
        if (precisionUnitString.equalsIgnoreCase(NONE))
            return 0;
        else if (precisionUnitString.equalsIgnoreCase(YEARS))
            return 1;
        else if (precisionUnitString.equalsIgnoreCase(MONTHS))
            return 2;
        else if (precisionUnitString.equalsIgnoreCase(DAYS))
            return 3;
        else if (precisionUnitString.equalsIgnoreCase(HOURS))
            return 4;
        else if (precisionUnitString.equalsIgnoreCase(MINUTES))
            return 5;
        else if (precisionUnitString.equalsIgnoreCase(SECONDS))
            return 6;
        else
            return 7; // milliseconds }
    }
    ...
    public int getStartTimeFromType(String type) {
        if (type.equals(START_DAY))
            ...
            throw new RuntimeException("Type '" + type
                    + "' is not a valid start time precision!");
    }

    /**
     * @return Start-time year
     * @throws RuntimeException
     *             if year is not within the start-time precision.
     */
    public int getStartTimeYear() throws RuntimeException {
        if (getStartTimePrecInt() >= 1) {
            // check the start-time parameter settings (e.g., to make sure day
            // exists in chosen month)
            checkStartTimeValues();
            return ((Integer) startYearParam.getValue()).intValue();
        } else {
            String str =
                    "cannot use the getStartTimeYear() method because start-time precision is \"";
     ...
     * @throws RuntimeException
     *             if month is not within the start-time precision.
     */
    public int getStartTimeMonth() throws RuntimeException {
        if (getStartTimePrecInt() >= 2) {
            ...
            String str =
                    "cannot use the getStartTimeMonth() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * @throws RuntimeException
     *             if day is not within the start-time precision.
     */
    public int getStartTimeDay() throws RuntimeException {
        if (getStartTimePrecInt() >= 3) {
            ...
            String str =
                    "cannot use the getStartTimeDay() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * @throws RuntimeException
     *             if hour is not within the start-time precision.
     */
    public int getStartTimeHour() throws RuntimeException {
        if (getStartTimePrecInt() >= 4) {
            ...
            String str =
                    "cannot use the getStartTimeHour() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * @throws RuntimeException
     *             if minute is not within the start-time precision.
     */
    public int getStartTimeMinute() throws RuntimeException {
        if (getStartTimePrecInt() >= 5) {
            ...
            String str =
                    "cannot use the getStartTimeMinute() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * @throws RuntimeException
     *             if second is not within the start-time precision.
     */
    public int getStartTimeSecond() throws RuntimeException {
        if (getStartTimePrecInt() >= 6) {
            ...
            String str =
                    "cannot use the getStartTimeSecond() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * @throws RuntimeException
     *             if millisecond is not within the start-time precision.
     */
    public int getStartTimeMillisecond() throws RuntimeException {
        if (getStartTimePrecInt() >= 7) {
            ...
            String str =
                    "cannot use the getStartTimeMillisecond() method because start-time precision is \"";
            String prec = getStartTimePrecision();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
     ...
     * are "Years", "Days","Hours", "Minutes", "Seconds", or "Milliseconds".
     * This does not change the "chosen" units held internally.
     * 
     * @param duration
     *            - in the units supplied
     * @param units
     *            - the units of the passed in duration
     */
    ...
    public void setDuractionConstraint(double min, double max) {
        ...
        if (durationConstraint.isAllowed(min)
                && durationConstraint.isAllowed(min)) {
            ...
            throw new RuntimeException(C
                    + "setDuractionConstraint - negative values not allowed");

    }

    /**
     * This puts a new discrete constraint (list of doubles) on the duration
     * parameter. All the new values must be within the default values (0 and
    ...
    public void setDurationConstraint(ArrayList doubles) {
        ...
        DoubleDiscreteConstraint constraint =
                new DoubleDiscreteConstraint(doubles);
    ...
    /**
     * This converts the input duration in it's present units into the desired
     * units. Duration-unit options are "Years", "Days","Hours", "Minutes",
     * "Seconds", or "Milliseconds". This assumes that there are 365.25 days per
     * year (correct only when the duration in years is an integer when divided
     * by 4).
     * 
     * @param duration
     *            - the duration after units conversion
     * @param presentUnits
     *            - units of the input
     * @param desiredUnits
     *            - units of the output
     * @return
     */
    private double convertDurationUnits(double duration, String presentUnits,
            String desiredUnits) {
    ...
    public double getDuration(String units) {
        ...
        if (isDurationDiscrete)
            duration =
                    ((Double) discreteDurationParam.getValue()).doubleValue();
        else
     ...
     * @param year
     * @throws RuntimeException
     *             if start-time precision is not "Years"
     */
    public void setStartTime(int year) throws RuntimeException {
        ...
        if (getStartTimePrecInt() == 1)
            ...
            throw new RuntimeException(START_TIME_ERR + "can't use the "
                    + method + " method because start-time precision is \""
                    + prec + "\"");
        }
    }
     ...
     * @params year, month
     * @throws RuntimeException
     *             if start-time precision is not "Months"
     */
     ...
     * @throws RuntimeException
     *             if start-time precision is not "Days"
     */
     ...
     * @throws RuntimeException
     *             if start-time precision is not "Hours"
     */
    public void setStartTime(int year, int month, int day, int hour)
        ...
        if (getStartTimePrecInt() == 4) {
            ...
            String prec = (String) startTimePrecisionParam.getValue();
            String method =
                    "setStartTime(int year, int month, int day, int hour)";
            throw new RuntimeException(START_TIME_ERR + "can't use the "
                    + method + " method because start-time precision is \""
                    + prec + "\"");
        }
     ...
     * @throws RuntimeException
     *             if start-time precision is not "Minutes"
     */
    public void
            setStartTime(int year, int month, int day, int hour, int minute)
                    throws RuntimeException {
     ...
     * @throws RuntimeException
     *             if start-time precision is not "Seconds"
     */
    public void setStartTime(int year, int month, int day, int hour,
            int minute, int second) throws RuntimeException {
        ...
        if (getStartTimePrecInt() == 6) {
            ...
            String prec = (String) startTimePrecisionParam.getValue();
            String method =
                    "setStartTime(int year, int month, int day, int hour, int minute, int second)";
            throw new RuntimeException(START_TIME_ERR + "can't use the "
                    + method + " method because start-time precision is \""
                    + prec + "\"");
        }
     ...
     * @throws RuntimeException
     *             if start-time precision is not "Milliseconds"
     */
    public void setStartTime(int year, int month, int day, int hour,
            int minute, int second, int millisecond) throws RuntimeException {
        ...
        if (getStartTimePrecInt() == 7) {
            ...
            String prec = (String) startTimePrecisionParam.getValue();
            String method =
                    "setStartTime(int year, int month, int day, int hour, int minute, int second, int millisecond)";
            throw new RuntimeException(START_TIME_ERR + "can't use the "
                    + method + " method because start-time precision is \""
                    + prec + "\"");
        }
    ...
    /**
     * This checks whether the start-time parameter values correspond to an
     * actaul date (e.g., can't have day=29 if month=2, unless it's a leap
     * year). Currently this is done by simply rebuilding the startTimeCalendar
     * (which will throw and exception if there is a problem), but a more
     * efficient approach could be implemented later.
     * 
     * @return
     */
    private void checkStartTimeValues() {
        // for efficiency there should be an if statement here to check whether
        // any parameters have changed
        buildStartTimeCalendar();
    }

    /**
     * This sets the Start-Time Calendar, making any fields greater than the
     * Start-Time Precision equal to defaults (usually the lowest allowed
     * value). This throws an exception if the Day is incompatable with the
     * chosen Month (and perhaps if any other problems are encountered, although
     * I can't think of any give our constraints on each parameter).
     * 
     * @throws Exception
     */
    private void buildStartTimeCalendar() throws RuntimeException {
        ...
        // set the month (subtract one to make compatible with GregorianCalendar
        // indexing)
        if (precisionInt > 1)
        ...
        if (precisionInt > 6)
            millisecond =
                    ((Integer) startMillisecondParam.getValue()).intValue();
        else
            millisecond = START_MILLISECOND_DEFAULT.intValue();

        // now make the calendar
        startTimeCal = new GregorianCalendar();//
        startTimeCal.setTimeZone(TimeZone.getTimeZone("GMT"));// TODO pls check,
                                                              // set to use same
                                                              // time zone
        ...
        try {
            ...
            throw new RuntimeException(
                    "Calendar Error: Invalid Day for the chosen Month");
        }
     ...
     * and millisecond) from the inpute GregorianCalendar. Fields above the
     * start-time precision are igored. For example, if the start- time
     * precision equals "Hour", then the year, month, day, and hour are set, but
     * the minute, second, and millisecond fields are not. If start- time
     * precision equals "None", then none of the fields are filled in.
     * 
     * @param cal
     */
    public void setStartTime(GregorianCalendar cal) {
        ...
        } // do nothing if getStartTimePrecInt() == 0

    }

    /**
     * This returns an end-time GregorianCalendar representing the start time
     * plus the duration. Note that this correctly accounts for leap years (and
     * leap seconds?) thanks to the sophistication of the
     * java.util.GregorianCalendar object. Note also the indexing difference for
     * the Month field (our parameter goes from 1 to 12, whereas
     * GregorianCalendar.MONTH goes from 0 to 11).
     */
    public GregorianCalendar getEndTimeCalendar() {
        if (getStartTimePrecInt() > 0) {
            ...
            long endTime_mSec =
                    startTimeCal.getTime().getTime() + durMsec.longValue();
            GregorianCalendar cal = new GregorianCalendar();
            cal.setTime(new Date(endTime_mSec));
            return cal;
        } else {
            String str =
                    "Can't use getEndTime() method because start-time precision = \"";
            String prec = (String) startTimePrecisionParam.getValue();
            throw new RuntimeException(START_TIME_ERR + str + prec + "\"");
        }
    }
    ...
    /**
     * This returns a ParameterList Iterator(e.g., to put in a GUI so users can
     * set values). This only includes start-time parameters that are within the
     * chosen precision.
     * 
     * @return
     */
    ...
    /**
     * This returns a ParameterList (e.g., to put in a GUI so users can set
     * values). This only includes start-time parameters that are within the
     * chosen precision.
     * 
     * @return
     */
     ...
     * (generally the lowest possible value). For example, if start-time
     * precision equals "Day", then the HOUR_OF_DAY, MINUTE, SECOND, and
     * MILLISECOND fields of the returned GregorianCalendar are all set to 0.
     * Note also the indexing difference for the Month field (our parameter goes
     * from 1 to 12, whereas GregorianCalendar.MONTH goes from 0 to 11).
     * 
     * @return
     * @throws Exception
     */
    public GregorianCalendar getStartTimeCalendar() throws RuntimeException {
        ...
        if (getStartTimePrecInt() == 0) {
            String str =
                    "cannot use the getStartTimeCalendar() method when start-time precision equals \"";
     ...
     * 
     * @param listener
     *            : Object that wants to listen to changes in Timespan object
    ...
    /**
     * this function is called whenenver any parameter changes in the params
     * list This function then notifies all the listeners about this change
     * 
     * @param e
     */
    ...
    public static TimeSpan fromXMLMetadata(Element el) {
        ...
        double duration =
                Double.parseDouble(el.attribute("duration").getValue());
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (att.getName().equals(TimeSpan.START_DAY.replaceAll(" ", ""))) {
                ...
                num++;
                    ...
                    TimeSpan.START_HOUR.replaceAll(" ", ""))) {
                hour = Integer.parseInt(att.getValue());
                num++;
            } else if (att.getName().equals(
                    TimeSpan.START_MILLISECOND.replaceAll(" ", ""))) {
                millisecond = Integer.parseInt(att.getValue());
                num++;
            } else if (att.getName().equals(
                    TimeSpan.START_MINUTE.replaceAll(" ", ""))) {
                minute = Integer.parseInt(att.getValue());
                num++;
            } else if (att.getName().equals(
                    TimeSpan.START_MONTH.replaceAll(" ", ""))) {
                month = Integer.parseInt(att.getValue());
                num++;
            } else if (att.getName().equals(
                    TimeSpan.START_SECOND.replaceAll(" ", ""))) {
                second = Integer.parseInt(att.getValue());
                num++;
            } else if (att.getName().equals(
                    TimeSpan.START_YEAR.replaceAll(" ", ""))) {
