...
/**
 * <p>title: GEMch: Gem Hazard Curves Calculator </p>
 * 
 * <p>This is the "final" GEM1 calculator. It accepts a logic tree on GMPEs and
 * an Hazard Calculator that accepts ERF with sources defined in terms of a 
 * Tectonic Environment</p>
 * 
...
public class GemComputeHazard implements Runnable {
	 ...
	 */
     public GemComputeHazard(int nproc, ArrayList<Site> siteList, EqkRupForecast ERF, HashMap<TectonicRegionType,ScalarIntensityMeasureRelationshipAPI> gmpeMap,ArbitrarilyDiscretizedFunc imlList, double maxSourceDist){	
    	...
    	String intensityMeasType = gmpeMap.get(trt).getIntensityMeasure().getName();
	...
	private void loopDoRange(int start, int end){	
		...
		ArbitrarilyDiscretizedFunc hcThread = (ArbitrarilyDiscretizedFunc) UnoptimizedDeepCopy.copy(imlList);
