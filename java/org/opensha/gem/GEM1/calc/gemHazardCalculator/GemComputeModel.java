...
public class GemComputeModel {
	...
	/**
	 * 
	 * @param inputToErf: GemLogicTree for input model
	 * @param gmpeLT: GemLogicTree for gmpes
	 * @param hazSite: list of sites where to compute hazard
	 * @param probLevel: probability level for hazard map
	 * @param outDir
	...
	public GemComputeModel(GemLogicTree<ArrayList<GEMSourceData>> modelLogicTree,
	    ...
	    // create a directory where to store results; all ancestor directories must exist
		...
		GemComputeHazardLogicTree compHaz = new GemComputeHazardLogicTree(this.modelLogicTree,this.gmpeLogicTree,this.calcSet);
        ...
        System.out.println("Wall clock time (excluding time for saving output files)");
        // 1h = 60*60*10^3 ms
	...
	/**
	 * 
	 * @param gemLogicTreeFile: file containing GemLogicTree object representing input model
	 * @param gmpeLT: GemLogicTree object for gmpes
	 * @param latmin: minimum latitude of rectangular region
	 * @param latmax: maximum latitude of rectangular region
	 * @param lonmin: minimum longitude of rectangular region
	 * @param lonmax: maximum longitude of rectangular region
	 * @param delta: discretization of rectangular region
	 * @param probLevel: probability level for hazard map
	 * @param outDir: output directory where store results
	 * @param outputHazCurve: true if you want to print also hazard curves
	 * @param calculation settings
	...
	public GemComputeModel(String gemLogicTreeFile, 
			...
			String outDir,
			boolean outputHazCurve,
			CalculationSettings calcSet) throws IOException, ClassNotFoundException{
			
		// initialize logic tree for input model
		GemLogicTree<ArrayList<GEMSourceData>> inputToErf = new GemLogicTree<ArrayList<GEMSourceData>>(gemLogicTreeFile);
		...
		new GemComputeModel(inputToErf, gmpeLogicTree, hazSite, probLevel, outDir, outputHazCurve, calcSet);
...
//		ArrayList<Site> hazSite = createListOfSitesFromRectangularRegion(latmin,latmax,lonmin,lonmax,delta);
	 ...
	 * @return
	 */
	private ArrayList<Site> createListOfSitesFromRegion(Region reg, double delta){
	 ...
	 * @return
	 */
	private ArrayList<Site> createListOfSitesFromRectangularRegion(double latmin, double latmax, double lonmin, double lonmax, double delta){
	...
	private void saveMeanHazardMap(){
		...
		for(int ip=0;ip<probLevel.length;ip++){
			...
			ArrayList<Double> meanHazMap = endBranchHazCurveList.getMeanGroundMotionMap(probLevel[ip], modelLogicTree, gmpeLogicTree);

			String duration = calcSet.getOut().get(TimeSpan.DURATION).toString();
			String intensityMeasureType = endBranchHazCurveList.getHcRepList().get(0).getIntensityMeasureType();
			String modelName = modelLogicTree.getModelName();
			
			// print mean hazard map
	        try {
	        	String outfile = dirName+modelName+"_hazardMap_"+intensityMeasureType+"_"+(probLevel[ip]*100)+"pe"+duration+"yr_mean"+".dat";
	...
	private void saveHazardCurves() throws IOException{
		...
		int nGMV = endBranchHazCurveList.getHcRepList().get(0).getGmLevels().size();
		...
		for(int iebr=0;iebr<endBranchHazCurveList.getHcRepList().size();iebr++){
			...
			// String outfile = dirName+endBranchHazCurveList.getEndBranchLabels().get(iebr)+".dat";//"-"+Double.toString(lat)+"_"+Double.toString(lon)+".dat";
			String branching = endBranchHazCurveList.getEndBranchLabels().get(iebr)+".dat";
			String outfile = dirName+modelName+"_hazardCurves_"+intensityMeasureType+"_"+duration+"yr"+"_"+
				branching.replaceAll("_","-");
			...
			for(int igmv=0;igmv<nGMV;igmv++){
				double gmv = endBranchHazCurveList.getHcRepList().get(iebr).getGmLevels().get(igmv);
				gmv = Math.exp(gmv);
			...
			for(int igp=0;igp<endBranchHazCurveList.getHcRepList().get(iebr).getNodesNumber();igp++){
				
				double lat = endBranchHazCurveList.getHcRepList().get(iebr).getGridNode().get(igp).getLocation().getLatitude();
				double lon = endBranchHazCurveList.getHcRepList().get(iebr).getGridNode().get(igp).getLocation().getLongitude();
				oWriter.write(String.format("%+8.4f %+7.4f ",lon,lat));
				
					// loop over ground motion values
					for(int igmv=0;igmv<nGMV;igmv++){
						double rateEx = 0.0;
						double probEx = endBranchHazCurveList.getHcRepList().get(iebr).getProbExceedanceList(igp)[igmv];
//						if (probEx < 0.99){
...
//    	String outfile = dirName+modelName+"_hazardCurves_"+intensityMeasureType+"_"+duration+"yr"+"_mean.dat";
//    	
//        FileOutputStream oOutFIS = new FileOutputStream(outfile);
//        BufferedOutputStream oOutBIS = new BufferedOutputStream(oOutFIS);
//        BufferedWriter oWriter = new BufferedWriter(new OutputStreamWriter(oOutBIS));
...
//        oWriter.write(String.format("%8s %8s "," "," "));
//		for (int igmv=0;igmv<nGMV;igmv++) {
//			double gmv = endBranchHazCurveList.getHcRepList().get(0).getGmLevels().get(igmv);
//			gmv = Math.exp(gmv);
//			oWriter.write(String.format("%7.4e ",gmv));
//		}
//		oWriter.write("\n");
//		
//		// loop over grid points
//		for(int igp=0;igp<endBranchHazCurveList.getHcRepList().get(0).getNodesNumber();igp++){
//			double lat = endBranchHazCurveList.getHcRepList().get(0).getGridNode().get(igp).getLocation().getLatitude();
//			double lon = endBranchHazCurveList.getHcRepList().get(0).getGridNode().get(igp).getLocation().getLongitude();
////			oWriter.write(lon+" "+lat+" ");
//			oWriter.write(String.format("%+8.4f %+7.4f ",lon,lat));
//			
//				// loop over ground motion values
//				for(int igmv=0;igmv<nGMV;igmv++){
//					double probEx = endBranchHazCurveList.getMeanHazardCurve(modelLogicTree,gmpeLogicTree).getProbExceedanceList(igp)[igmv];
...
//		
//		// loop over end-branches (each end-branch has its own hazard curve repository)
//		for(int endBranchIndex = 0; endBranchIndex < endBranchHazCurveList.getEndBranchLabels().size(); ++endBranchIndex) {
//			
//			// the hazard curves for the end branch
//			GEMHazardCurveRepository ghcr = endBranchHazCurveList.getHcRepList().get(endBranchIndex);
//			
//			// the endBranchLabel
//			String endBranchLabel = endBranchHazCurveList.getEndBranchLabels().get(endBranchIndex);
//			
//			// important: Append one SHAML "Result" element (of HazardResultType) per end branch / GemHazardCurveRepository
...
//			String imt = endBranchHazCurveList.getHcRepList().get(0).getIntensityMeasureType();
//			hrt.setIMT(imt);
//			
//			// result descriptor
//			hrt.getDescriptor().setEndBranchLabel(endBranchLabel);
//			
//			HazardCurveListType hclt = Shaml.Output.appendHazardCurveListType(hrt.getValues());
//			
...
//			
//			// loop over grid points
//			for(int gridPointIndex = 0; gridPointIndex < ghcr.getNodesNumber() ;++gridPointIndex) {
//				double lat = ghcr.getGridNode().get(gridPointIndex).getLocation().getLatitude();
//				double lon = ghcr.getGridNode().get(gridPointIndex).getLocation().getLongitude();
//				HazardCurveType hct = Shaml.Output.appendHazardCurveType(hclt.getList(), new Double(lon), new Double(lat));
