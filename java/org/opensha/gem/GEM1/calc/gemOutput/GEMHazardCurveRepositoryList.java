...
public class GEMHazardCurveRepositoryList
{
     ...
     */
    public GEMHazardCurveRepository getMeanHazardCurve(
            ...
            GemLogicTree<HashMap<TectonicRegionType, ScalarIntensityMeasureRelationshipAPI>> gmpeLT)
    {
        ...
        GEMHazardCurveRepository meanhc = new GEMHazardCurveRepository(
        ...
        for (int i = 0; i < hcRepList.size(); i++)
        {
            ...
            double wei = ilTree.getTotWeight(erfLab)
                    * gmpeLT.getTotWeight(gmpeLab);
    ...
    public GEMHazardCurveRepository getQuantiles(double probLevel)
    {
        ...
        // the initialization is done considering the first HazardCurveRepository in the HazardCurveRepositoryList
        UnoptimizedDeepCopy udp = new UnoptimizedDeepCopy();
        GEMHazardCurveRepository hcRep = new GEMHazardCurveRepository(
                (ArrayList<Site>) udp.copy(hcRepList.get(0).getGridNode()),
                (ArrayList<Double>) udp.copy(hcRepList.get(0).getGmLevels()),
                (ArrayList<Double[]>) udp
                        .copy(hcRepList.get(0).getProbExList()), hcRepList.get(
     ...
     * @return 				meanHM
     */
    public ArrayList<Double> getMeanHazardMap(
            double probEx,
            GemLogicTree<ArrayList<GEMSourceData>> inputToERFLT,
            GemLogicTree<HashMap<TectonicRegionType, ScalarIntensityMeasureRelationshipAPI>> gmpeLT)
    {
        GEMHazardCurveRepository meanHC = getMeanHazardCurve(inputToERFLT,
                gmpeLT);
        ArrayList<Double> meanHM = meanHC.getHazardMap(probEx);
        return meanHM;
    }

    public ArrayList<Double> getMeanGroundMotionMap(
            double probEx,
            GemLogicTree<ArrayList<GEMSourceData>> ilTree,
            GemLogicTree<HashMap<TectonicRegionType, ScalarIntensityMeasureRelationshipAPI>> gmpeLT)
    {
            ...
            HashMap<TectonicRegionType, GemLogicTree<ScalarIntensityMeasureRelationshipAPI>> gmpeLogicTreeHashMap)
    {
        ...
        for (int i = 0; i < hcRepList.size(); i++)
        {
            ...
            {
                StringTokenizer st = new StringTokenizer(labTokens.nextToken(),
                        "_");
    ...
    /**
     * Computes the PMFs using the curves defined in this model.
     * <p>
     * The map is used to have a clear data structure for the further
     * serialization. I was not able to serialize the OpenSHA object model.
     * 
     * @return the list of computed PMFs
     */
    public List<Map<Double, Double>> computePMFs()
    {
        List<Map<Double, Double>> PMFs = 
                new ArrayList<Map<Double, Double>>();
        ...
        for (GEMHazardCurveRepository set : hcRepList)
        {
            for (Double[] probs : set.getProbExList())
            {
                ...
                PMFs.add(ProbabilityMassFunctionCalc.getPMF(function).asMap());
            }
        }
    ...
    /**
     * Serializes the PMFs generated by this model in cache.
     * 
     * @param cache the cache used to store the serialized
     * version of the PMFs
     * @return the key used as index in cache
     */
    public String serializePMFs(Cache cache)
    {
        String key = new Integer(this.hashCode()).toString();
        cache.set(key, new Gson().toJson(computePMFs()));
