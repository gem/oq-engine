...
public class GscFriskInputAlternative {
	...
	private int maxVtx; //				L2 Alt		: Maximum number of points used to describe a source
	...
	private boolean maxMagFlg; // 		L1 After Alt: Flag (1 if Mmax is source dependent)
	private boolean nuBetaFlg; //		L1 After Alt: Flag (1 if nuBeta pairs are dependent) 
	private boolean depthsFlg; //		L1 After Alt: Flag (1 if depths pairs are dependent)
	private boolean gmpeFlg; //			L1 After Alt: Flag (1 if gmpes pairs are dependent)
	 ...
	 * @throws IOException
	 */
	public GscFriskInputAlternative(BufferedReader input, GscFriskInputHeader head) throws IOException {
		...
		if (matcher.find()) this.nSrcSets = Integer.valueOf(matcher.group(0)).intValue();
		matcher = INTNUM.matcher(strarr[1]);
		if (matcher.find()) this.nNuBeta = Integer.valueOf(matcher.group(0)).intValue();
		matcher = INTNUM.matcher(strarr[2]);
		if (matcher.find()) this.nMaxMag = Integer.valueOf(matcher.group(0)).intValue();
		matcher = INTNUM.matcher(strarr[3]);
		if (matcher.find()) this.nDepths = Integer.valueOf(matcher.group(0)).intValue();
		matcher = INTNUM.matcher(strarr[4]);
		if (matcher.find()) this.nSrcTot = Integer.valueOf(matcher.group(0)).intValue();
		matcher = INTNUM.matcher(strarr[5]);
		if (matcher.find()) this.maxVtx = Integer.valueOf(matcher.group(0)).intValue();
		...
		for (j=0; j < this.nMaxMag; j++ ) {
			...
			if (matcher.find()){
				this.maxMagWei[j] = Double.valueOf(matcher.group(0)).doubleValue();
			} else {
		...
		for (j=0; j < this.nNuBeta; j++ ) {
			matcher = FLOATNUM.matcher(strarr[j]);
			if (matcher.find()){
				this.nuBetaWei[j] = Double.valueOf(matcher.group(0)).doubleValue();
			} else {
		...
		for (j=0; j < this.nDepths; j++ ) {
			matcher = FLOATNUM.matcher(strarr[j]);
			if (matcher.find()){
				this.depthsWei[j] = Double.valueOf(matcher.group(0)).doubleValue();
			} else {
		...
		if (matcher.find()) {
			...
			if (Integer.valueOf(matcher.group(0)).intValue() == 1) this.maxMagFlg = true;
		}
		//
		matcher = INTNUM.matcher(strarr[1]);
		if (matcher.find()) {
			this.nuBetaFlg = false;
			if (Integer.valueOf(matcher.group(0)).intValue() == 1) this.nuBetaFlg = true;
		}
		//
		matcher = INTNUM.matcher(strarr[2]);
		if (matcher.find()) {
			this.depthsFlg = false;
			if (Integer.valueOf(matcher.group(0)).intValue() == 1) this.depthsFlg = true;
		}
		//
		matcher = INTNUM.matcher(strarr[3]);
		if (matcher.find()) {
			this.gmpeFlg = false;
			if (Integer.valueOf(matcher.group(0)).intValue() == 1) this.gmpeFlg = true;
		}
//		
