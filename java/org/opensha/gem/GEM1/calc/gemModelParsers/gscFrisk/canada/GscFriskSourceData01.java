...
//				if (srcIdx > 0) System.exit(0);
				...
				for (int geomIdx=0; geomIdx < src.geomNum; geomIdx++) {
					...
					// Create the region bordering the source area or the fault trace 
...
//						if (depIdx > 1) System.exit(0);
						...
						sumMfd = new SummedMagFreqDist(mminlg+MWDT/2,num,MWDT);
						double sumWei = 0.0;
...
//			    				if (nubIdx > 0) System.exit(0);
			    				...
			    				num = (int) Math.round((tmpMMax-mminlg)/MWDT);
...
//				    			System.out.println("Total Moment Rate: "+sumMfd.getTotalMomentRate());
						...
						// Converts the mfd (for Mlg) into mfd for moment magnitude
...
//						System.out.println("--> Mom rate:"+srcGem.getMagfreqDistFocMech().getMagFreqDist(0).getMomentRate(5));
						srclst.add(srcGem);
						System.out.printf("  Adding source %3d - Tot. rate: %7.4e (sumwei: %6.4f) avg depth: %6.2f \n",
								srclst.size(),mfdffm.getMagFreqDist(0).getTotalIncrRate(),sumWei,
