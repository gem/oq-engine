...
/**
 * <p><b>Title:</b> Turkey ERF<p>
 * 
 * <p><b>Description:</b> This class implements an ERF using the information parsed from the files 
 * that Koeri made available within the EMME project.</p>
 * 
 * @author 		M. Pagani
 * @version 	1.0
...
public class TurkeyEmmeSourceData extends GemFileParser  {
	 ...
	 * @param zoneGeom
	 */
	public TurkeyEmmeSourceData (TurkeyReadSourceData sourceData,TurkeyReadGMLFileSourceZones zoneGeom,
			TurkeyReadGMLFileFaults flt)  {		
		...
		if (useAreas) {
			...
			while (iter.hasNext()) {
				...
				// Info
				if (INFO) System.out.printf("Source: %d (name: %s)\n",srcCnt,zoneGeom.getName().get(srcCnt));
				...
				if (INFO) {
					System.out.printf("   mMin............................: %-5.2f \n",mMin);
					System.out.printf("   mMax............................: %-5.2f \n",mMax);
					System.out.printf("   aGR.............................: %-5.2f \n",aGR);
					System.out.printf("   bGR.............................: %-5.2f \n",bGR);
				}
	
				// Compute the total cumulative rate
				totCumRate = Math.pow(10,aGR-bGR*mMin)-Math.pow(10,aGR-bGR*mMax);
//				mNum = (int) Math.round((mMax-mMin)/mWdt+1);
				mNum = (int) Math.round( ((mMax-mWdt/2)-(mMin+mWdt/2))/mWdt+1);
				if (INFO) System.out.printf("   Tot Cum Rate.: %10.8f [ev/yr]\n",totCumRate);
				
				// Parameters: bGR, Total occurrence rate, mMin, mMax, Number of M extremes 
				GutenbergRichterMagFreqDist distGR = new GutenbergRichterMagFreqDist
				...
				ArbitrarilyDiscretizedFunc topRupDist = new ArbitrarilyDiscretizedFunc();
				double dm = (mMax-mMin+mWdt)/(mNum+1);
				double tmpM = mMin+mWdt/2;
				// NOTE: we assume top of rupture == 0 for all the magnitude (THIS IS A VERY 
				...
				// Get a source geometry (i.e. a LocationList) and create the evenly gridded region
...
//						ArbitrarilyDiscretizedFunc aveRupTopVsMag, double aveHypoDepth){
				...
				GEMAreaSourceData asrc = new GEMAreaSourceData(zoneGeom.getCode().get(srcCnt),
						zoneGeom.getCode().get(srcCnt),TectonicRegionType.ACTIVE_SHALLOW,
						rgg,mfdffm,topRupDist,5.0);
...
//			System.out.println(flt.getCoords().size()+" "+flt.getCode().size());
			...
			while (iterF.hasNext()) {
				...
				// Get a source geometry (i.e. a LocationList) and create the fault trace
				...
				double cumTotRate = Math.pow(10,aGR-bGR*mMin)-Math.pow(10,aGR-bGR*mMax);
				...
				// Parameters: bGR, Total occurrence rate, mMin, mMax, Number of M extremes 
				GutenbergRichterMagFreqDist magfrq = new GutenbergRichterMagFreqDist(bGR,cumTotRate,mMin,mMax,mNum);
						
				// NOTE by now we assume values of dip and rake ... they're undefined
				dip = 90.0;
				rake = 45.0;
				
				// Create a GEM fault source 
				GEMFaultSourceData fsrc = new GEMFaultSourceData(
						flt.getCode().get(cntF),
						flt.getName().get(cntF),
		...
		HashMap<String,Double> faultSystemLenght = new HashMap<String,Double>();
			...
			if (src instanceof GEMFaultSourceData){
				...
				if (faultSystemLenght.containsKey(fault.getID())){
					faultSystemLenght.put(src.getID(),
							faultSystemLenght.get(src.getID())+fault.getTrace().getTraceLength() );
				} else {
					faultSystemLenght.put(src.getID(),fault.getTrace().getTraceLength());
				}
			}
		}
		...
		for (GEMSourceData src: srcList){
			if (src instanceof GEMFaultSourceData){
				GEMFaultSourceData fault = (GEMFaultSourceData) src;
				double ratio = fault.getTrace().getTraceLength()/
...
//				srcList.set(idx,fault);
				...
				finalList.add(new GEMFaultSourceData(
						fault.getID(), 
						fault.getName(), 
						fault.getTectReg(), 
						mfd, 
