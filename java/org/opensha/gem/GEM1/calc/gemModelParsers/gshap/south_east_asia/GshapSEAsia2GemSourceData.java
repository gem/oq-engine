package org.opensha.gem.GEM1.calc.gemModelParsers.gshap.south_east_asia;
...
public class GshapSEAsia2GemSourceData extends GemFileParser {
	...
	//constructor
	public GshapSEAsia2GemSourceData() throws FileNotFoundException{
				
		    String inputfile = "../../data/gshap/south_east_asia/GshapSouthEastAsia.dat";
	        ...
	        try {
	        	...
	        	while((sRecord = oReader.readLine())!=null){
                    ...
                    if (minMag == 5.0) {
    					...
    					aVal = Math.log10(occRate/(Math.pow(10, -bVal*minMag)-Math.pow(10, -bVal*maxMag)));
                    }
                    else {
    					// compute aVal (GuttenberRichter law)
    					aVal = Math.log10(occRate/(Math.pow(10, -bVal*minMag)-Math.pow(10, -bVal*maxMag)));
					...
					double mminR;
					if(minMag!=maxMag){
						mminR = new BigDecimal(Math.round(minMag/dm)*dm+dm/2).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
						...
						mminR = new BigDecimal(Math.round(minMag/dm)*dm).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();	
					...
					// compute mfd setting the total cumulative rate (occRate defined
	    			...
	    			if (aveHypoDepth == 5) trt = trt1;		
	    			if (aveHypoDepth == 15) trt = trt2;
	    			if (aveHypoDepth == 80) trt = trt3;
...
//
	GshapSEAsia2GemSourceData seAsiaModel = new GshapSEAsia2GemSourceData();
	FileWriter file = new FileWriter("/Users/damianomonelli/Desktop/GshapSeaAsiaAreaSources.kml");
	seAsiaModel.writeAreaKMLfile(file);
	}
}
