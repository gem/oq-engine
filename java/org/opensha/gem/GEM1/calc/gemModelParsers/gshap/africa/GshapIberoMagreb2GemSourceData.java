...
/**
 * This parser reads the GSHAP input mode for Ibero-Magreb Region.   
 * The original input reports the following seismcity parameters:
 * earthquakes/year km2
 * b-val
 * Mmax 
 * @author l.danciu
 * 
...
public class GshapIberoMagreb2GemSourceData extends GemFileParser {
	...
	//constructor
	public GshapIberoMagreb2GemSourceData(String inputfile) throws FileNotFoundException{
		...
		// ArrayList of GEM area sources
	        ...
	        try {
	        	...
	        	while((sRecord = oReader.readLine())!=null){
					...
					st = new StringTokenizer(sRecord);
                   // bVal(GR); occRateN/year/km2; aValN - aVal normalized per year/km2, Maximum Magnitude
				   // aValN is used to recompute the occRate/year	
					...
					aGRNor = Math.log10(occRateNor/(Math.pow(10, -bGR*minMag)-Math.pow(10, -bGR*maxMag)));

//					System.out.println(" " + occRateNor + " " + bGR + " " + maxMag + " " +aGRNor);
...
//						System.out.println(lat+" "+lon);
					...
					GEMAreaSourceData srctmp = new GEMAreaSourceData(Integer.toString(srcIndex), sourceName, null, srcRegion, null, null,aveHypoDepth);
					...
					double occRate = Math.pow(10,aGR-bGR*(calcMinMag)) - Math.pow(10,aGR-bGR*((maxMag)));
...
//					System.out.println("occRate: " + occRate);
	    			...
	    			srcIndex = srcIndex+1;
					GEMAreaSourceData src = new GEMAreaSourceData(Integer.toString(srcIndex), sourceName, trt1, srcRegion, mfdffm, aveRupTopVsMag,aveHypoDepth);
...
//
//	IberoMagreb2GemSourceData IberoMagrebModel = new IberoMagreb2GemSourceData ("../../data/IberoMagreb/IberoMagrebInput.dat");
//	
