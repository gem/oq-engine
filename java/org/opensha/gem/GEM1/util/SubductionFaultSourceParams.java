...
public enum SubductionFaultSourceParams {
    ...
    AVERAGE_FAULT_GRID_SPACING(
            "Average fault discretization grid spacing (in km)"),
    ...
    public static SubductionFaultSourceParams getTypeForName(String name) {
        ...
        for (SubductionFaultSourceParams trt : SubductionFaultSourceParams
                .values()) {
            if (trt.name.equals(name))
                return trt;
        }
        throw new IllegalArgumentException(
                "Fault source parameter does not exist");
    }
