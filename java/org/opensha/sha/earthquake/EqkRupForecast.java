...
/**
 * <p>
 * Title: EqkRupForecast/p>
 * <p>
 * Description: Abstract class that provides the basic implementation for the
 * EqkRupForecast objects.
 * </p>
 * 
 * @author unascribed
 * @version 1.0
...
public abstract class EqkRupForecast implements EqkRupForecastAPI,
     ...
     * 
     * @return : Geographic region object specifying the applicable region of
     *         forecast
     */
    ...
    /**
     * This function returns the parameter with specified name from adjustable
     * param list
     * 
     * @param paramName
     *            : Name of the parameter needed from adjustable param list
     ...
     * 
     * @return : time span object is returned which contains start time and
     *         duration
     */
     ...
     * 
     * @param name
     *            String Name of the Adjustable Parameter
     * @param value
     *            Object Parameeter Value
    ...
    /**
     * Return the earhthquake source at index i. This methos returns the
     * reference to the class variable. So, when you call this method again,
     * result from previous method call is no longer valid. this is secret, fast
     * but dangerous method
     * 
     * @param i
    ...
    /**
     * Get the list of all earthquake sources. Clone is returned. So, list can
     * be save in ArrayList and this object subsequently destroyed
     * 
    ...
    /**
     * Get number of ruptures for source at index iSource This method iterates
     * through the list of 3 vectors for charA , charB and grB to find the the
     * element in the vector to which the source corresponds
     * 
     * @param iSource
     *            index of source whose ruptures need to be found
     */
     ...
     * method call is valid. This behavior is in contrast with getRupture(int
     * source, int i) method
     * 
     * @param source
     * @param i
     ...
     * reference to the class variable. So, when you call this method again,
     * result from previous method call is still valid. This behavior is in
     * contrast with the behavior of method getSource(int i)
     * 
     * @param iSource
     *            : index of the source needed
     * 
     * @return Returns the ProbEqkSource at index i
     * 
     *         FIX:FIX :: This function has not been implemented yet. Have to
     *         give a thought on that
     * 
     */
    ...
    public Element toXMLMetadata(Element root) {
        ...
        ListIterator paramIt =
                this.getAdjustableParameterList().getParametersIterator();
    ...
    public static EqkRupForecast fromXMLMetadata(Element root)
            throws InvocationTargetException {
        String className = root.attribute("className").getValue();
        System.out.println("Loading ERF: " + className);
        EqkRupForecast erf =
                (EqkRupForecast) MetadataLoader.createClassInstance(className);

        // add params
        System.out.println("Setting params...");
        Element paramsElement = root.element(Parameter.XML_GROUP_METADATA_NAME);
        ParameterList.setParamsInListFromXML(erf.getAdjustableParameterList(),
                paramsElement);
    ...
    /**
     * This draws a random event set. Non-poisson sources are not yet
     * implemented
     * 
     * @return
     */
