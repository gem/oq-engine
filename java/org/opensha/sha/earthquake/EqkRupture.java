...
public class EqkRupture implements java.io.Serializable {
    ...
    /**
     * This function doesn't create the ParameterList until the first attempt to
     * add a parameter is added. This is known as Lazy Instantiation, where the
     * class is not created until needed. This is a common performance
     * enhancement, because in general, not all aspects of a program are used
     * per user session.
     */
    public void addParameter(ParameterAPI parameter) {
        if (otherParams == null)
            otherParams = new ParameterList();
        if (!otherParams.containsParameter(parameter)) {
            otherParams.addParameter(parameter);
        } else {
            otherParams.updateParameter(parameter);
        }
    }
     ...
     * same as the ParameterList it is calling. People don't know that they have
     * been Added, this doesn't convey any more information than the short name
     * to me.
     */
    ...
    /**
     * Note: Since this takes a GriddedSurfaceAPI both a PointSurface and
     * GriddedSurface can be set here
     */
    ...
    public void setPointSurface(Location location, double aveStrike,
            double aveDip) {
        setPointSurface(location);
