 ...
 * </p>
 * <p>
 * Description: This makes a point source based on the inputs of the various
 * constructors:
 * </p>
 * </UL>
 * <p>
 * 
 * If an IncrementalMagFreqDist (or HypoMagFreqDistAtLoc) and duration have been
 * given, then the source is Poissonian and it is assumed that the duration
 * units are the same as those for the rates in the IncrementalMagFreqDist.
 * Also, magnitudes below the minimum are ignored, as are those with zero rates.
 * If magnitude/probability have been given, the source has only one rupture and
 * is not Poissonian.
 * </p>
 * 
 * @author Edward Field
...
public class PointEqkSource extends ProbEqkSource implements
    ...
    /**
     * Constructor specifying the Location, the IncrementalMagFreqDist, the
     ...
     * ingored). The source is set as Poissonian with this constructor.
     * 
     */
    public PointEqkSource(Location loc, IncrementalMagFreqDist magFreqDist,
            double duration, double aveRake, double aveDip, double minMag) {
        this.location = loc;
        ...
        /*
         * if( D )
         * System.out.println("PointEqkSource Constructor: totNumRups="+magsAndRates
         * .getNum()+
    ...
    /**
     * Constructor specifying the location, the IncrementalMagFreqDist, the
     * duration, the average rake, and the dip. This sets minMag to zero
     * (magnitudes from magFreqDist below are ignored in making the source). The
     * source is set as Poissonian with this constructor.
     * 
     */
    public PointEqkSource(Location loc, IncrementalMagFreqDist magFreqDist,
            double duration, double aveRake, double aveDip) {
     ...
     * cloned here and the depth is overwritten). This sets the source as
     * Poissonian
     */
    ...
    /**
     * Constructor specifying the location, a magnitude and probability, the
     * average rake, and the dip. The source is set as Poissonian with this
     * constructor.
     * 
     */
    ...
    public EvenlyGriddedSurfaceAPI getSourceSurface() {
        return probEqkRupture.getRuptureSurface();
    }
    ...
    private void setAll(HypoMagFreqDistAtLoc hypoMagFreqDistAtLoc) {
        ...
        IncrementalMagFreqDist[] magFreqDists =
                hypoMagFreqDistAtLoc.getMagFreqDistList();
        FocalMechanism[] focalMechanisms =
                hypoMagFreqDistAtLoc.getFocalMechanismList();
    ...
    private void setMagsAndRates(IncrementalMagFreqDist magFreqDist) {
        ...
        // if (D)
        // System.out.println(C+" numNonZeroMagDistPoints="+magsAndRates.getNum());
    }
    ...
    public ProbEqkRupture getRupture(int nthRupture) {
        ...
        // set)
        if (isPoissonian)
            probEqkRupture.setProbability(1 - Math.exp(-duration
                    * ((Double) rates.get(nthRupture)).doubleValue()));
        ...
        if (variableDepthRakeAndDip) {
            ...
            probEqkRupture.setPointSurface(location, dips.get(nthRupture)
                    .doubleValue());
        }
    ...
    public void setLocation(Location loc) {
        if (!variableDepthRakeAndDip) {
            ...
            throw new RuntimeException(
                    C
                            + "-- Error - can't set Location when variableDepthRakeAndDip = true");
    }
