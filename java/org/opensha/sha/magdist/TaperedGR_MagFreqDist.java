...
public class TaperedGR_MagFreqDist extends IncrementalMagFreqDist {
    ...
    public static String NAME = new String("Tapered GR Dist"); // for showing
                                                               // messages
    ...
    public TaperedGR_MagFreqDist(double min, double max, int num)
    ...
    public TaperedGR_MagFreqDist(double bValue, double totCumRate, double min,
            double max, int num) throws DiscretizedFuncException,
    ...
    public TaperedGR_MagFreqDist(double min, int num, double delta,
            double magLower, double magCorner, double totMoRate, double bValue)
    ...
    /**
     * Set All but magCorner. This finds the corner magnitude iteratively, such
     * that the final corner magnitude is guaranteed to be within 0.0001 of the
     * "true" corner magnitude (and accordingly, there is a slight discrepancy
     * in the final moment rate as well - typically orig/final moment rate =
     * 1.0001). This throws a runtime exception if it cannot find a corner
     * magnitude between magLower and maxX+0.0001 that satisfies the totMoRate.
     * TO DO: FIX EXCEPTION THROWING TO BE CONSISTENT WITH OTHER METHODS AND THE
     * MFD PARAMETER CLASS
     * 
     * @param magLower
     *            : lowest magnitude that has non zero rate
    ...
    public void setAllButCornerMag(double magLower, double totMoRate,
            double totCumRate, double bValue) throws MagFreqDistException,
            DiscretizedFuncException, DataPoint2DException {
                ...
                if (getTotalMomentRate() > totMoRate) {
                    ...
                    if (magStart < magLower)
                        throw new RuntimeException(
                                this.NAME
                                        + ": Error - could not find corner magnitude that satisfies the moment rate (magLower too high?).");
                    deltaMag /= 10.0;
                    break;
                }
            }
        ...
        boolean success =
                (getTotalMomentRate() >= totMoRate && moRateBelow < totMoRate);
        if (!success)
            throw new RuntimeException(
                    this.NAME
                            + ": Error - could not find corner magnitude that satisfies the moment rate (maxX too low?).");
        ...
        if (D) {
            ...
            System.out.println("Final/Orig totMoRate = " + getTotalMomentRate()
                    / totMoRate + " (should be just greater than one)");
    ...
    private void calculateRelativeRates() throws DataPoint2DException {
        ...
        // make an array with the cumulative distribution values (offset by
        // delta/2, with one additional point)
        double[] cumRate = new double[num + 1];
        for (i = 0; i < cumRate.length; ++i) {
            double mag = i * delta + minX - delta / 2.0;
            cumRate[i] =
                    Math.pow(10, -bValue * mag)
                            * Math.exp(-Math.pow(10, 1.5 * (mag - magCorner)));
        }
    ...
    public String getDefaultInfo() throws DataPoint2DException {
        return ("minMag=" + minX + "; maxMag=" + maxX + "; numMag=" + num
                ...
                + "; magCorner=" + (float) magCorner + "; totMoRate="
     ...
     * num, delta); f.setAllButTotMoRate(this.magLower, this.magCorner,
