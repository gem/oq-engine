...
/**
 * <p>
 * Title:SummedMagFreqDist.java
 * </p>
 * <p>
 * Description: This class is for summing the various Magnitude Frequency
 * distributions
 * </p>
 * 
...
public class SummedMagFreqDist extends IncrementalMagFreqDist {
    ...
    private boolean saveMagFreqDists = false; // whether you want to store each
                                              // distribution
    private boolean saveAllInfo = false; // whether you want to save info for
                                         // each distribution
    private DiscretizedFuncList savedMagFreqDists; // to save the each
                                                   // distribution
     ...
     * @param num
     * @param delta
     * @param saveMagFreqDist
     *            : whether you want to store each distribution
     * @param saveAllInfo
     *            : whether you want to save info for each distribution using
     *            the parameters we call the parent class constructors to
     *            initialise the parent class variables
     */
     ...
     * @param saveAllInfo
     *            : whether you want to save info for each distribution using
     *            the min, max and num we calculate the delta
     */
    ...
    /**
     * this function adds the new magnitude frequency distribution the min, num
     * and delta(or max) of new distribution should match min, max and num as
     * specified in the constructor
     * 
     * @param magFreqDist
     *            new Magnitude Frequency distribution to be added
     */
    ...
    public void addIncrementalMagFreqDist(IncrementalMagFreqDist magFreqDist)
        ...
        /*
         * check whether mun,num and delta of new distribution matches the min,
         * num and delta in the constructor
         */
        ...
        for (int i = 0; i < num; ++i)
            // add the y values from this new distribution
    ...
    /**
     * This asusmes the function passed in as a MFD, and adds the rate for each
     * x-axis value to the closest x-axis value in this MFD (ignoring magnitudes
     * that are above and below the range here). If the preserveRates boolean is
     * false, then the moment rate of each point is preserved (although the
    ...
    public void addResampledMagFreqDist(DiscretizedFuncAPI func,
    ...
    /**
     * This adds the rate & mag passed in to the MFD after rounding to the
     ...
     * SummedMagFreqDist class and calling the set functions of this from
     * outside
     * 
     * @param point
    ...
    public void set(DataPoint2D point) throws MagFreqDistException {
        throw new MagFreqDistException(
                "Cannot Access the set function of the SummedMagFreqDist from outside this class");
    }
    ...
    public void
            removeIncrementalMagFreqDist(IncrementalMagFreqDist magFreqDist)
                    throws DiscretizedFuncException, DataPoint2DException {
        ...
        if (saveMagFreqDists) { // check if this distribution exists
            ...
            if (index == -1) // if this distribution is not found in the list
                throw new DiscretizedFuncException(
                        "this distribution does not exist");
            else
                // remove the distribution if it is found
        ...
        for (int i = 0; i < num; ++i)
            // remove the rates associated with the removed distribution
            super.set(i, this.getY(i) - magFreqDist.getY(i));
    }

    /**
     * 
     * @return : returns the vector of Strings of Info about each added
     *         distribution
     */
    ...
    public ArrayList getAllInfo() {
        ...
        if (saveMagFreqDists) {
            ...
            // if distribution info is not saved
            return null;
    }
