...
/**
 * <b>Title:</b> GriddedSubsetSurface
 * <p>
 * 
 ...
 * Gridded Surface actually stores the data points.
 * <p>
 * 
 ...
 * Think of this as a "ZOOM" function into a GriddedSurface.
 * <p>
 * 
 * <b>Note:</b> SetLocation, setAveStrike, setAveDip have been disabled, this
 * class is read-only into the dataset.
 * <p>
 * 
 * @see Window2D
 ...
 * @version 1.0
 */
public class GriddedSubsetSurface extends ContainerSubset2D<Location> implements
     ...
     * @param startRow
     *            Start row into the main GriddedSurface.
     * @param startCol
     *            Start column into the main GriddedSurface.
     * @exception ArrayIndexOutOfBoundsException
     *                Thrown if window indexes exceed the main GriddedSurface
     *                indexes.
     */
    public GriddedSubsetSurface(int numRows, int numCols, int startRow,
     ...
     *            Start column into the main GriddedSurface.
     * @param data
     *            The main GriddedSurface this is a window into
     * @exception ArrayIndexOutOfBoundsException
     *                Thrown if window indexes exceed the main GriddedSurface
     *                indexes.
     */
    public GriddedSubsetSurface(int numRows, int numCols, int startRow,
            int startCol, EvenlyGriddedSurfaceAPI data)
    ...
    /**
     * Resizes the window view into the main GriddedSurface data.
     * <p>
     * 
     * Note: This function uses some advanced features of a transactional
     * nature. A transaction is basically a series of steps that must follow
     * each other. If any of the steps fails, the previous steps must be rolled
     * back. I perform this by calling initTransaction(), rollback() if error,
     * else commit(). It sounds more complicated than it it. This approach
     * basically resets the window size to the starting size if any of the new
     * indexes fail. Each is checked one at a time.
     * 
     * @param startRow
     *            The Start row into the main GriddedSurface.
     * @param startCol
     *            Start column into the main GriddedSurface.
     * @param numRows
     *            The new length of the window.
     * @param numCols
     ...
     *                indexes.
     */
    public void setLimits(int startRow, int startCol, int numRows, int numCols)
            throws ArrayIndexOutOfBoundsException {
    ...
    /**
     * Replaces the real data of the GriddedSurface with a new surface.
     * <p>
     * 
     * Note: This could be a dangerous thing to do if the indexes are invalid
     * for the new surface. I am not sure if this is being checked. Please
     * consult the Container2D documentation for further information.
     * 
     * @param gs
     *            The new newMainSurface value
     */
    ...
    /**
     * Proxy method that returns the aveStrike of the main GriddedSurface.
     * <P>
     * 
     * SWR: Note - should we be returning the main GriddedSurface ave strike, or
     * the ave. strike for the subsurface, which may be different from the main
     * surface.
     * 
    ...
    public String toString() {
        ...
        if (data != null)
            b.append("Ave. Strike = "
                    + ((EvenlyGriddedSurface) data).getAveStrike() + '\n');
        if (data != null)
            b.append("Ave. Dip = " + ((EvenlyGriddedSurface) data).getAveDip()
                    + '\n');
