...
public interface IntensityMeasureRelationshipAPI extends NamedObjectAPI,
    ...
    /**
     * This sets the Site, EqkRupture, and perhaps other things from the
     * propEffect object passed in (implementation details will vary according
     ...
     * user-specified probability. This value is not what is returned by the
     * getExceedProbability() method, as the latter is what is computed for a
     * specified given IML. It's important to understand this distinction.
     * 
     * @param prob
     * 
     ...
     * value in the Intensity-Measure Parameter) will be exceeded. This function
     * does not set or return the value in the exceedProbParam, as the latter is
     * for for computing the IML at a user-supplied prob.
     * 
    ...
    /**
     * Returns an ParameterList of all other parameters. Other parameters are
     * those that the exceedance probability depends upon, but that are not a
     * supported IMT (or one of their independent parameters) and are not
     * contained in, or computed from, the site or eqkRutpure objects. Note that
     * this does not include the exceedProbParam (which exceedance probability
     * does not depend on).
