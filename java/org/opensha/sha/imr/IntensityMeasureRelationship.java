...
/**
 * <b>Title:</b> IntensityMeasureRelationship
 * <p>
 * 
 * <b>Description:</b> Abstract base class for Intensity Measure Relationship
 * (IMR). All IMRs compute the probability of exceeding a particular shaking
 * level (specified by an intensity-measure Parameter) given a Site and
 * EqkRupture object. Subclasses will implement specific types of IMRs (e.g.,
 * AttenuationRelationship). This abstract IMR class also contains separate
 * parameterList objects for the site, earthquake rupture, and
 * propagation-effect related parameters, as well as a list of "other"
 * parameters that don't fit into those three categories. This class also
 * contains a list of supported intensity-measure parameters (which may have
 * internal independent parameters). These five lists combined (siteParams,
 * EqkRuptureParams, propagationEffectParams, supportedIMParams, and
 * otherParams) constitutes the complete list of parameters that the exceedance
 * probability depends upon. The only other parameter is exceedProbParam, which
 * is used to compute the IML at a particular probability in subclasses that
 * support the getIML_AtExceedProb() method.
 * <p>
 * 
...
public abstract class IntensityMeasureRelationship implements
    ...
    public final static String XML_METADATA_SITE_PARAMETERS_NAME =
            "SiteParameters";
    ...
    /**
     * Exceed Prob parameter, used only to store the exceedance probability to
     * be used by the getIML_AtExceedProb() method of subclasses (if a subclass
     * supports this method). Note that calling the getExceedProbability() does
     * not store the value in this parameter.
     */
     ...
     * These are any parameters that the exceedance probability depends upon
     * that is not a supported IMT (or one of their independent parameters) and
     * is not contained in, or computed from, the site or eqkRutpure objects.
     * Note that this does not include the exceedProbParam (which exceedance
     * probability does not depend on).
     */
    protected ParameterList otherParams = new ParameterList();

    /**
     * The current Site object (passing one in will set site-related
     * parameters).
     */
    protected Site site;
    ...
    /**
     * Intensity Measure. This is a specification of the type of shaking one is
     * concerned about. Its representation as a Parameter makes the
     * specification quite general and flexible. IMRs compute the probability of
     * exceeding the "value" field of this im Parameter.
     */
    ...
    public IntensityMeasureRelationship() {
        exceedProbParam =
                new DoubleParameter(EXCEED_PROB_NAME, EXCEED_PROB_MIN,
    ...
    /**
     * This sets the Site & EqkRupture (and subclasses can set propagation
     * effect params more efficiently by overriding this).
     * 
     * @param propEffect
     */
    ...
    /**
     * Sets the EqkRupture object in the IMR as a reference to the one passed
     * in.
     * 
     * @param eqkRupture
     *            The new EqkRupture object
     */
    ...
    /**
     * Sets the value of the currently chosen Intensity-Measure Parameter. This
     * is the value that the probability of exceedance is computed for.
     * 
     * @param iml
     *            The new value for the intensityMeasure Parameter
     */
    ...
    /**
     * Gets a reference to the currently chosen Intensity-Measure Parameter from
     * the IMR.
     * 
     ...
     * but by finding and setting the internally held one with the same name,
     * and also setting the values of any of its independent parameters to be
     * equal to the associated values of those passed in. Note that the IML is
     * not set (the value of the selected IM is not set as the value of that
     * passed in). The latter could be changed if anyone so desires.
     * 
     * @param intensityMeasure
     *            The new intensityMeasure Parameter
     */
    public void setIntensityMeasure(ParameterAPI intensityMeasure)
        ...
        if (isIntensityMeasureSupported(intensityMeasure)) {
            ...
            ListIterator it =
                    ((DependentParameterAPI) intensityMeasure)
     ...
     * 
     * @param intensityMeasure
     *            The new intensityMeasureParameter name
     */
    ...
    /**
     * This sets the probability to be used for getting the IML at a
     * user-specified probability. This value is stored in exceedProbParam. This
     * value is not what is returned by the getExceedProbability() method, as
     * the latter is what is computed for a specified given IML. It's important
     ...
     * supported IMs, but not checking whether the value (IML) of that passed in
     * is supported (this could be changed is anyone so desires). The name and
     * value of all independent parameters associated with the intensity measure
     * are also checked.
     * 
     * @param intensityMeasure
    ...
    public boolean isIntensityMeasureSupported(ParameterAPI intensityMeasure) {
        ...
        if (supportedIMParams.containsParameter(intensityMeasure)) {
            ...
            int numIndParams =
                    ((DependentParameterAPI) supportedIMParams
                            .getParameter(intensityMeasure.getName()))
                            .getNumIndependentParameters();
            // ParameterAPI param = supportedIMParams.getParameter(
            // intensityMeasure.getName() );
            ListIterator it =
                    ((DependentParameterAPI) intensityMeasure)
                            .getIndependentParametersIterator();
            ...
            // now check to make sure both have the same number of independent
            // params
            if (((DependentParameterAPI) intensityMeasure)
                    .getNumIndependentParameters() == numIndParams)
                return true;
            else
                return false;
        } else {
    ...
    /**
     * Checks if the Parameter is a supported intensity-Measure (checking only
     * the name).
     * 
     * @param intensityMeasure
     *            Name of the intensity Measure parameter
     * @return
     */
    ...
    /**
     * Sets the eqkRupture, site, and intensityMeasure objects simultaneously.
     * <p>
     * 
     ...
     * i.e. the transaction is not handled gracefully.
     * <p>
     * 
     * This will take alot of design and work so it is held off for now until it
     * is decided that it is needed.
     * <p>
     * 
     * 
     * 
    ...
    public void setAll(EqkRupture eqkRupture, Site site,
            ParameterAPI intensityMeasure) throws ParameterException,
    ...
    /**
     * Returns a pointer to a parameter if it exists in one of the parameter
     * lists
     * 
     * @param name
     *            Parameter key for lookup
     * @return The found parameter
     * @throws ParameterException
     *             If parameter with that name doesn't exist
     */
    public ParameterAPI getParameter(String name) throws ParameterException {
        ...
        ListIterator<ParameterAPI<?>> it =
                supportedIMParams.getParametersIterator();
        while (it.hasNext()) {
     ...
     * supported IMT (or one of their independent parameters) and are not
     * contained in, or computed from, the site or eqkRutpure objects. Note that
     * this does not include the exceedProbParam (which exceedance probability
     * does not depend on).
     * 
    ...
    /**
     * Returns the iterator over all Propagation-Effect related parameters A
     * Propagation-Effect related parameter is any parameter for which the value
     * can be compute from a Site and eqkRupture object.
     * 
    ...
    /**
     * Returns the iterator over all supported Intensity-Measure Parameters.
     * 
    ...
    /**
     * Returns a list of all supported Intensity-Measure Parameters.
     * 
     * @return The Supported Intensity-Measures Iterator
     */
    ...
    public Element toXMLMetadata(Element root) {
        ...
        Element siteParamsElement =
                xml.addElement(IntensityMeasureRelationship.XML_METADATA_SITE_PARAMETERS_NAME);
        while (paramIt.hasNext()) {
            Parameter param = (Parameter) paramIt.next();
        ...
        if (imt != null)
            imt.toXMLMetadata(xml,
                    IntensityMeasureRelationship.XML_METADATA_IMT_NAME);
        // ParameterAPI period = this.getParameter(PeriodParam.NAME);
        // if (period != null)
        // imtElem.addAttribute(PeriodParam.NAME.replaceAll(" ", ""),
        // period.getValue().toString());
        // ParameterAPI damping = this.getParameter(DampingParam.NAME);
        // if (damping != null)
        // imtElem.addAttribute(DampingParam.NAME.replaceAll(" ", ""),
        // damping.getValue().toString());
        return root;
    }

    public static IntensityMeasureRelationship fromXMLMetadata(Element root,
        ...
        IntensityMeasureRelationship imr =
                (IntensityMeasureRelationship) MetadataLoader
                        .createClassInstance(className, args, argNames);
        ...
        Element siteParamsElement =
                root.element(IntensityMeasureRelationship.XML_METADATA_SITE_PARAMETERS_NAME);
        ...
        // set IMT
        Element imtElem =
                root.element(IntensityMeasureRelationship.XML_METADATA_IMT_NAME);
        if (imtElem != null) {
        ...
        //
        // Attribute damping =
        // imtElem.attribute(DampingParam.NAME.replaceAll(" ", ""));
        // if (damping != null) {
