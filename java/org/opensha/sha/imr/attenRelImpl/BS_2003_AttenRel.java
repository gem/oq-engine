...
/**
 * <b>Title:</b> BS_2003_AttenRel
 * <p>
 * 
 * <b>Description:</b> This implements the site effect models developed by
 * Bazzuro and Cornell(2004), Baturay and Stewart(2003), applied to the
 * Abrahamson & Silva (1997) rock-site predictions.
 * <p>
 * 
...
public class BS_2003_AttenRel extends AttenuationRelationship implements
    ...
    private DoubleConstraint AF_InterceptparamConstraint =
            new DoubleConstraint(-2, 2);
    ...
    private DoubleConstraint AF_slopeParamConstraint = new DoubleConstraint(-1,
            1);
    ...
    public final static String AF_ADDITIVE_REF_ACCELERATION_PARAM_NAME =
            "AF Add. Ref. Acceleration";
    ...
    private DoubleConstraint AFaddRefAccParamConstraint = new DoubleConstraint(
            0, 0.5);
    ...
    public final static String AF_STD_DEV_PARAM_INFO =
            "Standard Deviation of the amplification factor from the ground response analyses"
                    + " regression model";
    private DoubleConstraint AF_StdDevParamConstraint = new DoubleConstraint(0,
            1.0);
    ...
    public final static String NUM_RUNS_PARAM_INFO =
            "Number of runs of the wave propagation"
                    + " simulation for the site";
    private IntegerConstraint numRunsConstraint = new IntegerConstraint(1,
            Integer.MAX_VALUE);
    ...
    public void setSite(Site site) throws ParameterException {
        ...
        AF_InterceptParam.setValue((Double) site.getParameter(
                AF_INTERCEPT_PARAM_NAME).getValue());
        AF_AddRefAccParam.setValue((Double) site.getParameter(
                AF_ADDITIVE_REF_ACCELERATION_PARAM_NAME).getValue());
        AF_SlopeParam.setValue((Double) site.getParameter(AF_SLOPE_PARAM_NAME)
                .getValue());
        AF_StdDevParam.setValue((Double) site.getParameter(
                AF_STD_DEV_PARAM_NAME).getValue());
        ...
        softSoilParam.setValue((Boolean) (site.getParameter(SOFT_SOIL_NAME)
                .getValue()));
        numRunsParam.setValue((Integer) site.getParameter(NUM_RUNS_PARAM_NAME)
                .getValue());
        this.site = site;
    ...
    protected void updateCoefficients() throws ParameterException {
        ...
        if (horzCoeffs.containsKey(key.toString())) {
            coeffs =
                    (BS_2003_AttenRelCoefficients) horzCoeffs.get(key
                            .toString());
        } else {
    ...
    public double getStdDev() {
        ...
        if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_NONE)) { // "None (zero)"
            ...
            if (im.getName().equals(SA_Param.NAME))
                periodParamVal =
                        (float) ((Double) saPeriodParam.getValue())
                                .doubleValue();
            else
                periodParamVal = 0;
            ...
            if (periodParamVal < 0.75)
                ...
                saPeriodParam
                        .setValue(new Double(format.format(periodParamVal)));
                // linear interpolation to get the Std Dev.
                double stdDev =
                        ((periodParamVal - 0.75) / (1.5 - 0.75))
    ...
    /**
     * @return The stdDev value for Baturay and Stewart (2003) Site Correction
     *         Model
     */
    private double getStdDevForBS() {
        ...
        boolean softSoilCase =
                ((Boolean) this.softSoilParam.getValue()).booleanValue();
        ...
        if (vs30 <= 180 || softSoilCase)
            ...
            stdDev_gNet = 0.56;
        double stdDev =
                Math.pow(stdDev_gNet, 2) + Math.pow(stdError, 2) + coeffs.tau
                        * coeffs.tau;
    ...
    protected void initSiteParams() {
        ...
        AF_InterceptParam =
                new DoubleParameter(this.AF_INTERCEPT_PARAM_NAME,
                        AF_InterceptparamConstraint,
                        this.AF_INTERCEPT_PARAM_DEFAULT);
        AF_InterceptParam.setInfo(this.AF_INTERCEPT_PARAM_INFO);

        // make the AF slope parameter
        this.AF_SlopeParam =
                new DoubleParameter(this.AF_SLOPE_PARAM_NAME,
                        this.AF_slopeParamConstraint,
                        this.AF_SLOPE_PARAM_DEFAULT);
        AF_SlopeParam.setInfo(this.AF_SLOPE_PARAM_INFO);

        // make theb AF Additive Reference Paramerter
        this.AF_AddRefAccParam =
                new DoubleParameter(
                        this.AF_ADDITIVE_REF_ACCELERATION_PARAM_NAME,
                        this.AFaddRefAccParamConstraint,
                        this.AF_ADDITIVE_REF_ACCERLATION_DEFAULT);
        AF_AddRefAccParam.setInfo(this.AF_ADDITIVE_REF_ACCELERATION_PARAM_INFO);

        // make the AF Std. Dev.
        this.AF_StdDevParam =
                new DoubleParameter(this.AF_STD_DEV_PARAM_NAME,
        ...
        this.numRunsParam =
                new IntegerParameter(this.NUM_RUNS_PARAM_NAME,
    ...
    class BS_2003_AttenRelCoefficients implements NamedObjectAPI {
        ...
        /**
         * Constructor for the BS_2003_AttenRelCoefficients object that sets all
         * values at once
         * 
         * @param name
