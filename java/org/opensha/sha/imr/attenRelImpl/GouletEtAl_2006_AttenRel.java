...
public class GouletEtAl_2006_AttenRel extends BC_2004_AttenRel {
    ...
    public GouletEtAl_2006_AttenRel(
    ...
    private double getStdDevForGoulet() {
        ...
        double cVal =
                ((Double) this.AF_AddRefAccParam.getValue()).doubleValue();
        double stdDevAF =
                ((Double) this.AF_StdDevParam.getValue()).doubleValue();
        ...
        // double stdDev =
        // Math.pow((bVal*Math.exp(asRockMean))/(Math.exp(asRockMean)+cVal)+1,
        // 2)*
        // (Math.pow(Math.exp(asRockStdDev),2)-Math.pow(tau,
        // 2))+Math.pow(stdDevAF,2)+Math.pow(tau,2);
        double stdDev =
                Math.pow((bVal * Math.exp(asRockMean))
                        / (Math.exp(asRockMean) + cVal) + 1, 2)
