...
public class AS_2008_AttenRel extends AttenuationRelationship implements
	...
	private void readCoeffFile() {
		try {
			...
			ArrayList<String> coeff = FileUtils.loadFile(this.getClass()
					.getResource(AS_2008_CoeffFile));
			// reading the Period
	...
	public double getMean() {
		...
		if (per[iper] < Td || (Td >= 10.0 && iTd == 22)) {
			mean = (getMean(iper, 0, vs30, rRup, rJB, f_as, rX, f_rv, f_nm,
					mag, dip, rupWidth, depthTop, pga_rock)) + f10;
			// System.out.println("From getMean, if(per<Td), mean = "+
			// Math.exp(mean));

		} else {
			double medSa1100WithTdMinus = Math.exp(getMean(iTd, 0, 1100.0,
					rRup, rJB, f_as, rX, f_rv, f_nm, mag, dip, rupWidth,
					depthTop, pga_rock));

			double medSa1100WithTdPlus = Math.exp(getMean(iTd + 1, 0, 1100.0,
					rRup, rJB, f_as, rX, f_rv, f_nm, mag, dip, rupWidth,
					depthTop, pga_rock));
			...
			double medSa1100AtTd0 = Math.exp(Math.log(medSa1100WithTdPlus
					/ medSa1100WithTdMinus)
					/ Math.log(per[iTd + 1] / per[iTd])
					* Math.log(Math.pow(10, -1.25 + 0.3 * mag) / per[iTd])
					+ Math.log(medSa1100WithTdMinus));
			double mean1100AtTd = (medSa1100AtTd0)
					* Math.pow(Math.pow(10, -1.25 + 0.3 * mag) / per[iper], 2);
	...
	private void computeRockPGA(double rJB, double rX) {
		if (rock_pga_is_not_fresh) {
			pga_rock = Math.exp(getMean(1, 0, 1100.0, rRup, rJB, f_as, rX,
					f_rv, f_nm, mag, dip, rupWidth, depthTop, 0.0));
	...
	protected void initSiteParams() {
		...
		depthTo1pt0kmPerSecParam = new DepthTo1pt0kmPerSecParam(
				DEPTH_1pt0_WARN_MIN, DEPTH_1pt0_WARN_MAX);
	...
	protected void initEqkRuptureParams() {
		...
		rupTopDepthParam = new RupTopDepthParam(RUP_TOP_WARN_MIN,
				RUP_TOP_WARN_MAX);
		rupWidthParam = new RupWidthParam(RUP_WIDTH_WARN_MIN,
				RUP_WIDTH_WARN_MAX);
	...
	protected void initPropagationEffectParams() {
		...
		DoubleConstraint warnJB = new DoubleConstraint(DISTANCE_MINUS_WARN_MIN,
	...
	private double getMean(int iper, int iTd, double vs30, double rRup,
		...
		if (mag <= c1) {
			f1 = a1[iper] + a4 * (mag - c1) + a8[iper] * Math.pow(8.5 - mag, 2)
					+ (a2[iper] + a3 * (mag - c1)) * Math.log(rR);
		} else {
			f1 = a1[iper] + a5 * (mag - c1) + a8[iper] * Math.pow(8.5 - mag, 2)
					+ (a2[iper] + a3 * (mag - c1)) * Math.log(rR);
		}
		...
		double cgMean = f1 + a12[iper] * f_rv + a13[iper] * f_nm + a15[iper]
				* f_as + f4 + f5 + f6 + f8;
	...
	public double getStdDev(int iper, String stdDevType, String component,
		...
		if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_NONE))
			...
			sigmaBPGA = Math.sqrt(Math.pow(sigma0PGA, 2)
					- Math.pow(sigmaamp, 2));
			...
			sigma = Math.sqrt(Math.pow(sigmaB, 2) + Math.pow(sigmaamp, 2)
					+ Math.pow(dterm, 2) * Math.pow(sigmaBPGA, 2) + 2.0 * dterm
					* sigmaB * sigmaBPGA * rho[iper]);

			// get tau - inter-event term (Eq. 25)
			tau = Math.sqrt(Math.pow(tau0, 2) + Math.pow(dterm, 2)
					* Math.pow(tauBPGA, 2) + 2.0 * dterm * tauB * tauBPGA
					* rho[iper]);
