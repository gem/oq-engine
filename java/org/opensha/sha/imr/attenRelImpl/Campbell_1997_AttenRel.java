...
/**
 * <b>Title:</b> Campbell_1997_AttenRel
 * <p>
 * 
 * <b>Description:</b> This implements the Attenuation Relationship developed by
 * Campbell (1997, Seismological Research Letters, vol 68, num 1, pp 154-179),
 * as corrected in the Erratum (Campbell, 2000, Seismological Research Letters,
 * vol 71, num 3, pp 352-354)
 * <p>
 * 
...
public class Campbell_1997_AttenRel extends AttenuationRelationship implements
    ...
    /**
     * Campbell's Basin-Depth Parameter, defined as depth (km) to Cretaceous,
     * crystalline basement, 5 km/sec P-wave velicity, or 3 km/sec S-wave
     * velocity.
     */
     ...
     * Options are "Reverse" if 157.5>rake>22.5 or "Other" otherwise.
     * 
     * @param rake
     *            in degrees
    ...
    protected void setFaultTypeFromRake(double rake)
        ...
        // NOTE: FLT_TYPE_UNKNOWN is not possible unless rake can == null, which
        // it can't
     ...
     * 
     * @param site
     *            The new site object which contains the "Campbell Site Type"
     *            and "Campbell Basin Depth" Parameters
    ...
    public void setSite(Site site) throws ParameterException {
        ...
        siteTypeParam.setValue((String) site.getParameter(SITE_TYPE_NAME)
                .getValue());
        basinDepthParam.setValueIgnoreWarning((Double) site.getParameter(
                BASIN_DEPTH_NAME).getValue());
        this.site = site;
        setPropagationEffectParams();
     ...
     * @param propEffect
     * @throws ParameterException
     *             : Thrown if the Site object doesn't contain
    ...
    /**
     * This sets the DistanceSeis propagation effect parameter based on the
    ...
    protected void updateCoefficients() throws ParameterException {
        ...
        // Only SA coefficients here (PGA and PGV are hard coded because they're
        // different)
        StringBuffer key =
                new StringBuffer(im.getName() + "/" + saPeriodParam.getValue());
        if (coefficients.containsKey(key.toString())) {
            coeff =
                    (Campbell_1997_AttenRelCoefficients) coefficients.get(key
                            .toString());
        } else {
    ...
    public double getMean() throws IMRException {
            ...
            if (component.equals(ComponentParam.COMPONENT_AVE_HORZ)) {
                ...
                mean =
                        lnPGA
                                ...
                                * mag
                                - 1.5
                                * Math.log(dist + 0.079 * Math.exp(0.661 * mag))
                                + 1.89
                                * Math.log(dist + 0.361 * Math.exp(0.576 * mag))
                                - 0.11 * F;
            }
        }
        ...
        // Now do SA:
            ...
            // check whether it's the zero period case (for which there are not
            // coeffs)
                    ...
                    mean =
                            lnPGA
                                    ...
                                    * Math.log(dist + 0.079
                                    ...
                                    + 1.89
                                    * Math.log(dist + 0.361
                                            * Math.exp(0.576 * mag)) - 0.11 * F;
                }
            } else {
                // compute Horizontal case:
                updateCoefficients();
                mean =
                        lnPGA + coeff.c1_h + coeff.c2_h
                                * tanh(coeff.c3_h * (mag - 4.7))
                                + (coeff.c4_h + coeff.c5_h * mag) * dist + 0.5
                                * coeff.c6 * S_sr + coeff.c6 * S_hr + coeff.c7
                                * tanh(coeff.c8 * depth) * (1 - S_hr);
                if (depth < 1.) {
                    mean +=
                            coeff.c6 * (1.0 - depth) * (1.0 - 0.5 * S_sr)
                                    * (1.0 - S_hr);
                }
                    ...
                    mean +=
                            coeff.c1_v
                                    ...
                                    * Math.log(dist + 0.361
                                    ...
                                    - 0.11
                                    * F
                                    + (coeff.c4_v * tanh(0.51 * depth) + coeff.c5_v
                                            * tanh(0.57 * depth)) * (1 - S_hr);
                }
            }
        }
        ...
        // Now do PGV:
            ...
            // compute horizontal case
            mean =
                    lnPGA + 0.26 + 0.29 * mag - 1.44
                            * Math.log(dist + 0.0203 * Math.exp(0.958 * mag))
                            + 1.89
                            * Math.log(dist + 0.361 * Math.exp(0.576 * mag))
                            + (0.0001 - 0.000565 * mag) * dist - 0.12 * F
                            - 0.15 * S_sr - 0.30 * S_hr + 0.75
                            * tanh(0.51 * depth) * (1 - S_hr);
            if (depth < 1.) {
            ...
            if (component.equals(ComponentParam.COMPONENT_VERT)) {
                mean +=
                        -2.15
                                ...
                                * mag
                                - 1.24
                                * Math.log(dist + 0.00394
                                        * Math.exp(1.17 * mag))
                                + 1.44
                                * Math.log(dist + 0.0203
                                        * Math.exp(0.958 * mag))
                                + 0.1
                                * F
                                + (0.46 * tanh(2.68 * depth) - 0.53 * tanh(0.47 * depth))
                                * (1 - S_hr);
            }
        }
    ...
    public double getStdDev() throws IMRException {
        ...
        if (stdDevTypeParam.getValue()
            ...
            // PGA dependent case:
                ...
                try {
                    dist =
                            ((Double) distanceSeisParam.getValue())
                                    .doubleValue();
                ...
                pga =
                        -3.512
                                ...
                                * mag
                                - 1.328
                                * 0.5
                                * Math.log(dist * dist + 0.0222
                                        * Math.exp(1.294 * mag))
                                + (1.125 - 0.112 * Math.log(dist) - 0.0957 * mag)
                                * F + (0.44 - 0.171 * Math.log(dist)) * S_sr
                                + (0.405 - 0.222 * Math.log(dist)) * S_hr;
                if (depth <= 1.0) {
                    pga +=
                            (0.405 - 0.222 * Math.log(dist) - (0.44 - 0.171 * Math
            ...
            // Now do SA:
                ...
                double period =
                        ((Double) saPeriodParam.getValue()).doubleValue();
                if (period == 0) {
                    if (component.equals(ComponentParam.COMPONENT_AVE_HORZ)) {
                        return (sigma);
                    } else { // vertical component
    ...
    protected void initSiteParams() {
        ...
        DoubleConstraint basinDepthConstraint =
                new DoubleConstraint(BASIN_DEPTH_MIN, BASIN_DEPTH_MAX);
        basinDepthConstraint.setNullAllowed(true);
        basinDepthConstraint.setNonEditable();
        basinDepthParam =
                new WarningDoubleParameter(BASIN_DEPTH_NAME,
                        basinDepthConstraint, BASIN_DEPTH_UNITS);
    ...
    protected void initSupportedIntensityMeasureParams() {
        ...
        while (keys.hasMoreElements()) {
            Campbell_1997_AttenRelCoefficients coeff =
                    (Campbell_1997_AttenRelCoefficients) coefficients.get(keys
                            .nextElement());
            if (coeff.period >= 0) {
                set.add(new Double(coeff.period));
            }
        }
        // add the zero period by hand since there are no corresponding
        // coefficients
    ...
    protected void initCoefficients() {
        ...
        // There are no coefficients for PGA or PGV because these are hard wired
        // in the code
        // SA/0.05
        Campbell_1997_AttenRelCoefficients coeff0 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.05")).doubleValue(), 0.05, 0.05, 0.0,
                        0.0, -0.0011, 0.000055, 0.20, 0.0, 0.0, -1.32, 0.0,
                        0.0, 0.0, 0.0);
        // SA/0.075
        Campbell_1997_AttenRelCoefficients coeff1 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.075")).doubleValue(), 0.075, 0.27,
                        0.0, 0.0, -0.0024, 0.000095, 0.22, 0.0, 0.0, -1.21,
                        0.0, 0.0, 0.0, 0.0);
        // SA/0.1
        Campbell_1997_AttenRelCoefficients coeff2 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.1")).doubleValue(), 0.1, 0.48, 0.0,
                        0.0, -0.0024, 0.000007, 0.14, 0.0, 0.0, -1.29, 0.0,
                        0.0, 0.0, 0.0);
        // SA/0.15
        Campbell_1997_AttenRelCoefficients coeff3 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.15")).doubleValue(), 0.15, 0.72, 0.0,
                        0.0, -0.0010, -0.00027, -0.02, 0.0, 0.0, -1.57, 0.0,
                        0.0, 0.0, 0.0);
        // SA/0.2
        Campbell_1997_AttenRelCoefficients coeff4 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.2")).doubleValue(), 0.2, 0.79, 0.0,
                        0.0, 0.0011, -0.00053, -0.18, 0.0, 0.0, -1.73, 0.0,
                        0.0, 0.0, 0.0);
        // SA/0.3
        Campbell_1997_AttenRelCoefficients coeff5 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.3")).doubleValue(), 0.3, 0.77, 0.0,
                        0.0, 0.0035, -0.00072, -0.40, 0.0, 0.0, -1.98, 0.0,
                        0.0, 0.0, 0.0);
        // SA/0.5
        Campbell_1997_AttenRelCoefficients coeff6 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.5")).doubleValue(), 0.5, -0.28, 0.74,
                        0.66, 0.0068, -0.001, -0.42, 0.25, 0.62, -2.03, 0.46,
                        -0.74, 0.0, 0.0);
        // SA/0.75
        Campbell_1997_AttenRelCoefficients coeff7 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.75")).doubleValue(), 0.75, -1.08,
                        1.23, 0.66, 0.0077, -0.001, -0.44, 0.37, 0.62, -1.79,
                        0.67, -1.23, 0.0, 0.0);
        // SA/1.0
        Campbell_1997_AttenRelCoefficients coeff8 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("1.0")).doubleValue(), 1.0, -1.79, 1.59,
                        0.66, 0.0085, -0.001, -0.38, 0.57, 0.62, -1.82, 1.13,
                        -1.59, 0.18, -0.18);
        // SA/1.5
        Campbell_1997_AttenRelCoefficients coeff9 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("1.5")).doubleValue(), 1.5, -2.65, 1.98,
                        0.66, 0.0094, -0.001, -0.32, 0.72, 0.62, -1.81, 1.52,
                        -1.98, 0.57, -0.49);
        // SA/2.0
        Campbell_1997_AttenRelCoefficients coeff10 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("2.0")).doubleValue(), 2.0, -3.28, 2.23,
                        0.66, 0.0100, -0.001, -0.36, 0.83, 0.62, -1.65, 1.65,
                        -2.23, 0.61, -0.63);
        // SA/3.0
        Campbell_1997_AttenRelCoefficients coeff11 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("3.0")).doubleValue(), 3.0, -4.07, 2.39,
                        0.66, 0.0108, -0.001, -0.22, 0.86, 0.62, -1.31, 1.28,
                        -2.39, 1.07, -0.84);
        // SA/4.0
        Campbell_1997_AttenRelCoefficients coeff12 =
                new Campbell_1997_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("4.0")).doubleValue(), 4.0, -4.26, 2.03,
                        0.66, 0.0112, -0.001, -0.30, 1.05, 0.62, -1.35, 1.15,
                        -2.03, 1.26, -1.17);
     ...
     * to calculate the Mean and StdDev for the Campbell_1997_AttenRel. One
     * instance of this class holds the set of coefficients for each period (one
     * row of his table 5 & 6).<br>
    ...
    class Campbell_1997_AttenRelCoefficients implements NamedObjectAPI {
        ...
        /**
         * Constructor for the Campbell_1997_AttenRelCoefficients object that
        ...
        public Campbell_1997_AttenRelCoefficients(String name, double period,
                double c1_h, double c2_h, double c3_h, double c4_h,
                double c5_h, double c6, double c7, double c8, double c1_v,
                double c2_v, double c3_v, double c4_v, double c5_v) {
            this.period = period;
            this.name = name;
            this.c1_h = c1_h;
            this.c2_h = c2_h;
        ...
        /**
         * Gets the name attribute of the Campbell_1997_AttenRelCoefficients
         * object
         * 
    ...
    public URL getInfoURL() throws MalformedURLException {
        return new URL(
                "http://www.opensha.org/documentation/modelsImplemented/attenRel/Campbell_1997.html");
    }
