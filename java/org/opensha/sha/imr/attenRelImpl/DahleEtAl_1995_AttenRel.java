...
/**
 * <b>Title:</b> DahleEtAl_1995_AttenRel
 * <p>
 * 
 * <b>Description:</b> This implements the Attenuation Relationship developed by
 * Dahle et al. (1995, Proc. 5th Int Conf on Seismic Zonation, Oct 17-19, 1995,
 * Nice, France, p 1005-1012)
 * <p>
 * 
...
public class DahleEtAl_1995_AttenRel extends AttenuationRelationship implements
     ...
     * as that passed in. Warning constrains are ingored.
     * 
     * @param eqkRupture
     ...
     * Site object passed in (the Site object must have a parameter with the
     * same name as that in vs30Param). This also sets the internally held Site
     * object as that passed in.
     * 
     * @param site
     *            The new site value which the site-related parameter
    ...
    protected void updateCoefficients() throws ParameterException {
        ...
        if (coefficients.containsKey(key.toString())) {
            coeff =
                    (DahleEtAl_AttenRelCoefficients) coefficients.get(key
                            .toString());
        } else {
     ...
     */
    public DahleEtAl_1995_AttenRel(
    ...
    public double getMean() throws IMRException {
        ...
        double mean =
                coeff.c1 + coeff.c2 * mag + coeff.c3 * Math.log(R) + coeff.c4
                        * R + coeff.c5 * S;

        /*
         * convert to PSV for comparison with their figures (only for tests!):
         * if ( coeff.period != 0.0 ) mean /= (6.28318 /coeff.period);
         */
    ...
    protected void initSupportedIntensityMeasureParams() {
        ...
        while (keys.hasMoreElements()) {
            DahleEtAl_AttenRelCoefficients coeff =
                    (DahleEtAl_AttenRelCoefficients) coefficients.get(keys
                            .nextElement());
            if (coeff.period >= 0) {
    ...
    protected void initOtherParams() {
        ...
        stdDevTypeParam =
                new StdDevTypeParam(stdDevTypeConstraint, STD_DEV_TYPE_DEFAULT);
    ...
    protected void initCoefficients() {
        ...
        // PGA
        DahleEtAl_AttenRelCoefficients coeff =
                new DahleEtAl_AttenRelCoefficients(PGA_Param.NAME, 0, -1.579,
                        0.554, -0.560, -0.00302, 0.326, 6.0, 0.75, 0.73);

        // SA/0.00
        DahleEtAl_AttenRelCoefficients coeff0 =
                new DahleEtAl_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.00")).doubleValue(), 0.00, -1.579,
                        0.554, -0.560, -0.00302, 0.326, 6.0, 0.75, 0.73);
        // SA/0.025
        DahleEtAl_AttenRelCoefficients coeff1 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("0.025")).doubleValue(), 0.025, -7.106,
                        0.554, -0.560, -0.00302, 0.326, 6.0, 0.75, 0.73);
        // SA/0.05
        DahleEtAl_AttenRelCoefficients coeff2 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("0.05")).doubleValue(), 0.05, -5.375,
                        0.449, -0.575, -0.00246, 0.308, 6.0, 0.78, 0.76);
        // SA/0.1
        DahleEtAl_AttenRelCoefficients coeff3 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("0.1")).doubleValue(), 0.1, -4.608,
                        0.486, -0.609, -0.00198, 0.381, 6.0, 0.81, 0.79);
        // SA/0.2
        DahleEtAl_AttenRelCoefficients coeff4 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("0.2")).doubleValue(), 0.2, -4.746,
                        0.645, -0.674, -0.00155, 0.470, 6.0, 0.82, 0.80);
        // SA/0.5
        DahleEtAl_AttenRelCoefficients coeff5 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("0.5")).doubleValue(), 0.5, -5.717,
                        0.920, -0.761, -0.00106, 0.566, 6.0, 0.83, 0.81);
        // SA/1.0
        DahleEtAl_AttenRelCoefficients coeff6 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("1.0")).doubleValue(), 1.0, -6.595,
                        1.084, -0.792, -0.00075, 0.588, 6.0, 0.82, 0.79);
        // SA/2.0
        DahleEtAl_AttenRelCoefficients coeff7 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("2.0")).doubleValue(), 2.0, -7.205,
                        1.131, -0.762, -0.00051, 0.536, 6.0, 0.79, 0.75);
        // SA/4.0
        DahleEtAl_AttenRelCoefficients coeff8 =
                new DahleEtAl_AttenRelCoefficients("SA/"
                        + (new Double("4.0")).doubleValue(), 4.0, -7.324,
                        1.009, -0.629, -0.00038, 0.496, 6.0, 0.73, 0.67);
     ...
     * to calculate the Mean and StdDev for this relationship.<br>
    ...
    class DahleEtAl_AttenRelCoefficients implements NamedObjectAPI {
        ...
        /**
         * Constructor for the DahleEtAl_AttenRelCoefficients object that sets
         * all values at once
         * 
         * @param name
         *            Description of the Parameter
        ...
        public DahleEtAl_AttenRelCoefficients(String name, double period,
                ...
                double rh, double sigmaBay, double sigmaLs) {
            this.name = name;
            this.period = period;
            this.c1 = c1;
            this.c2 = c2;
