...
/**
 * <b>Title:</b> Field_2000_AttenRel
 * <p>
 * 
 * <b>Description:</b> This implements the Attenuation Relationship developed by
 * Field (2000, Bulletin of the Seismological Society of America, vol 90, num
 * 6b, pp S209-S221)
 * <p>
 * 
 ...
 * <LI>pgvParam - Peak Ground Velocity (from 1-sec SA using the Newmark & Hall
 * (1982) mult factor of 37.27*2.54)
 * </UL>
 * <p>
...
public class Field_2000_AttenRel extends AttenuationRelationship implements
     ...
     * Options are "Reverse" if 135>rake>45 or "Other/Unknown" otherwise.
     * 
     * @param rake
     *            in degrees
    ...
    /**
     * This sets the site-related parameters (vs30Param and basinDepthParam)
     * based on what is in the Site object passed in. This also sets the
     * internally held Site object as that passed in. Warning constrains are
     * ingored.
     * 
     * @param site
     *            The new site
     ...
     * @param propEffect
     * @throws ParameterException
     *             Thrown if the Site object doesn't contain Vs30 parameter.
     * @throws InvalidRangeException
     *             thrown if rake is out of bounds
     */
    ...
    protected void updateCoefficients() throws ParameterException {
        ...
        if (im.getName().equals(PGV_Param.NAME)) {
            coeff =
                    (Field_2000_AttenRelCoefficients) coefficients
                            .get("SA/1.0");
        } else {
            ...
            if (coefficients.containsKey(key.toString())) {
                coeff =
                        (Field_2000_AttenRelCoefficients) coefficients.get(key
                                .toString());
            } else {
    ...
    public double getStdDev() throws IMRException {
        ...
        if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_TOTAL_MAG_DEP)) { // "Total - Mag Dep."
            temp_std =
                    Math.sqrt(coeff.intra_slope * mag + coeff.intra_intercept
                            + coeff.tau * coeff.tau);
        } else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_TOTAL)) { // "Total"
            temp_std =
                    Math.sqrt((coeff.intra_mag_ind * coeff.intra_mag_ind + coeff.tau
                            * coeff.tau));
        } else if (stdDevType.equals(StdDevTypeParam.STD_DEV_TYPE_INTER)) { // "Inter-Event"
            temp_std = coeff.tau;
        } else if (stdDevType
                .equals(StdDevTypeParam.STD_DEV_TYPE_INTRA_MAG_DEP)) { // "Intra-Event - Mag. Dep."
            temp_std =
                    Math.sqrt(coeff.intra_slope * mag + coeff.intra_intercept);
    ...
    /**
     * Creates the Vs30 & basinDepth arameters and adds them to the siteParams
    ...
    protected void initSupportedIntensityMeasureParams() {
        ...
        while (keys.hasMoreElements()) {
            Field_2000_AttenRelCoefficients coeff =
                    (Field_2000_AttenRelCoefficients) coefficients.get(keys
                            .nextElement());
            if (coeff.period >= 0) {
    ...
    protected void initOtherParams() {
        ...
        stdDevTypeConstraint
                .addString(StdDevTypeParam.STD_DEV_TYPE_INTRA_MAG_DEP);
    ...
    protected void initCoefficients() {
        ...
        // PGA
        Field_2000_AttenRelCoefficients coeff =
                new Field_2000_AttenRelCoefficients(PGA_Param.NAME, 0.0, 0.853,
                        0.872, 0.442, -0.067, -0.960, -0.154, 8.90, 0.067,
                        -0.14, -0.1, 0.8771, 0.23, 0.47);

        // SA/0.00
        Field_2000_AttenRelCoefficients coeff0 =
                new Field_2000_AttenRelCoefficients(SA_Param.NAME + '/'
                        + (new Double("0.0")).doubleValue(), 0.0, 0.853, 0.872,
                        0.442, -0.067, -0.960, -0.154, 8.90, 0.067, -0.14,
                        -0.1, 0.8771, 0.23, 0.47);
        // SA/0.3
        Field_2000_AttenRelCoefficients coeff1 =
                new Field_2000_AttenRelCoefficients("SA/"
                        + (new Double("0.3")).doubleValue(), 0.3, 0.995, 1.096,
                        0.501, -0.112, -0.841, -0.350, 7.20, 0.057, -0.12,
                        -0.11, 0.9924, 0.26, 0.53);
        // SA/1.0
        Field_2000_AttenRelCoefficients coeff2 =
                new Field_2000_AttenRelCoefficients("SA/"
                        + (new Double("1.0")).doubleValue(), 1.0, -0.164,
                        -0.267, 0.903, 0.0, -0.914, -0.704, 6.20, 0.12, -0.25,
                        -0.1, 0.9516, 0.22, 0.53);
        // SA/3.0
        Field_2000_AttenRelCoefficients coeff3 =
                new Field_2000_AttenRelCoefficients("SA/"
                        + (new Double("3.0")).doubleValue(), 3.0, -2.267,
                        -2.681, 1.083, 0.0, -0.720, -0.674, 3.00, 0.11, -0.18,
                        0.14, -0.66, 0.3, 0.52);
     ...
     * to calculate the Mean and StdDev for the Field_2000_AttenRel. One
     * instance of this class holds the set of coefficients for each period (one
     * row of their table 8).<br>
    ...
    class Field_2000_AttenRelCoefficients implements NamedObjectAPI {
        ...
        /**
         * Constructor for the Field_2000_AttenRelCoefficients object that sets
         * all values at once
         * 
         * @param name
         *            Description of the Parameter
    ...
    public URL getInfoURL() throws MalformedURLException {
        return new URL(
                "http://www.opensha.org/documentation/modelsImplemented/attenRel/Field_2000.html");
    }
