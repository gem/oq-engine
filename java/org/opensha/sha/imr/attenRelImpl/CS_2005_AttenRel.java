...
/**
 * <b>Title:</b> CS_2005_AttenRel
 * <p>
 * 
 * <b>Description:</b> This implements the Attenuation Relationship developed by
 * Choi and Stewart (2005, "Nonlinear Site Amplification as a Function of 30 m
 * Shear Wave Velocity", Earthquake Spectre, v. 21, pp. 1-30). This applies an
 * alternative site-response correction to the Abrahamson & Silva (1997)
 * relationship.
 * <p>
 * 
...
public class CS_2005_AttenRel extends AttenuationRelationship implements
    ...
    public CS_2005_AttenRel(ParameterChangeWarningListener warningListener) {
        ...
        as_1997_attenRel.getParameter(as_1997_attenRel.SITE_TYPE_NAME)
                .setValue(as_1997_attenRel.SITE_TYPE_ROCK);
        ...
        this.sigmaTruncTypeParam =
                (SigmaTruncTypeParam) as_1997_attenRel
                        .getParameter(SigmaTruncTypeParam.NAME);
        this.sigmaTruncLevelParam =
                (SigmaTruncLevelParam) as_1997_attenRel
                        .getParameter(SigmaTruncLevelParam.NAME);
        this.exceedProbParam =
                (DoubleParameter) as_1997_attenRel
                        .getParameter(as_1997_attenRel.EXCEED_PROB_NAME);
        this.stdDevTypeParam =
                (StdDevTypeParam) as_1997_attenRel
                        .getParameter(StdDevTypeParam.NAME);
        this.saPeriodParam =
                (PeriodParam) as_1997_attenRel.getParameter(PeriodParam.NAME);
        ...
        /*
         * TreeSet set = new TreeSet(); Enumeration keys = horzCoeffs.keys(); //
         * same as for vertCoeffs while ( keys.hasMoreElements() ) {
         * CS_2005_AttenRelCoefficients c = ( CS_2005_AttenRelCoefficients )
         * horzCoeffs.get( keys.nextElement() );
         * System.out.println(c.period+"\t"
         * +c.b1+"\t"+c.vRef+"\t"+c.c+"\t"+c.b2+"\t"+c.tau+"\t"+c.e1+"\t"+c.e3);
         * }
         */
    }
    ...
    /**
     * This sets the site-related parameter (vs30Param) based on what is in the
     * Site object passed in. WarningExceptions are ingored
     * 
     * @param site
     *            The new site value which contains a Vs30 Parameter
    ...
    public void setSite(Site site) throws ParameterException {
        ...
        softSoilParam.setValue((Boolean) (site.getParameter(SOFT_SOIL_NAME)
                .getValue()));
        this.site = site;
     ...
     * be used given the current intensityMeasure (im) Parameter.
     */
    protected void updateCoefficients() throws ParameterException {
        ...
        if (horzCoeffs.containsKey(key.toString())) {
            coeffs =
                    (CS_2005_AttenRelCoefficients) horzCoeffs.get(key
                            .toString());
        } else {
    ...
    private double getLnAmpFactor(double vs30, double lnRockPGA) {
        ...
        return coeffs.c * Math.log(vs30 / coeffs.vRef) + b
                * (lnRockPGA - Math.log(0.1));
    }
    ...
    protected void initCoefficients() {
        ...
        CS_2005_AttenRelCoefficients coeff =
                new CS_2005_AttenRelCoefficients(PGA_Param.NAME, 0.0, -0.64,
                        418, -0.36, -0.14, 0.27, 0.44, 0.50);

        CS_2005_AttenRelCoefficients coeff0 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.01")).doubleValue(), 0.01, -0.64, 418,
                        -0.36, -0.14, 0.27, 0.44, 0.50);
        CS_2005_AttenRelCoefficients coeff1 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.02")).doubleValue(), 0.02, -0.63, 490,
                        -0.34, -0.12, 0.26, 0.45, 0.51);
        CS_2005_AttenRelCoefficients coeff2 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.03")).doubleValue(), 0.03, -0.62, 324,
                        -0.33, -0.11, 0.26, 0.46, 0.51);
        CS_2005_AttenRelCoefficients coeff3 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.04")).doubleValue(), 0.04, -0.61, 233,
                        -0.31, -0.11, 0.26, 0.47, 0.51);
        CS_2005_AttenRelCoefficients coeff4 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.05")).doubleValue(), 0.05, -0.64, 192,
                        -0.29, -0.11, 0.25, 0.47, 0.52);
        CS_2005_AttenRelCoefficients coeff5 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.06")).doubleValue(), 0.06, -0.64, 181,
                        -0.25, -0.11, 0.25, 0.48, 0.52);
        CS_2005_AttenRelCoefficients coeff6 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.075")).doubleValue(), 0.075, -0.64,
                        196, -0.23, -0.11, 0.24, 0.48, 0.52);
        CS_2005_AttenRelCoefficients coeff7 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.09")).doubleValue(), 0.09, -0.64, 239,
                        -0.23, -0.12, 0.23, 0.49, 0.52);
        CS_2005_AttenRelCoefficients coeff8 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.10")).doubleValue(), 0.10, -0.60, 257,
                        -0.25, -0.13, 0.23, 0.49, 0.53);
        CS_2005_AttenRelCoefficients coeff9 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.12")).doubleValue(), 0.12, -0.56, 299,
                        -0.26, -0.14, 0.24, 0.49, 0.53);
        CS_2005_AttenRelCoefficients coeff10 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.15")).doubleValue(), 0.15, -0.53, 357,
                        -0.28, -0.18, 0.25, 0.49, 0.54);
        CS_2005_AttenRelCoefficients coeff11 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.17")).doubleValue(), 0.17, -0.53, 406,
                        -0.29, -0.19, 0.26, 0.48, 0.55);
        CS_2005_AttenRelCoefficients coeff12 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.20")).doubleValue(), 0.20, -0.52, 453,
                        -0.31, -0.19, 0.27, 0.47, 0.56);
        CS_2005_AttenRelCoefficients coeff13 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.24")).doubleValue(), 0.24, -0.52, 493,
                        -0.38, -0.16, 0.29, 0.47, 0.56);
        CS_2005_AttenRelCoefficients coeff14 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.30")).doubleValue(), 0.30, -0.52, 532,
                        -0.44, -0.14, 0.35, 0.46, 0.57);
        CS_2005_AttenRelCoefficients coeff15 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.36")).doubleValue(), 0.36, -0.51, 535,
                        -0.48, -0.11, 0.38, 0.46, 0.57);
        CS_2005_AttenRelCoefficients coeff16 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.40")).doubleValue(), 0.40, -0.51, 535,
                        -0.50, -0.10, 0.40, 0.46, 0.57);
        CS_2005_AttenRelCoefficients coeff17 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.46")).doubleValue(), 0.46, -0.50, 535,
                        -0.55, -0.08, 0.42, 0.45, 0.58);
        CS_2005_AttenRelCoefficients coeff18 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.50")).doubleValue(), 0.50, -0.50, 535,
                        -0.60, -0.06, 0.42, 0.45, 0.59);
        CS_2005_AttenRelCoefficients coeff19 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.60")).doubleValue(), 0.60, -0.49, 535,
                        -0.66, -0.03, 0.42, 0.44, 0.60);
        CS_2005_AttenRelCoefficients coeff20 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.75")).doubleValue(), 0.75, -0.47, 535,
                        -0.69, 0.00, 0.42, 0.44, 0.63);
        CS_2005_AttenRelCoefficients coeff21 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.85")).doubleValue(), 0.85, -0.46, 535,
                        -0.69, 0.00, 0.42, 0.44, 0.63);
        CS_2005_AttenRelCoefficients coeff22 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("1.00")).doubleValue(), 1.00, -0.44, 535,
                        -0.70, 0.00, 0.42, 0.44, 0.64);
        CS_2005_AttenRelCoefficients coeff23 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("1.50")).doubleValue(), 1.50, -0.40, 535,
                        -0.72, 0.00, 0.42, 0.44, 0.67);
        CS_2005_AttenRelCoefficients coeff24 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("2.00")).doubleValue(), 2.00, -0.38, 535,
                        -0.73, 0.00, 0.43, 0.44, 0.69);
        CS_2005_AttenRelCoefficients coeff25 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("3.00")).doubleValue(), 3.00, -0.34, 535,
                        -0.74, 0.00, 0.45, 0.44, 0.71);
        CS_2005_AttenRelCoefficients coeff26 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("4.00")).doubleValue(), 4.00, -0.31, 535,
                        -0.75, 0.00, 0.47, 0.44, 0.73);
        CS_2005_AttenRelCoefficients coeff27 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("5.00")).doubleValue(), 5.00, -0.30, 535,
                        -0.75, 0.00, 0.49, 0.44, 0.75);
        // add zero-period case; same as 0.01 sec.
        CS_2005_AttenRelCoefficients coeff28 =
                new CS_2005_AttenRelCoefficients(SA_Param.NAME + "/"
                        + (new Double("0.0")).doubleValue(), 0.0, -0.64, 418,
                        -0.36, -0.14, 0.27, 0.44, 0.50);
    ...
    class CS_2005_AttenRelCoefficients implements NamedObjectAPI {
        ...
        /**
         * Constructor for the CS_2005_AttenRelCoefficients object that sets all
         * values at once
         * 
         * @param name
        ...
        public CS_2005_AttenRelCoefficients(String name, double period,
                double b1, double vRef, double c, double b2, double tau,
                double e1, double e3) {
