...
/**
 * <b>Title:</b> WarningDoublePropagationEffectParameter
 * <p>
 * 
 * <b>Description:</b> Base Propagation Effect Parameter that implements the
 * WarningParameterAPI. This class is only needed and distinct from the
 * WarningDoubleParameter because multiple inheritance is not supported in Java.
 * These PropagationEffect Parameters need a different base class
 * (PropagationEffectParameter) than the WarningDoubleParameter (
 * DoubleParameter ). It basically has the same functionality. See
 * WarningDoublepParameter for further documentation.
 * <p>
 * 
 * @author Steven W. Rock
 * @version 1.0
 */
public abstract class WarningDoublePropagationEffectParameter extends
        PropagationEffectParameter implements WarningParameterAPI {
    ...
    /**
     * Listeners that are interested in receiveing warnings when the warning
     * constraints are exceeded. Only created if needed, else kept null, i.e.
     * "Lazy Instantiation".
     */
    ...
    /**
     * Set to true to turn off warnings, will automatically set the value,
     * unless exceeds Absolute contrsints. Set to false so that warning
     * constraints are enabled, i.e. throw a WarningConstraintException if
     * exceed recommened warnings.
     */
    ...
    /**
     * Returns warning constraint enabled/disabled. If true warnings are turned
     * off , will automatically set the value, unless exceeds Absolute
     * contrsints. If set to false warning constraints are enabled, i.e. throw a
    ...
    /**
     * Adds a listener to receive warning events when the warning constraints
     * are exceeded. Only permitted if this parameter is currenlty editable,
     ...
     * else an EditableException is thrown.
     */
    public synchronized void removeParameterChangeWarningListener(
            ParameterChangeWarningListener listener) throws EditableException {
    ...
    /**
     * Sets the constraint if it is a DoubleConstraint and the parameter is
     * currently editable.
     * 
     * @param warningConstraint
     *            The new constraint for warnings
     * @throws ParameterException
     *             Thrown if the constraint is not a DoubleConstraint
     * @throws EditableException
     *             Thrown if the isEditable flag set to false.
     */
    ...
    /**
     * Gets the min value of the constraint object. If the constraint is not set
     * returns null.
     */
    public Object getWarningMin() throws Exception {
        if (warningConstraint != null)
            return warningConstraint.getMin();
        else
            return null;
    }

    /**
     * Returns the maximum allowed value of the constraint object. If the
     * constraint is not set returns null.
     */
    public Object getWarningMax() {
        if (warningConstraint != null)
            return warningConstraint.getMax();
        else
            return null;
     ...
     */
    public synchronized void setValue(Object value) throws ConstraintException,
            WarningException {
        ...
        if (!isAllowed(value)) {
            ...
            if (D)
                System.out.println(S
                        + "Setting allowed and recommended null value: ");
            this.value = null;
            org.opensha.commons.param.event.ParameterChangeEvent event =
                    new org.opensha.commons.param.event.ParameterChangeEvent(
                            this, getName(), getValue(), value);
            firePropertyChange(event);
        } else if (!ignoreWarning && !isRecommended(value)) {

            if (D)
                System.out.println(S + "Firing Warning Event");

            ParameterChangeWarningEvent event =
                    new ParameterChangeWarningEvent((Object) this, this,
                            this.value, value);

            fireParameterChangeWarning(event);
            throw new WarningException(S + "Value is not recommended: "
                    + value.toString());
        } else {
            if (D)
                System.out.println(S
                        + "Setting allowed and recommended value: ");
            this.value = value;
            org.opensha.commons.param.event.ParameterChangeEvent event =
                    new org.opensha.commons.param.event.ParameterChangeEvent(
                            this, getName(), getValue(), value);
    ...
    /**
     * Set's the parameter's value bypassing all checks including the absolute
     * constraint check. WARNING: SWR: This may be a bug. Should we bypass the
     * Absolute Constraints. ???
     */
    public void setValueIgnoreWarning(Object value) throws ConstraintException,
            ParameterException {
        // this.value = value;
    ...
    public boolean isRecommended(Object obj) {
        if (warningConstraint != null)
            return warningConstraint.isAllowed((Double) obj);
        else
            return true;
     ...
     * @return -1 if this value < obj value, 0 if equal, +1 if this value > obj
     *         value
     * @exception ClassCastException
     *                Is thrown if the comparing object is not a
     *                DoubleParameter, or DoubleDiscreteParameter.
     */
    public int compareTo(Object obj) throws ClassCastException {
        ...
        if (obj instanceof DoubleParameter) {
            ...
            WarningDoublePropagationEffectParameter param =
                    (WarningDoublePropagationEffectParameter) obj;
            n2 = (Double) param.getValue();
        }
    ...
    /**
     * Standard Java function. Creates a copy of this class instance so
     * originaly can not be modified
     */
