...
public class GemComputeHazardLogicTree {
    ...
    private LogicTree<ArrayList<GEMSourceData>> ilt;
     ...
     * @param AttenRel
     */
    public GemComputeHazardLogicTree(
            LogicTree<ArrayList<GEMSourceData>> inputToERFLT,
            LogicTree<HashMap<TectonicRegionType, ScalarIntensityMeasureRelationshipAPI>> gmpeLT,
    ...
    public GEMHazardCurveRepositoryList calculateHaz() {
        ...
        // Calculate the number of GMPE logic tree end branches. This
        // corresponds to the
        // number of Elements in the HashMap that associate the end branches of
        // the logic
        // tree to sets of GMPEs dependent on the tectonic region (in principle
        // in each GMPE
        // set there should be one GMPE associated to each Tectonic region
        // contained in the
        // input model)
        ...
        for (int i = 0; i < ilt.getEBMap().size(); i++) {
            ...
            System.out.println("ERF End-branch: " + currentModelLabel + " ,"
                    + (i + 1) + " of " + ilt.getEBMap().size());

            // Instantiate ERF
            GEM1ERF modelERF =
                    new GEM1ERF(ilt.getEBMap().get(currentModelLabel), calcSet);// new
                                                                                // GEM1ERF(ilt.getEBMap().get(currentModelLabel));
            // set ERF parameters
            // setErfParams(modelERF);
            // update forecast
            // ParameterChangeEvent event = new
            // ParameterChangeEvent(modelERF,"ciccio","ciccio","ciccio");
            ...
            for (int j = 0; j < nEndBranchGmpe; j++) {
                ...
                // Label identifying the current end branch of the GMPE logic
                // tree
                String currentGmpeLabel = iterGmpeLabel.next();
                System.out.println("GMPE End-branch: " + currentGmpeLabel
                        + " ," + (j + 1) + " of " + nEndBranchGmpe);

                // Calculate the hazard
                GemComputeHazard compHaz =
                        new GemComputeHazard(
                                (Integer) calcSet.getOut().get(
                                        CpuParams.CPU_NUMBER.toString()),
                                (ArrayList<Site>) calcSet.getOut().get(
                                        SiteParams.SITE_LIST.toString()),
                                modelERF,
                                gmpeLT.getEBMap().get(currentGmpeLabel),
                                (ArbitrarilyDiscretizedFunc) calcSet.getOut()
                                        .get(IMLListParams.IML_LIST.toString()),
                                (Double) calcSet.getOut().get(
                                        DistanceParams.MAX_DIST_SOURCE
                                                .toString()));
                hcRepList.add(compHaz.getValues(), currentModelLabel + "_"
                        + currentGmpeLabel);
            }
        }
        return hcRepList;
    ...
    //
    // // Instantiate the repository for the results
    // GEMHazardCurveRepositoryList hcRepList = new
    // GEMHazardCurveRepositoryList();
    //
    // // Set Gmpe(s) parameters
    // setGmpeParams();
    //
    ...
    // Iterator<String> iterGEMInputToERFLabels =
    // GEMInputToERFLabels.iterator();
    //
    // // Calculate the number of GMPE logic tree end branches. This corresponds
    // to the
    // // number of Elements in the HashMap that associate the end branches of
    // the logic
    // // tree to sets of GMPEs dependent on the tectonic region (in principle
    ...
    //
    // // Instantiate ERF
    // //GEM1ERF modelERF = new
    // GEM1ERF(ilt.getEBMap().get(currentGEMInputToERFLabel));
    // //modelERF.updateForecast();
    // // Iterator over GEMgmpe
    // Set<String> gmpeLabels = gmpeLT.getEBMap().keySet();
    // Iterator<String> iterGmpeLabel = gmpeLabels.iterator();
    ...
    // // Calculate the hazard
    // GemComputeHazard compHaz = new
    // GemComputeHazard((Integer)calcSet.getOut().get(CpuParams.CPU_NUMBER.toString()),
    // (ArrayList<Site>)calcSet.getOut().get(SiteParams.SITE_LIST.toString()),
    // (ArrayList<GEMSourceData>)ilt.getEBMap().get(currentGEMInputToERFLabel),
    // gmpeLT.getEBMap().get(currentGmpeLabel),
    ...
    public static
        ...
        for (int ig = 0; ig < gmpeLT.getEBMap().size(); ig++) {
            ...
            for (int itec = 0; itec < numTecRegType; itec++) {
                ...
                // if a gmpe for the current tectonic region type is considered
                // in the logic tree than set the params
                if (gmpeLT.getEBMap().get(Integer.toString(ig + 1)).get(trt) != null) {

                    // current attenuation relationship short name
                    String arShortName =
                            gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                    .get(trt).getShortName();

                    // if the calculation settings for the corresponding gmpe
                    // exists than set the parameters
                    if (calcSet.getGmpe().get(arShortName) != null) {

                        // intensity measure type
                        String imt =
                                calcSet.getOut()
                                        .get(IntensityMeasureParams.INTENSITY_MEAS_TYPE
                                                .toString()).toString();
                        gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                .get(trt).setIntensityMeasure(imt);

                        // truncation type
                        String sttp =
                                calcSet.getOut().get(SigmaTruncTypeParam.NAME)
                                        .toString();
                        gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                .get(trt)
                        ...
                        String sdtp =
                                calcSet.getOut().get(StdDevTypeParam.NAME)
                                        .toString();
                        gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                .get(trt).getParameter(StdDevTypeParam.NAME)
                                .setValue(sdtp);

                        // component
                        String cp =
                                calcSet.getGmpe().get(arShortName)
                                        .get(ComponentParam.NAME).toString();
                        gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                .get(trt).getParameter(ComponentParam.NAME)
                                .setValue(cp);

                        // vs 30 (This is optional)
                        if ((Double) calcSet.getGmpe().get(arShortName)
                                .get(Vs30_Param.NAME) != null) {
                            double vs30 =
                                    (Double) calcSet.getGmpe().get(arShortName)
                                            .get(Vs30_Param.NAME);
                            gmpeLT.getEBMap().get(Integer.toString(ig + 1))
                                    .get(trt).getParameter(Vs30_Param.NAME)
                                    .setValue(vs30);
                        }
                    ...
                    // else throw exception
                        ...
                        System.out.println("Calculation settings for "
                                + arShortName + " does not exist!");
                        System.exit(0);
                    }
                }
            }
    ...
    private void setErfParams(GEM1ERF modelERF) {
        ...
        modelERF.setParameter(
                GEM1ERF.AREA_SRC_RUP_TYPE_NAME,
                (String) calcSet.getErf().get(SourceType.AREA_SOURCE)
                        .get(GEM1ERF.AREA_SRC_RUP_TYPE_NAME));
        // lower seismogenic depth
        modelERF.setParameter(
                GEM1ERF.AREA_SRC_LOWER_SEIS_DEPTH_PARAM_NAME,
                (Double) calcSet.getErf().get(SourceType.AREA_SOURCE)
                        .get(GEM1ERF.AREA_SRC_LOWER_SEIS_DEPTH_PARAM_NAME));
        ...
        modelERF.setParameter(
                ...
                (String) calcSet.getErf().get(SourceType.AREA_SOURCE)
                        .get(GEM1ERF.AREA_SRC_MAG_SCALING_REL_PARAM_NAME));

        // set point source parameters
        // source modeling type
        modelERF.setParameter(
                GEM1ERF.GRIDDED_SEIS_RUP_TYPE_NAME,
                (String) calcSet.getErf().get(SourceType.GRID_SOURCE)
                        .get(GEM1ERF.GRIDDED_SEIS_RUP_TYPE_NAME));
        // lower seismogenic depth
        modelERF.setParameter(
                GEM1ERF.GRIDDED_SEIS_LOWER_SEIS_DEPTH_PARAM_NAME,
                (Double) calcSet.getErf().get(SourceType.GRID_SOURCE)
        ...
        modelERF.setParameter(
                ...
                (Double) calcSet.getErf().get(SourceType.FAULT_SOURCE)
                        .get(GEM1ERF.FAULT_RUP_OFFSET_PARAM_NAME));
        // fault discretization
        modelERF.setParameter(
                GEM1ERF.FAULT_DISCR_PARAM_NAME,
                (Double) calcSet.getErf().get(SourceType.FAULT_SOURCE)
                        .get(GEM1ERF.FAULT_DISCR_PARAM_NAME));
        // mag scaling relationship
        modelERF.setParameter(
                GEM1ERF.FAULT_MAG_SCALING_REL_PARAM_NAME,
                (String) calcSet.getErf().get(SourceType.FAULT_SOURCE)
                        .get(GEM1ERF.FAULT_MAG_SCALING_REL_PARAM_NAME));
        // standard deviation
        modelERF.setParameter(
                GEM1ERF.FAULT_SCALING_SIGMA_PARAM_NAME,
                (Double) calcSet.getErf().get(SourceType.FAULT_SOURCE)
                        .get(GEM1ERF.FAULT_SCALING_SIGMA_PARAM_NAME));
        ...
        modelERF.setParameter(
                ...
                (String) calcSet.getErf()
                        .get(SourceType.SUBDUCTION_FAULT_SOURCE)
                        .get(GEM1ERF.SUB_MAG_SCALING_REL_PARAM_NAME));
        // standard deviation
        modelERF.setParameter(
        ...
        modelERF.setParameter(
                ...
                (String) calcSet.getErf()
                        .get(SourceType.SUBDUCTION_FAULT_SOURCE)
                        .get(GEM1ERF.SUB_FLOATER_TYPE_PARAM_NAME));
