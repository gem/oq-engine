...
public class GscFriskInputHeader {
	...
	int nRupL; // 				L5			: Number of rupture lengths to account of m-length variability
	...
	public GscFriskInputHeader(BufferedReader input) throws IOException {
		...
		if (matcher.find()) this.npexc = Integer.valueOf(matcher.group(0)).intValue();
		...
		strarr = line.split("\\s+");
		matcher = INTNUM.matcher(strarr[0]); 
		if (matcher.find()) this.natt = Integer.valueOf(matcher.group(0)).intValue();	
		if (INTNUM.matcher(strarr[1]).find()) this.nstep = Integer.valueOf(matcher.group(0)).intValue();
		if (FLOATNUM.matcher(strarr[2]).find()) this.vIncr = Double.valueOf(matcher.group(0)).doubleValue();
		if (FLOATNUM.matcher(strarr[3]).find()) this.hIncr = Double.valueOf(matcher.group(0)).doubleValue();
		if (INTNUM.matcher(strarr[4]).find()) this.nRupL = Integer.valueOf(matcher.group(0)).intValue();
		if (INTNUM.matcher(strarr[5]).find()) this.outContrl = Integer.valueOf(matcher.group(0)).intValue();
		// Sixth line
		strarr = input.readLine().split("\\s+");
		if (INTNUM.matcher(strarr[0]).find()) this.nVals = Integer.valueOf(matcher.group(0)).intValue();	
		...
		strarr = line.split("\\s+");
		matcher = INTNUM.matcher(strarr[0]); 
		if (matcher.find()) this.nGloAlt = Integer.valueOf(matcher.group(0)).intValue();
