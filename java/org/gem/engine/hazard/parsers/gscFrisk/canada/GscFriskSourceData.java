                ...
                for (int geomIdx = 0; geomIdx < src.geomNum; geomIdx++) {
                    ...
                    for (int depIdx = 0; depIdx < src.geomDepth[geomIdx].length; depIdx++) {
                        ...
                        for (int i = 0; i < src.mMax[geomIdx].length; i++)
                            if (mmax < src.mMax[geomIdx][i])
                                mmax = src.mMax[geomIdx][i];

                        int num = (int) Math.round((mmax - MMIN) / MWDT);

                        // Define the summed mfd
                        sumMfd =
                                new SummedMagFreqDist(MMIN + MWDT / 2, num,
                                        MWDT);
                        ...
                        for (int mmaxIdx = 0; mmaxIdx < src.mMax[geomIdx].length; mmaxIdx++) {
                            for (int nubIdx = 0; nubIdx < src.beta[geomIdx].length; nubIdx++) {
                                ...
                                num =
                                        (int) Math
                                                .round((src.mMax[geomIdx][mmaxIdx] - MMIN)
                                                        / MWDT);
                                IncrementalMagFreqDist mfd =
                                        new IncrementalMagFreqDist(MMIN + MWDT
                                                / 2, src.mMax[geomIdx][mmaxIdx]
                                                - MWDT / 2, num);

                                double tmp =
                                        src.mMax[geomIdx][mmaxIdx] - MWDT / 2;
                                ...
                                double alphaGR =
                                        Math.log(src.nu[geomIdx][nubIdx]
                                                / (Math.exp(-betaGR
                                                        * src.mMin[geomIdx]) - Math
                                                        .exp(-betaGR
                                                                * src.mMax[geomIdx][mmaxIdx])));
                                ...
                                while (mup <= src.mMax[geomIdx][mmaxIdx] - MWDT
                                    ...
                                    System.out
                                            .printf("%7.4e %5.2f:  %5.2f-%5.2f: %7.3f %8.4f\n",
                                                    alphaGR, betaGR,
                                                    mup - MWDT, mup, lowExt,
                                                    occ);
                                    mup += MWDT;
                        ...
                        System.out
                                .println("-->"
                                        + srcGem.getMagfreqDistFocMech()
                                                .getMagFreqDist(0)
                                                .getTotalMomentRate());
                        System.out.println("--> Mom rate:"
                                + srcGem.getMagfreqDistFocMech()
                                        .getMagFreqDist(0).getMomentRate(5));
                        srclst.add(srcGem);
                    }
                }
