...
public class australia2GemSourceData extends GemFileParser {
	...
	public australia2GemSourceData(){
		...
		try{
		    ...
		    System.out.println(this.getClass().getClassLoader().getResource(inputfile).getPath());
		    Document doc = db.parse(this.getClass().getClassLoader().getResource(inputfile).getPath());
		    doc.getDocumentElement().normalize();
		    
		    // source model
		    NodeList SourceModel = doc.getElementsByTagName("Source_Model");
		    // magnitude type
		    String MagnitudeType = SourceModel.item(0).getAttributes().item(0).getNodeValue();
		    ...
		    for(int i=0;i<NPolygons;i++){
			    ...
			    // Polygon area
			    double PolygonArea = Double.parseDouble(Polygon.getAttributes().getNamedItem("area").getNodeValue());
			    ...
			    for(int ic=0;ic<PolygonChildList.getLength();ic++){
				    ...
				    // Polygon boundary
				    if(PolygonChildList.item(ic).getNodeName().equalsIgnoreCase("boundary")){
				    	
				    	LocationList PolygonBoundary = new LocationList();
					    StringTokenizer st = new StringTokenizer(PolygonChildList.item(ic).getFirstChild().getNodeValue());
					    while(st.hasMoreTokens()){
					    	PolygonBoundary.add(new Location(Double.valueOf(st.nextToken()),Double.valueOf(st.nextToken())));
					    }
					    //System.out.println("Polygon Boundary"+PolygonBoundary);
					    PolygonRegion = new Region(PolygonBoundary, Bordertype);
					    System.out.println("Polygon Boundary: "+PolygonRegion);
					    
				    } // end polygon boundary

				    
				    // Exclude boundary
				    if(PolygonChildList.item(ic).getNodeName().equalsIgnoreCase("exclude")){
				    	
				    	LocationList PolygonExcludeBoundary = new LocationList();
					    StringTokenizer st = new StringTokenizer(PolygonChildList.item(ic).getFirstChild().getNodeValue());
					    while(st.hasMoreTokens()){
					    	PolygonExcludeBoundary.add(new Location(Double.valueOf(st.nextToken()),Double.valueOf(st.nextToken())));
					    }
					    
					    // add to list
					    listExReg.add(new Region(PolygonExcludeBoundary,Bordertype));
					    
				    } // end exclude boundary
				    
				    // Recurrence model
				    if(PolygonChildList.item(ic).getNodeName().equalsIgnoreCase("recurrence")){

					    minMag = Double.valueOf(PolygonChildList.item(ic).getAttributes().getNamedItem("min_magnitude").getNodeValue());
					    System.out.println("min_magnitude: \"" + minMag + "\"");		
					    
					    maxMag = Double.valueOf(PolygonChildList.item(ic).getAttributes().getNamedItem("max_magnitude").getNodeValue());
					    System.out.println("max_magnitude: \"" + maxMag+ "\"");
					    
					    lambda = Double.valueOf(PolygonChildList.item(ic).getAttributes().getNamedItem("Lambda_Min").getNodeValue());
					    System.out.println("Lambda_Min: \"" + lambda + "\"");
					    
					    bVal = Double.valueOf(PolygonChildList.item(ic).getAttributes().getNamedItem("b").getNodeValue());
					    System.out.println("b: \"" + bVal + "\"");
					    
					    System.out.println("min_mag: \"" + PolygonChildList.item(ic).getAttributes().getNamedItem("min_mag").getNodeValue() + "\"");
					    
					    depth = Double.valueOf(PolygonChildList.item(ic).getAttributes().getNamedItem("depth").getNodeValue());
					    ...
					    double totCumRate = Math.pow(10, aVal-bVal*minMagnitude) - Math.pow(10, aVal-bVal*maxMag);
					    ...
					    double mminR;
					    if(minMag!=maxMag){
						    mminR = new BigDecimal(Math.floor(minMagnitude/dm)*dm+dm/2).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
						    mmaxR = new BigDecimal(Math.ceil(maxMag/dm)*dm-dm/2).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
					    	...
					    	mminR = new BigDecimal(Math.floor(minMagnitude/dm)*dm).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();	
					    	mmaxR = new BigDecimal(Math.ceil(maxMag/dm)*dm).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
					    }
						...
						IncrementalMagFreqDist mfd = new GutenbergRichterMagFreqDist(bVal, totCumRate, mminR, mmaxR, numMag);
					   
						// focal mechanism
						FocalMechanism fm = new FocalMechanism(strike,dip,rake);
					    
					    // magnitude freq dist for focal mechanism
					    magfreqDistFocMech = new MagFreqDistsForFocalMechs(mfd,fm);
						
						// top of rupture distribution
					    // the australia model has a uniform top of rupture distribution
					    EvenlyDiscretizedFunc topr = new EvenlyDiscretizedFunc(mminR,mmaxR,numMag);
					    for(int im=0;im<numMag;im++) topr.set(im, depth);
			    ...
			    // go through the excluded regions and check if there are some that overlaps
			    // if so join them
			    ...
			    for(int ier=0;ier<listExReg.size();ier++){
			    	...
			    	for(int ierr=ier+1;ierr<listExReg.size();ierr++){
			    		if(Region.union(newExcReg, listExReg.get(ierr))!=null){
			    			newExcReg = Region.union(newExcReg, listExReg.get(ierr));
			    ...
			    srcDataList.add(new GEMAreaSourceData(Integer.toString(i+1), Integer.toString(i+1), trt, 
			    		PolygonRegion, magfreqDistFocMech, 
	...
	public static void main(String[] args) throws IOException{
		...
		model.writeAreaSources2KMLfile(new FileWriter("/Users/damianomonelli/Desktop/AustraliaAreaSources.kml"));
