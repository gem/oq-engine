...
public class MexicoSourceData extends CrisisSourceData {
	...
	public MexicoSourceData(BufferedReader input) {
		...
		HashMap<Integer,TectonicRegionType> tecRegMap = new HashMap<Integer,TectonicRegionType>();
		...
		for (GEMSourceData srcDat: srcList){
			...
			if (tecRegMap.containsKey(idx)){
				if (tecRegMap.get(idx).equals(TectonicRegionType.SUBDUCTION_INTERFACE)){
					System.out.printf("Interface....[%d]:%s\n",idx,srcDat.getName());
					srcList.get(idx).setTectReg(TectonicRegionType.SUBDUCTION_INTERFACE);
				}
				if (tecRegMap.get(idx).equals(TectonicRegionType.SUBDUCTION_SLAB)){
