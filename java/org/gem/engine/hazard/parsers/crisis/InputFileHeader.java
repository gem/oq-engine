...
public class InputFileHeader {
	...
	int nIntLev; // 		L4           : Number of intensity levels used to compute the Haz. Curve 	
	double[][] SpecOrd; //	L5 	         : Spectral ordinate (each row: T, Lower Limit, Upper Limit)
	String[] SpecOrdUM; // 	L5           : Units of measure of the spectral ordinate
	
	double maxDist; // 		L5+nSpecOrd+1: Maximum distance (considers contributions coming from shorter distances)
	double ratioTriDist; // L5+nSpecOrd+1: Ratio between triangle size and maximum distance 
	...
	public InputFileHeader(BufferedReader input) throws IOException {
		...
		// Second line after spectral ordinates: Reference grid where compute the hazard
		strarr = input.readLine().split("\\,");
