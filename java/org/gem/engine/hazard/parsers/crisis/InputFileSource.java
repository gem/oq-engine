...
public class InputFileSource {
	...
	private int occMod; //			L2 		: Occurrence model index: 1 - Poisson, 2- Characteristic
	...
	private double occRate; //	    Lx 		: Rate of occurrence for earthquakes above the mmin 
	...
	private double muUn; //			Lx 		: Mu (Magnitude Upper) un-truncated expected value
	...
	public InputFileSource(BufferedReader input) throws IOException {
		...
		if (matcher.find()) this.occMod = Integer.valueOf(matcher.group(0)).intValue();
		// Source type
		matcher = INTNUM.matcher(strarr[1]);
		if (matcher.find()) this.srcType = Integer.valueOf(matcher.group(0)).intValue();
		// Attenuation model
		matcher = INTNUM.matcher(strarr[2]);
		if (matcher.find()) this.attMod = Integer.valueOf(matcher.group(0)).intValue();
...
//		if (matcher.find()) this.attMod = Integer.valueOf(matcher.group(0)).intValue();		
		...
		// TODO 	- There are three params missing we need to read in the first line
		//    		- Add checks when reading the parameters
		
		// Third line
		line = input.readLine();
		matcher = INTNUM.matcher(line);
		if (matcher.find()) this.nVtx = Integer.valueOf(matcher.group(0)).intValue();
