...
/**
 * <b>Title:</b> InputFileParser <p>
 * 
 * <b>Description:</b> This class extends the GemInput Parser class. It parses a Crisis input 
 * file and creates a list of GemSourceData objects.<p>
 * Note that Crisis can associate distinct GMPE to distinct seismic sources of the same input.
 * 
 * @author M.M. Pagani
 * @created	2010-03-01
 * @version 1.0
...
//	    	BufferedReader input =  new BufferedReader(file);
	    	try {
	    		...
	    		for (int i=0; i<head.nRegions; i++ ) {
    				...
    				HashMap<Double,Integer> depthMap = new HashMap<Double,Integer>();
    				...
    				for (int j=0; j < tmpsrc.getCoords().length; j++) {
    					...
    					// This is a hash map used to identify the number and values of depths 
    					// used to specify the source
    					if (depthMap.containsKey(tmpsrc.getDepth())) {
    						depthMap.put(tmpsrc.getDepth()[j],depthMap.get(tmpsrc.getDepth())+1);
    					} else {
    				...
    				if (INFO) {
	    				System.out.println("  Number of depths: "+depthMap.keySet().size());
	    				Iterator<Double> iterator = depthMap.keySet().iterator();
    				...
    				if (tmpsrc.getOccMod() == 1) { 
    					...
    					if (INFO) System.out.printf("  mmax: %.2f \n",tmpsrc.getMuUn());
    					double tmpMM = Math.ceil(tmpsrc.getMuUn()/MWDT)*MWDT;
    					if (INFO) System.out.printf("  recomputed mmax: %.2f\n",tmpMM);
    					
        				// -------------------------------------------------------------------------
        				//                                                Crisis Double truncated GR
    					int num = (int) Math.round((tmpMM-MMIN)/MWDT);
    					mfd = new IncrementalMagFreqDist(MMIN+MWDT/2,
    							tmpMM-MWDT/2,num);

    					double mup = MMIN + MWDT;
    					double betaGR = tmpsrc.getBetaGR();
    					double alphaGR = Math.log(tmpsrc.getOccRate() / 
    							(Math.exp(-betaGR*tmpsrc.getMmin()) - 
    							 Math.exp(-betaGR*tmpMM)) );
    					...
    					double roundMMin = Math.floor(tmpsrc.getMCharMin()/MWDT)*MWDT;
    					double roundMMax = Math.ceil(tmpsrc.getMCharMax()/MWDT)*MWDT;
    					int num = (int) Math.round((roundMMax-roundMMin)/MWDT); 
    					
//    					mfd = new IncrementalMagFreqDist(roundMMin+MWDT/2,
//    							roundMMax-MWDT/2,num);
    					mfd = new IncrementalMagFreqDist(roundMMin+MWDT/2, num, MWDT);
    					

    					int idx = 0;
    					// This is the denominator to be used to compute occurrences
    					double den = GaussianDistCalc.getCDF((tmpsrc.getMCharMax()-tmpsrc.getMCharExp())/
    							tmpsrc.getSigmaMChar()) - 
    							GaussianDistCalc.getCDF((tmpsrc.getMCharMin()-tmpsrc.getMCharExp())/
    	    					tmpsrc.getSigmaMChar());
    					double mmin, mmax;
    					// Expected MChar
    					...
    					while (mup <= roundMMax-MWDT/2) {
    						...
    						double occ = 1./tmpsrc.getMedianRate() * 
								( (GaussianDistCalc.getCDF((mup-mexp)/sigma)) -
								  (GaussianDistCalc.getCDF((mmin-mexp)/sigma)) ) / den;
    							
    						if(INFO) System.out.printf("%5.2f %5.2f %5.2f: %8.5f %8.5f\n",
    								mmin,
    								mup,
    								den,
    								mup-MWDT/2,
    					...
    					if (INFO) System.out.printf(" sum of occ: %7.4f (original rate: %7.4f)\n",
    						soc,1./tmpsrc.getMedianRate());
    					
    				} else {
	    				// To do throw an exception
	    			    System.err.println("Occorrence/mfd model: unsupported option");
    				...
    				if (depthMap.keySet().size() == 1){
    					...
    					if (USEAREASOURCES){
	    					...
	    					GEMAreaSourceData src = new GEMAreaSourceData("0", name, 
	    						TectonicRegionType.ACTIVE_SHALLOW, reg, mfdffm, depTopRup, depth);
	    					srcList.add(src);
    					}
    					...
    					if (USEDIPPINGAREAS){
	    					...
	    					// Create a list with depths (depths ordered in increasing order)
		    				...
		    				for (int j=0; j < tmpsrc.getCoords().length; j++) {
		    					if (Math.abs(tmpsrc.getDepth()[j]-upperDepth) < 1e-1){
		    						upperTrace.add(new Location(tmpsrc.getCoords()[j][1],
			    							tmpsrc.getCoords()[j][0],upperDepth));
		    					} else {
		    						lowerTrace.add(new Location(tmpsrc.getCoords()[j][1],
			    							tmpsrc.getCoords()[j][0],lowerDepth));
		    					}	
		    				}
		    				
		    				// Find the directions of the two traces and - eventually - revert one trace
		    				if (Math.abs(upperTrace.getStrikeDirectionDifference(lowerTrace)) > 90.0){
			    				lowerTrace.reverse();
		    				}
	    					...
	    					if (upperTrace.getTraceLength() < 1e-2){
			    				// To do throw an exception
			    			    System.err.println("Upper trace lenght equal to 0");
			    			    throw new RuntimeException("");	
	    					}
	    					// Do some checks
	    					if (lowerTrace.getTraceLength() < 1e-2){
			    				// To do throw an exception
			    			    System.err.println("Lower trace lenght equal to 0");
			    			    throw new RuntimeException("");	
	    					}
...
//		    					for (Location loc : lowerTrace.getLocationList){
...
//		    					}
	    					...
	    					// Subduction source
	    					GEMSubductionFaultSourceData src = new GEMSubductionFaultSourceData(
	    							String.format("%d",subSrcCounter), name, 
		    						TectonicRegionType.SUBDUCTION_INTERFACE,
	    							upperTrace, lowerTrace, rake, mfd, true);
	       					srcList.add(src);
	       					
	       					if (INFO) System.out.println("Adding subduction ========== "+ name);
	    				...
	    				// To do throw an exception
	    			    System.err.println("Source geometry: unsupported option");
	    			    throw new RuntimeException("");	
    				}
