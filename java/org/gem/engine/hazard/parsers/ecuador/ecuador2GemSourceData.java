...
public class ecuador2GemSourceData extends GemFileParser {
	...
	public ecuador2GemSourceData() throws IOException{
        ...
        BufferedReader oReader = new BufferedReader(new FileReader(this.getClass().getClassLoader().getResource(inputfile1).getPath()));
        ...
        oReader = new BufferedReader(new FileReader(this.getClass().getClassLoader().getResource(inputfile2).getPath()));
        ...
        // list source names
        for(String name: sourceNames) System.out.println(sourceNames.indexOf(name)+1+" "+name);
        
        // check if number of seismicity parameter sets is the same of source polygons
        System.out.println("Number of polygons: "+sourcePolygons.size()+", number of b values: "+bValues.size());
        ...
        for(int i=0;i<sourceNames.size();i++){
        	...
        	int numVal = (int) (((mMax.get(i)-dm/2) - (minMagnitude+dm/2))/dm + 1);
        	GutenbergRichterMagFreqDist mfd = new GutenbergRichterMagFreqDist(bValues.get(i), totCumRates.get(i),
        			minMagnitude+dm/2, mMax.get(i)-dm/2, numVal);

        	// focal mechanism
        	FocalMechanism fm = new FocalMechanism(strike,dip,rake);
        	
        	// pair mfd-fm
        	MagFreqDistsForFocalMechs mfdFm = new MagFreqDistsForFocalMechs(mfd,fm);
        	...
        	while(magIter.hasNext()) aveRupTopVsMag.set(magIter.next(), aveHypoDepth);
        	
        	// create GEMAreaSourceData
        	srcDataList.add(new GEMAreaSourceData(Integer.toString(i+1), sourceNames.get(i), trt, 
        			reg, mfdFm, 
	...
	public static void main(String[] args) throws IOException{
		...
		model.writeAreaSources2KMLfile(new FileWriter("/Users/damianomonelli/Desktop/EcuadorSource.kml"));
		//model.writeSource2CLformat(new FileWriter("/Users/damianomonelli/Desktop/EcuadorSource.dat"));
