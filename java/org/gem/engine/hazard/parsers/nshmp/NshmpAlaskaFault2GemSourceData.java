...
public class NshmpAlaskaFault2GemSourceData extends GemFileParser{
	 ...
	 */
	// constructor
	public NshmpAlaskaFault2GemSourceData(String inputfile, TectonicRegionType trt, double fileWeight,
					...
					if(FType==2){ // GR fault
						...
						for(int imag=0;imag<NMagBranches;imag++){
							...
							if(st.countTokens()==5){
								...
								// with this option the a value is not given in this line
								// but a values are given with the fault trace coordinates
...
//					     &   (itest(ift).eq.0)) then
						...
						if(FType==2 && nmag[0]>1 && itest==0 && !Double.isNaN(aVal[0])){
...
//					      if((itype(ift).eq.1).and.(itest(ift).eq.1)) then 
				...
				for(int isp=0;isp<NSegPoints;isp++){
					...
					if(FType==2 && Double.isNaN(aVal[0])){
						...
						meanAValue = meanAValue + Double.valueOf(st.nextToken()).doubleValue();
					}
					else {
						lat = Double.valueOf(st.nextToken()).doubleValue();
						lon = Double.valueOf(st.nextToken()).doubleValue();
...
//						System.out.println("Magnitude branch: "+(imag+1));
						...
						// compute total moment rate
						double tmr = totMoRate(minMag[imag], nmag[imag], dMag[imag],meanAValue, bVal[imag]);
...
//					        nmag= (mmax- magmin(ift,imag))/dmag(ift,imag) + 1.4 line 1884
    ...
    private SummedMagFreqDist sumMagFreqDist(ArrayList<IncrementalMagFreqDist> mfds){
    ...
    public static void main(String[] args) throws IOException{
    	...
    	NshmpAlaskaFault2GemSourceData model = new NshmpAlaskaFault2GemSourceData(inDir+inFile,TectonicRegionType.ACTIVE_SHALLOW, 1.0,
    			-Double.MAX_VALUE, Double.MAX_VALUE, -Double.MAX_VALUE, Double.MAX_VALUE);
    	
    	model.writeFaultSources2KMLfile(new FileWriter("/Users/damianomonelli/Desktop/AlaskaFaultSources.kml"));
