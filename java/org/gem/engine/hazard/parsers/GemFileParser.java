...
//	}
	...
	public void writeAreaGMTfile (FileWriter file) throws IOException{
		...
		for (GEMSourceData dat: srcDataList){
			if (dat instanceof GEMAreaSourceData){
				...
				if (INFO) System.out.println("==="+dat.getID()+" "+dat.getName());
				...
				if (INFO) System.out.printf("Area: %6.2e",area);
	...
	public void writeSources2KMLfile (FileWriter file) throws IOException{
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMFaultSourceData){
				...
				StirlingGriddedSurface faultSurface = new StirlingGriddedSurface(
						src.getTrace(),
						src.getDip(),
						src.getSeismDepthUpp(),
						src.getSeismDepthLow(),
		                1.0);
				
				for(int ic=faultSurface.getSurfacePerimeterLocsList().size()-1;ic>=0;ic--){
					double currentDepth = faultSurface.getSurfacePerimeterLocsList().get(ic).getDepth();
			...
			if (dat instanceof GEMSubductionFaultSourceData){
				...
				// create area source object
				GEMSubductionFaultSourceData src = (GEMSubductionFaultSourceData) dat;
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMPointSourceData){
				...
				String descr = "";
				// loop  over focal mechanisms
				for(int ifm=0;ifm<src.getHypoMagFreqDistAtLoc().getNumFocalMechs();ifm++){
					
					descr = descr + "Mmin = "+src.getHypoMagFreqDistAtLoc().getMagFreqDist(ifm).getMinX()+", "+
					                "Mmax = "+src.getHypoMagFreqDistAtLoc().getMagFreqDist(ifm).getMaxX()+", "+
					                "TotalCumulativeRate (ev/yr) = "+src.getHypoMagFreqDistAtLoc().getMagFreqDist(ifm).getTotalIncrRate()+", "+
					                "Strike: "+src.getHypoMagFreqDistAtLoc().getFocalMech(ifm).getStrike()+", "+
					                "Dip: "+src.getHypoMagFreqDistAtLoc().getFocalMech(ifm).getDip()+", "+
					                "Rake: "+src.getHypoMagFreqDistAtLoc().getFocalMech(ifm).getRake()+", "+
					                "Average Hypo Depth (km): "+src.getAveHypoDepth();
				...
				// write coordinates
			    double lon = src.getHypoMagFreqDistAtLoc().getLocation().getLongitude()-0.05;
			    double lat = src.getHypoMagFreqDistAtLoc().getLocation().getLatitude()+0.05;
			    out.write(lon+","+lat+"\n");
			    lon = src.getHypoMagFreqDistAtLoc().getLocation().getLongitude()-0.05;
			    lat = src.getHypoMagFreqDistAtLoc().getLocation().getLatitude()-0.05;
			    out.write(lon+","+lat+"\n");
			    lon = src.getHypoMagFreqDistAtLoc().getLocation().getLongitude()+0.05;
			    lat = src.getHypoMagFreqDistAtLoc().getLocation().getLatitude()-0.05;
			    out.write(lon+","+lat+"\n");
			...
			if (dat instanceof GEMFaultSourceData){
				...
				// description
				String descr = "Mmin = "+src.getMfd().getMinX()+", "+
				                "Mmax = "+src.getMfd().getMaxX()+", "+
				                "TotalCumulativeRate (ev/yr) = "+src.getMfd().getTotalIncrRate()+", "+
				                "Dip: "+src.getDip()+", "+
				                "Rake: "+src.getRake()+".";
				...
				for(int ic=faultSurface.getSurfacePerimeterLocsList().size()-1;ic>=0;ic--){
					double lon = faultSurface.getSurfacePerimeterLocsList().get(ic).getLongitude();
					double lat = faultSurface.getSurfacePerimeterLocsList().get(ic).getLatitude();
					double depth = faultSurface.getSurfacePerimeterLocsList().get(ic).getDepth();
	...
	public void writeAreaSources2KMLfile (FileWriter file) throws IOException{
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMAreaSourceData){
				...
				// total moment rate
				double tmr = src.getMagfreqDistFocMech().getFirstMagFreqDist().getTotalMomentRate();
				// normalize by the area source
				double area = src.getArea();
				
				System.out.println("Source name: "+src.getName()+", total moment rate: "+tmr+", area: "+area+", total moment rate normalized by area: "+tmr/area);
				
				tmr = tmr/area;
				
				sourceTMR.add(tmr);
				
				if(tmr>maxTmr) maxTmr = tmr;
				if(tmr<minTmr) minTmr = tmr;
				...
				if(lonMax>maxLon) maxLon = lonMax;
		...
		double deltaColorBar = (Math.log10(maxTmr)-Math.log10(minTmr))/colorName.size();
	    ...
	    for(int i=0;i<colorName.size();i++){
	    	out.write("<TR><TD bgcolor=#"+colorName.get(i)+">&nbsp;</TD><TD bgcolor=#FFFFFF>");
	    	out.write(String.format("%2.2e",Math.pow(10,(Math.log10(maxTmr)-(i+1)*deltaColorBar))));
	    	out.write(" to ");
	    	out.write(String.format("%2.2e",Math.pow(10,(Math.log10(maxTmr)-i*deltaColorBar))));
	    	out.write("</TR>\n");
	    }
		out.write("</TABLE>]]>\n");
		out.write("</description>\n");
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMAreaSourceData){
				...
				for(int i=0;i<colorName.size();i++){
					...
					if(i!=(colorName.size()-1) && Math.log10(sourceTMR.get(sourceIndex))>Math.log10(maxTmr)-(i+1)*deltaColorBar && Math.log10(sourceTMR.get(sourceIndex))<=Math.log10(maxTmr)-i*deltaColorBar){
				...
				for(int ic=color.length()-1;ic>=0;ic--) newColor = newColor+color.charAt(ic);
				...
				for(int ifm=0;ifm<src.getMagfreqDistFocMech().getNumFocalMechs();ifm++){
					...
					if(src.getMagfreqDistFocMech().getMagFreqDist(ifm) instanceof GutenbergRichterMagFreqDist){
						GutenbergRichterMagFreqDist mfd = (GutenbergRichterMagFreqDist) src.getMagfreqDistFocMech().getMagFreqDist(ifm);
						
						double bVal = mfd.get_bValue();		
						
						double aVal = bVal*mfd.getMagLower() + Math.log10(mfd.getTotCumRate());
						
						descr = descr+", a value: "+aVal+", b value: "+bVal;
					}
				...
				out.write(src.getRegion().getBorder().get(0).getLongitude()+","+src.getRegion().getBorder().get(0).getLatitude()+"\n");
				out.write("</coordinates>\n");
				out.write("</LinearRing>\n");
				out.write("</outerBoundaryIs>\n");
	 ...
	 * This writes all sources into KML file
	 */
	public void writeGridSources2KMLfile (FileWriter file, double gridSpacing) throws IOException{
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMPointSourceData){
				...
				// source coordinates
				double srcLat = src.getHypoMagFreqDistAtLoc().getLocation().getLatitude();
				double srcLon = src.getHypoMagFreqDistAtLoc().getLocation().getLongitude();
				
				// region associated to the point
				LocationList locList = new LocationList();
				locList.add(new Location(srcLat+gridSpacing/2,srcLon-gridSpacing/2));
				locList.add(new Location(srcLat-gridSpacing/2,srcLon-gridSpacing/2));
				locList.add(new Location(srcLat-gridSpacing/2,srcLon+gridSpacing/2));
				locList.add(new Location(srcLat+gridSpacing/2,srcLon+gridSpacing/2));
			    ...
			    MagFreqDistsForFocalMechs magfreqDistFocMech = new MagFreqDistsForFocalMechs(src.getHypoMagFreqDistAtLoc().getFirstMagFreqDist(), src.getHypoMagFreqDistAtLoc().getFirstFocalMech());
				
				// create corresponding GEMAreaSourceData
				GEMAreaSourceData srcArea = new GEMAreaSourceData(src.getID(), src.getName(), src.getTectReg(), 
						srcReg, magfreqDistFocMech, 
						src.getAveRupTopVsMag(), src.getAveHypoDepth());
				
				// total moment rate
				double tmr = src.getHypoMagFreqDistAtLoc().getFirstMagFreqDist().getTotalMomentRate();
				// normalize by the area source
		...
		for (GEMSourceData dat: srcDataList){
			...
			if (dat instanceof GEMPointSourceData){
				...
				String descr = "";
				// loop  over focal mechanisms
				for(int ifm=0;ifm<srcArea.getMagfreqDistFocMech().getNumFocalMechs();ifm++){
					
					descr = descr + "Mmin = "+srcArea.getMagfreqDistFocMech().getMagFreqDist(ifm).getMinX()+", "+
					                "Mmax = "+srcArea.getMagfreqDistFocMech().getMagFreqDist(ifm).getMaxX()+", "+
					                "TotalCumulativeRate (ev/yr) = "+srcArea.getMagfreqDistFocMech().getMagFreqDist(ifm).getTotalIncrRate()+", "+
					                "Strike: "+srcArea.getMagfreqDistFocMech().getFocalMech(ifm).getStrike()+", "+
					                "Dip: "+srcArea.getMagfreqDistFocMech().getFocalMech(ifm).getDip()+", "+
					                "Rake: "+srcArea.getMagfreqDistFocMech().getFocalMech(ifm).getRake()+", "+
					                "Average Hypo Depth (km): "+src.getAveHypoDepth();
				...
				for(int ic=0;ic<srcArea.getRegion().getBorder().size();ic++){
					double lon = srcArea.getRegion().getBorder().get(ic).getLongitude();
					double lat = srcArea.getRegion().getBorder().get(ic).getLatitude();
					out.write(lon+","+lat+"\n");
				}
				out.write("</coordinates>\n");
				out.write("</LinearRing>\n");
			...
			if (dat instanceof GEMFaultSourceData){
				...
				for(int i=0;i<nrows-1;i++){
					...
					for(int j=0;j<ncol-1;j++){
						...
						double lon3 = faultSurface.get(i+1,j+1).getLongitude();
						double lat3 = faultSurface.get(i+1,j+1).getLatitude();
						double depth3 = faultSurface.get(i+1,j+1).getDepth();
...
//					    out.write("</Style>");
					    ...
					    out.write("</altitudeMode>");
					    out.write("<coordinates>");
						out.write(lon1+","+lat1+","+(maxDepth-depth1)*10e3+"\n");
						out.write(lon2+","+lat2+","+(maxDepth-depth2)*10e3+"\n");
						out.write(lon3+","+lat3+","+(maxDepth-depth3)*10e3+"\n");
						out.write(lon4+","+lat4+","+(maxDepth-depth4)*10e3+"\n");
					    out.write("</coordinates>\n");
					    out.write("</LinearRing>\n");
					    out.write("</outerBoundaryIs>\n");
					    out.write("</Polygon>\n");
	...
	public void writeSubductionFaultSources2KMLfile (FileWriter file) throws IOException{
	...
	public void writeFaultGMTfile (FileWriter file) throws IOException{
		...
		for (GEMSourceData dat: srcDataList){
			if (dat instanceof GEMFaultSourceData){
				...
				if (INFO) System.out.println("MinX "+momRateDist.getMinX()+" MaxX"+momRateDist.getMaxX());
				if (INFO) System.out.println("Length:"+len);
	...
	public void writeSource2CLformat(FileWriter file) throws IOException{
		...
		for (GEMSourceData src: srcDataList){
			...
			if(src instanceof GEMAreaSourceData){
				...
				for(int i=0;i<areaSrc.getMagfreqDistFocMech().getNumFocalMechs();i++){
					
					if(areaSrc.getMagfreqDistFocMech().getMagFreqDist(i) instanceof GutenbergRichterMagFreqDist){
						
						GutenbergRichterMagFreqDist mfd = (GutenbergRichterMagFreqDist)areaSrc.getMagfreqDistFocMech().getMagFreqDist(i);
						...
						// a value
						double aVal = mfd.get_bValue()*mMin + Math.log10(mfd.getTotCumRate());
						
						// write mfd parameters
						out.write("gr "+aVal+" "+mfd.get_bValue()+" "+mMax+"\n");
						
						// write focal mechanism
						FocalMechanism fm = areaSrc.getMagfreqDistFocMech().getFocalMech(i);
						out.write(fm.getStrike()+" "+fm.getDip()+" "+fm.getRake()+"\n");
				...
				for(int i=0;i<areaSrc.getAveRupTopVsMag().getNum();i++){
					out.write(areaSrc.getAveRupTopVsMag().getX(i)+" "+areaSrc.getAveRupTopVsMag().getY(i)+" ");
				}
				out.write("\n");
