...
public class CY_2008_test extends NGATest {
	...
	private static final String RESULT_SET_PATH = "NGA_ModelsTestFiles/CY08/";
	...
	public double doSingleFileTest(File file) {
		...
		if (fileName.contains("MEDIAN")) { // test mean
			...
			if (fileName.contains("SIGTM")) {
				...
				cy_08.getParameter(FaultTypeParam.NAME).setValue(
						CY_2008_AttenRel.FLT_TYPE_STRIKE_SLIP);
		...
		AftershockParam aftershockParam = (AftershockParam) cy_08
		...
		try {
			ArrayList<String> testDataLines = FileUtils.loadFile(file
					.getAbsolutePath());
			...
			for (int j = 1; j < numLines; ++j) {
				...
				try {
					...
					((WarningDoubleParameter) cy_08.getParameter(MagParam.NAME))
					...
					cy_08.getParameter(DistanceRupParameter.NAME)
							.setValue(rRup);
					DistRupMinusJB_OverRupParameter distRupMinusJB_OverRupParam = (DistRupMinusJB_OverRupParameter) cy_08
					...
					double rx = Double.parseDouble(st.nextToken()); // R(x) (
																	...
																	// to fault
																	// strike)
					DoubleParameter distRupMinusDistX_OverRupParam = (DoubleParameter) cy_08
					...
					// cy_08.getParameter(CY_2008_AttenRel.RUP_WIDTH_NAME).setValue(new
					// Double(CY_2008_AttenRel.RUP_WIDTH_MIN));
					// else if (w > AS_2008_AttenRel.RUP_WIDTH_MAX)
					// cy_08.getParameter(CY_2008_AttenRel.RUP_WIDTH_NAME).setValue(new
					// Double(CY_2008_AttenRel.RUP_WIDTH_MAX));
					// else
					// cy_08.getParameter(CY_2008_AttenRel.RUP_WIDTH_NAME).setValue(new
					// Double(w));
					...
					double ztor = Double.parseDouble(st.nextToken()); // Ztor,
																		...
																		// of
																		// top
					cy_08.getParameter(RupTopDepthParam.NAME).setValue(
							new Double(ztor));

					vs30 = Double.parseDouble(st.nextToken().trim());
					((WarningDoubleParameter) cy_08
					...
					double zsed = Double.parseDouble(st.nextToken()); // Zsed,
																		// sediment/basin
																		// depth
					cy_08.getParameter(DepthTo1pt0kmPerSecParam.NAME).setValue(
							new Double(zsed));
					...
					for (int k = 0; k < num; ++k) {
						cy_08.getParameter(PeriodParam.NAME).setValue(
								new Double(period[k]));
						if (isMedian)
							openSHA_Val = Math.exp(cy_08.getMean());
						else
							openSHA_Val = cy_08.getStdDev();
	...
	private String getOpenSHAParams(AttenuationRelationship attenRel) {
		...
		// str += "\nDDWidth = " +
		// attenRel.getParameter(CY_2008_AttenRel.RUP_WIDTH_NAME).getValue();
		str += "\nzTor = "
