...
/**
 * <p>
 * Title: AttenRelResultsChecker
 * </p>
 * <p>
 * Description: This class provide the common implementation of the functions
 * for all the AttenuationRelationships testcases classes
 * </p>
 * 
...
public class AttenRelResultsChecker {
	...
	private final ParameterList list = new ParameterList();
	private final ScalarIntensityMeasureRelationshipAPI imr;
	private String resultFile = null;

	// hardcode strings string val for all the X and Y axis values that are
	// shown
	...
	// Deciaml format to restrict the result to 6 places of decimal for the IMR
	// computed values
	...
	private final ArrayList controlParamVector = new ArrayList();

	// ArrayList to store the IndependentParams name and Value
	private final ArrayList independentParamVector = new ArrayList();
	...
	public AttenRelResultsChecker(ScalarIntensityMeasureRelationshipAPI imr,
			String file, double tolerence) {
		this.imr = imr;
		// this.resultFile = this.getClass().getClassLoader().getResource(file)
		// .getPath();
	 ...
	 * actual result that is stored in the file with each test set. Each test
	 * set corresponds to different parameters setting to get the target result.
	 * That result set is like a benchMark for us to compare the SHA outputs for
	 * those parameters settings with the result in the file.
	 * 
	 * @throws IOException
	 */
	...
	public boolean readResultFile(String newOutFile) throws IOException {
		...
		BufferedReader br = new BufferedReader(new InputStreamReader(this
				.getClass().getResourceAsStream(resultFile)));
		// read the first line in the file which is the name of the
		// AttenuationRelationship and discard it
		...
		while (str != null) {
			...
			if (str.equalsIgnoreCase("")) {
				...
				if (str.trim().startsWith(intensitySetString)) {
					if (outBuff != null)
						outBuff.append(str + "\n");
					// setting the imr parameters default value with the start
					// of new test case
					...
					ListIterator supportedIntensityMeasureIterator = imr
					...
					failedParamsSetting = "\n\nTest Case Number: "
							+ testCaseNumber + "\n";
					failedParamsSetting += "Intensity Measure Type: "
							+ intensityMeasureName + "\n";
				}

				// if the String read is the Param name and its value from the
				// file
				else if (str.startsWith(parameterSetString)) {
					if (outBuff != null)
						outBuff.append(str + "\n");
					// getting the parameterName
					String paramName = str.substring(str.indexOf("\"") + 1,
							str.indexOf("\")")).trim();

					// getting teh parameter Value
					String paramVal = str.substring(str.indexOf("=") + 1)
							.trim();
					if (paramVal.startsWith("\""))
						paramVal = paramVal.substring(
								paramVal.indexOf("\"") + 1,
								paramVal.lastIndexOf("\"")).trim();

					failedParamsSetting += "\t" + "\"" + paramName + "\""
							+ " = " + paramVal + "\n";

					// System.out.println("ParameterName: "+paramName);
					// System.out.println("ParameterVal: "+paramVal);
					// we only need to get the parameters whose names have been
					// given in the
					// file, result of the params will be set with the default
					// values
					ParameterAPI tempParam = list.getParameter(paramName);

					// setting the value of the param based on which type it is:
					// StringParameter,
					// DoubleParameter,IntegerParameter or
					// WarningDoublePropagationEffectParameter(special parameter
					// for propagation)
					...
					if (tempParam instanceof WarningDoublePropagationEffectParameter) {
						((WarningDoublePropagationEffectParameter) tempParam)
					...
					// getting the value of the Y_Axis Param as specified in the
					// file from the test cases
					// which will be compared to the SHA value for the Y_Axis
					// Param
					double yAxisParamVal = new Double(str.substring(
							str.indexOf("=") + 1).trim()).doubleValue();
					decimalFormat.setMaximumFractionDigits(6);
					failedParamsSetting += "GetValue For Param: " + "\""
							+ yControlName + "\": " + "\n";
					double yAxisParamValFromSHA = this
							.getCalculation(yAxisValue);
					double orignNoRoundSHAVal = yAxisParamValFromSHA;
					yAxisParamValFromSHA = Double.parseDouble(decimalFormat
							.format(yAxisParamValFromSHA));
					failedParamsSetting += "\tOpenSHA value for: " + "\""
							+ yControlName + "\" = " + yAxisParamValFromSHA;
					failedParamsSetting += ",\tbut it should be : "
							+ yAxisParamVal + "\n";

					// this line has the old rounding error we found
					// if (outBuff != null) outBuff.append(getParamValString +
					// "(\"" + st + "\") = " +
					// decimalFormat.format(yAxisParamValFromSHA) + "\n");
					// this line makes files without the rounding error
					if (outBuff != null)
						outBuff.append(getParamValString + "(\"" + st
								+ "\") = "
								+ decimalFormat.format(orignNoRoundSHAVal)
								+ "\n");

					// compare the computed result using SHA with the target
					// result for the defined set of parameters
					boolean tempRsult = compareResults(yAxisParamValFromSHA,
							yAxisParamVal);
					// if the test was failure the add it to the test cases
					// Vecotr that stores the values for that failed
					if (tempRsult == false) {
					...
					// this.testCaseNumberVector.add(new
					// Integer(this.testCaseNumber));*/

					// adding the Control Param names and Value to ArrayList for
					// all the test cases
					this.controlParamVector.add(this
							.getControlParametersValueForTest());
					// adding the Independent Param names and Value to ArrayList
					// for all the test cases
					this.independentParamVector.add(this
							.getIndependentParametersValueForTest());
				}
		...
		// if the size of this vector is not zero then return false(to make sure
		// that some test did failed)
		/*
		 * if(this.testCaseNumberVector.size() >0) return false;
		 */
		return result;
	}

	/**
	 * This function compares the values we obtained after running the values
	 * for the IMR and the target Values( our benchmark)
	 * 
	 * @param valFromSHA
	 *            = values we got after running the OpenSHA code for the IMR
	 * @param targetVal
	 *            = values we are comparing with to see if OpenSHA does correct
	 *            calculation
	 * @return
	 */
	private boolean compareResults(double valFromSHA, double targetVal) {
		// comparing each value we obtained after doing the IMR calc with the
		// target result
		// and making sure that values lies with the .01% range of the target
		// values.
		// comparing if the values lies within the actual tolerence range of the
		// target result
		double result = 0;
		if (targetVal != 0) {
		...
		if (result < this.tolerence) {
			...
			System.out.println("bad result. myVal: " + valFromSHA + " target: "
					+ targetVal + " diff: " + result + "%");
			return false;
		}
	}
	 ...
	 * 
	 * @param type
	 *            1 for mean, 2 for std. dev. and 3 for exceedence probability
	 * @return The imr calculation
	 */
	private double getCalculation(int type) {
		double result = 0.0;
		/**
		 * @todo FIX - Legend AttenRel translation done here. may be poor
		 *       design, what if AttenRel types change to another type in
		 *       future. Translated parameters should deal directly with
		 *       ParameterAPI, not specific subclass types.
		 */

		ParameterAPI imParam = imr.getIntensityMeasure();
		if (imParam instanceof WarningDoubleParameter) {
			...
			if (((Double) imParam.getValue()).doubleValue() > 0)
				warnParam.setValueIgnoreWarning(new Double(Math
						.log(((Double) imParam.getValue()).doubleValue())));
		}
	...
	/**
	 * 
	 * @returns the name and value for the independent params setting for the
	 *          test cases
	 */
	private String getIndependentParametersValueForTest() {
		...
		while (it.hasNext()) {
			...
			independentParamValue += tempParam.getName() + " = "
					+ tempParam.getValue() + "\n\t";
		}
	...
	/**
	 * 
	 * @returns the name and Value for the control params setting of the test
	 *          cases IMR
	 */
	...
	/**
	 * 
	 * @returns the ArrayList that contains the Values for control param value
	 *          for all the test cases
	 */
	...
	/**
	 * 
	 * @returns the ArrayList that contains the Values for independent param
	 *          value for all the test cases
	 */
	 ...
	 */
	/*
	 * public ArrayList getFailedTestResultNumberList(){ return
	 * this.testCaseNumberVector; }
	 */
}
