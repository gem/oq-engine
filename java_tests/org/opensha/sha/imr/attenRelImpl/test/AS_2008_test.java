...
public class AS_2008_test extends NGATest {
	...
	private static final String RESULT_SET_PATH = "NGA_ModelsTestFiles/AS08/";
	...
	public double doSingleFileTest(File file) {
		...
		if (fileName.contains("MEDIAN")) { // test mean
			...
			if (fileName.contains("SIGTM")) {
				...
				as_2008.getParameter(FaultTypeParam.NAME).setValue(
						AS_2008_AttenRel.FLT_TYPE_STRIKE_SLIP);
		...
		try {
			...
			double period[] = this.loadPeriods(testDataLines.get(0));
			for (int j = 1; j < numLines; ++j) {
				StringTokenizer st;
				double mag;
				// ((WarningDoublePropagationEffectParameter)as_2008.getParameter(DistanceRupParameter.NAME)).setValueIgnoreWarning(new
				// Double(rrup));
				double dist_jb;
				double vs30;
				try {
					...
					((WarningDoubleParameter) as_2008
							.getParameter(MagParam.NAME))
					...
					as_2008.getParameter(DistanceRupParameter.NAME).setValue(
							rRup);
					DistRupMinusJB_OverRupParameter distRupMinusJB_OverRupParam = (DistRupMinusJB_OverRupParameter) as_2008
							.getParameter(DistRupMinusJB_OverRupParameter.NAME);

					double rx = Double.parseDouble(st.nextToken()); // R(x) (
																	// Horizontal
																	// distance
																	// from top
																	// of
																	...
																	// to fault
																	// strike)
					DoubleParameter distRupMinusDistX_OverRupParam = (DoubleParameter) as_2008
							.getParameter(DistRupMinusDistX_OverRupParam.NAME);

					if (rRup > 0) {
						distRupMinusJB_OverRupParam
								.setValueIgnoreWarning((rRup - dist_jb) / rRup);
						if (rx >= 0.0) { // sign determines whether it's on the
											// hanging wall (distX is always >=
											// 0 in
											// distRupMinusDistX_OverRupParam)
							distRupMinusDistX_OverRupParam.setValue((rRup - rx)
									/ rRup);
						} else {
							distRupMinusDistX_OverRupParam.setValue((rRup + rx)
					...
					as_2008.getParameter(DipParam.NAME).setValue(
							new Double(dip));

					double w = Double.parseDouble(st.nextToken()); // W, width
																	// of rup
																	// plane
					// not sure what i should do here....
					if (w < RupWidthParam.MIN)
						as_2008.getParameter(RupWidthParam.NAME).setValue(
								new Double(RupWidthParam.MIN));
					else if (w > RupWidthParam.MAX)
						as_2008.getParameter(RupWidthParam.NAME).setValue(
								new Double(RupWidthParam.MAX));
					else
						as_2008.getParameter(RupWidthParam.NAME).setValue(
								new Double(w));
					// as_2008.getParameter(AS_2008_AttenRel.RUP_WIDTH_NAME).setValue(new
					// Double(AS_2008_AttenRel.RUP_WIDTH_DEFAULT));

					double ztor = Double.parseDouble(st.nextToken()); // Ztor,
																		// depth
																		// of
																		// top
					as_2008.getParameter(RupTopDepthParam.NAME).setValue(
							new Double(ztor));

					vs30 = Double.parseDouble(st.nextToken().trim());
					((WarningDoubleParameter) as_2008
							.getParameter(Vs30_Param.NAME))
							.setValueIgnoreWarning(new Double(vs30));

					double zsed = Double.parseDouble(st.nextToken()); // Zsed,
																		// sediment/basin
																		// depth
					as_2008.getParameter(DepthTo1pt0kmPerSecParam.NAME)
							.setValue(new Double(zsed));
					...
					for (int k = 0; k < num; ++k) {
						as_2008.getParameter(PeriodParam.NAME).setValue(
								new Double(period[k]));
						if (isMedian)
							openSHA_Val = Math.exp(as_2008.getMean());
						else
							openSHA_Val = as_2008.getStdDev();
						...
						if (result > tolerance) {
							String failedResultMetadata = "Results from file "
									...
									+ (float) zsed
									+
									// "\n\tSet distRupMinusJB_OverRupParam = "
									// +
									// as_2008.getParameter(DistRupMinusJB_OverRupParameter.NAME).getValue()
									// +
					...
					if (result > tolerance) {
						String failedResultMetadata = "Results from file "
								...
								+ "  \tMag = " + (float) mag + "  rrup = "
								+ (float) rRup + "  rjb = " + (float) dist_jb
								+ "\n\t" + "FaultType = " + fltType + "  rx = "
								+ (float) rx + "  dip = " + (float) dip
								+ "\n\t" + "w = " + (float) w + "  ztor = "
								+ (float) ztor + "  vs30 = " + (float) vs30
								+ "\n\t"
								+ "zsed = "
								+ (float) zsed
								+
								...
								// +
								"\n" + testValString + " from OpenSHA = "
								+ openSHA_Val + "  should be = " + tested_Val;
						failLine = fileLine;
						failMetadata = "Line: " + fileLine;
						failMetadata += "\nTest number= " + "(" + j + "/"
								+ numLines + ")" + " failed for "
