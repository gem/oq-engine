 ...
 * the AW_2010_AttenRel Class. A tolerance level of 1E-8 is provided.
 * Run as JUnit test.
 * 
 * Input: Excel spreadsheet prepared by D. Monelli converted to CSV text file
 * in 2 possible formats:
 * (1) For Finite Ruptures: in AW2010FINITE.TXT (Format: ruptureDistance, 
 *     MMI(Mw=5), MMI(Mw=6), MMI(Mw=7), MMI(Mw=8), Sigma)
 * (2) For Point Ruptures: in AW2010POINT.TXT (Format: hypocentralDistance, 
 *     MMI(Mw=5), MMI(Mw=6), MMI(Mw=7), Sigma)
 * 
...
public class AW_2010_test implements ParameterChangeWarningListener {
     ...
     * The average rake is not needed by the IPE but is used to define the
     * EqkRupture object. In the test, the Site is defined on the same latitude
     * as the hypocenter but shifted 0.1 degrees East.
     * 
     */
    @Test
     ...
     * results of the getMeanForFiniteRup(m,r) method when passing
     ...
     * distinction is done between point and finite rupture.
     * For this test, the defined finite rupture is data taken from the
     * Elsinore fault (USGS California model: aFault_aPriori_D2.1.in).
     * The average rake is not needed by the IPE but is used to define the
     * EqkRupture object. The Site is defined on the same latitude
     * as the hypocenter but shifted 0.1 degrees East.
     */
    @Test
     ...
     * In this test, the point source is defined with magnitude 4.0 (too low).
     * The average rake is not needed by the IPE but is used to define the
     ...
     * In this test, the point source is defined with magnitude 9.0 (too high).
     * The average rake is not needed by the IPE but is used to define the
     * EqkRupture object. Hypocenter coordinates are lat, lon, depth in km.
     */
    @Test(expected = org.opensha.commons.exceptions.WarningException.class)
     ...
     * The point source is defined with magnitude 5 (acceptable value).
     * The average rake is not needed by the IPE but is used to define the
     * EqkRupture object. Hypocenter coordinates are lat, lon, depth in km.
     * The Site is defined on the same latitude as the hypocenter but shifted
     * 4 degrees East.
     */
    @Test(expected = org.opensha.commons.exceptions.WarningException.class)
     ...
     * For this test, the defined finite rupture is data taken from the
     * Elsinore fault (USGS California model: aFault_aPriori_D2.1.in).
     * The Site is defined at the intersection of the equator and Greenwich
     * meridian (should be enough far from California).
     */
    @Test(expected = org.opensha.commons.exceptions.WarningException.class)
     ...
     * follows: closest distance to rupture(for finite ruptures)/
     * hypocentral distance (for point ruptures), mmi for mag 5, mmi for mag 6,
     * mmi for mag 7, mmi for mag 8 (only for finite ruptures) and sigma.
     */
    ...
    /*
     * Checks if computed values are same as expected values.
     * Different methods are called for Finite Ruptures and Point Ruptures.
     * Note that for finite ruptures, 4 values are given for magnitudes
     * 5,6,7,8. For point ruptures, only 3 values are given corresponding to
     * magnitudes 5, 6, 7.
     */
