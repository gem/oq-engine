 ...
 * 	<Rupture Distance>, <MMI for Mw=5>, <MMI for Mw=6>, <MMI for Mw=7>, <MMI for Mw=8>,Sigma>
 *  (2) for Point Ruptures: in AW2010POINT.TXT
 *  <Hypocentral Distance>, <MMI for Mw=5>, <MMI for Mw=6>, <MMI for Mw=7>, <MMI for Mw=8>, Sigma>

 *  
 *  Please refer to the following files (added):
 *  	AW_2010_AttenRel.java (in org.opensha.sha.imr.attenRelImpl)
 *  	MMI_Param.java 	(in org.opensha.sha.imr.param.IntensityMeasureParams)
 *  	DistanceHypoParameter.java (in org.opensha.sha.imr.param.PropagationEffectParams)
 *  
 *  and the following files (modified):
 *  	AttenuationRelationship.java (in org.opensha.sha.imr)
 *  	PropagationEffectParams.java
 *   
 * @author     Aurea Moemke
...
public class AW_2010_test {
	...
	private void loadTestData(File testFile, int numMags, double rRup[], double vals[][], double sigma[]){
		...
		//Load data per line in ASCII Text format using a Scanner to get each line
		String lineContent = scanner.nextLine(); //NOTE that first line in file contains a description in this case
	...
	private void doTest(boolean isFiniteRupture, double tolerance, int numlines, File inFile){
		int [] mw = {5, 6, 7, 8};
		System.out.println("length of mw"+ mw.length);
		int numMags = 0;
		
		if (isFiniteRupture)
			numMags = 4;  //for finite rupture, magnitude 5, 6, 7, 8
		else 
			numMags = 3; //for point rupture, only 5, 6, 7
		System.out.println("num of mags"+ numMags);
		...
		for(int i = 0; i < r.length; i++) {
			for(int j = 0; j < numMags; j++) {
				...
				if (isFiniteRupture){
					computedmean = aw_2010_AttenRel.getMeanForFiniteRup(mw[j], r[i]);
					computedstddev = aw_2010_AttenRel.getStdDevForFiniteRup(r[i]);
				} else{
					computedmean = aw_2010_AttenRel.getMeanForPointRup(mw[j], r[i]);
					computedstddev = aw_2010_AttenRel.getStdDevForPointRup(r[i]);
				}
				System.out.println("mag = " + mw[j] + " rupture = "+ r[i] + " mean: expected = "+expectedmean+
						", computed= "+computedmean+ "; stddev: expected = "+expectedstddev + ", computed = "+computedstddev);
