...
public class TestGaussianDistCalc {
	...
	public void testGetCDF() {
		...
		for(int i=0;i < func.getNum(); ++i) {
			...
			System.out.println("SRV = " + (float)srv + "; Computed Prob = "+ prob_computed
					+ ";  Ratio = "+ prob_computed/prob );
			
			checkRatio(srv, Integer.MIN_VALUE, ratio, 3.64);
			checkRatio(srv, -4, ratio, 1.0035);
			if (srv == 0)
				assertEquals("Should be equal at srv == 0", prob, prob_computed, 0.0);
			checkRDiff(srv, 0, rDiff, 0.000001);
		}
	}
	
	private static void checkRDiff(double srv, double srvMin, double rDiff, double maxRatio) {
		if (srv > srvMin) {
	...
	private static void checkRatio(double srv, double srvMin, double ratio, double maxRatio) {
		if (srv > srvMin) {
			assertTrue("Ratio of computed to actual prob should be less than "
