...
To run a risk-only calculation on top of a previous hazard run you have to click the `Continue` button on the corresponding hazard calculation and select the zip file containing the risk `job.ini` and the related inputs
