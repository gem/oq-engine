...
-  Ruptures are limited at the top and at the bottom by two planes
   ...
   Figure `1.1 <#fig:single_rupture>`__)

Source data
:::::::::::

For the definition of a point source the following parameters are
required (Figure `1.1 <#fig:single_rupture>`__ shows some of the
...
Figure `1.2 <#fig:point_source_multiple_ruptures>`__ shows ruptures
...
Figure `1.3 <#fig:point_source_ruptures>`__ shows the ruptures generated
by the point source.
...
specified as described in the previous section (see page ).
...
Hybrid Characteristic earthquake model (à la (Youngs and Coppersmith 1985))
   ...
   the system (Figure `1.7 <#fig:yc_gr_mfd>`__). However, the total
...
“Arbitrary” Magnitude Frequency Distribution
   ...
   in Figure `1.8 <#fig:arb_mfd>`__) can be input as:
...
#. *Earthquake Rupture Forecast Calculator*
   ...
   the description of the PSHA workflow (see
   Section `[subsec:classical_psha] <#subsec:classical_psha>`__ at
   page ).
...
Section `[subsec:event_based_psha] <#subsec:event_based_psha>`__ at
page ).
...
#. *Simple Fault Rupture* - in which the geometry is defined by the
   ...
   seismogenic depths. An example is shown below in
   Listing `[lst:input_rupture_simple] <#lst:input_rupture_simple>`__.
...
branching set
   ...
   -  An uncertainty type (for a comprehensive list of the types of
      uncertainty currently supported see page )
...
Section `[subsec:config_classical_psha] <#subsec:config_classical_psha>`__
at page  as a reference and we emphasize herein major differences.
...
file example described in the Section
`[subsec:config_classical_psha] <#subsec:config_classical_psha>`__ at
page .
...
(Section `[subsec:config_event_based_psha] <#subsec:config_event_based_psha>`__).
...
Section `[subsec:output_event_based_psha] <#subsec:output_event_based_psha>`__.
