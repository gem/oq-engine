...
\texttt{vs30} field is inferred, not measured. This is the default, so you
can just skip the field. Also, most of the GMPEs are not sensitive to
it.  The same can be said for the \texttt{backarc} parameter: $0$
means false and this is the default, so you can skip such column. All
columns that are not needed by the GMPEs your are using can be safely
skipped, while you will get an error if a relevant column is missing.
