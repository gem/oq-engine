Version 3.19 is the culmination of 4 months of work involving nearly 330
...
Last year we included the UCERF model for California in the USA model.
Since the UCERF model is implemented in terms of multifault sources,
...
while not it takes only 10 minutes.

Intense usage of numba.

Ruptures generated from MultiFaultSources and extracted with `oq
extract ruptures` in some cases (due to inhomogeneous mesh arrays) could not
be read by an event-based calculation. This has been fixed.
...
The problem was not saving the investigation time parameter, needed
...
Chris di Caprio from GNS Science contributed the full set of GMPEs
for the latest New Zealand model.

We fixed an implementation bug in the ZalachorisRathje2019 GMPE, reported by
Erika Schiappapietra. The magnitude scaling factor formula used is
...
Ali Talha Atici reported an issue with calculations using the
...
different geometries and therefore hazard curves.

It is now forbidden to use non-standard methods in GMPE classes, resulting
in an error rather than a warning, as in the last 3 years.
...
scenario calculations can be performed by using the GEM exposures and
vulnerability functions. Alternatively, one can use a `rupture_dict`
parameter in the job.ini to specify a planar rupture.

Moreover, we have now a script which is able to store the entire world
exposure and site parameters in a HDF5 and a fast way to retrieve the
assets and site parameters around any site in the world.

Finally, we have a geolocation utility which is able to determine
the country and the mosaic model to use for each site in the world.
...
it can be and it is used to run nightly tests on ~500 sites of interests.
...
We added support for Python 3.11 and remove support for Python 3.8, as
...
keep the numbers the same, both on an old processor and a new processor, we
...
We use now urllib3 to avoid SSL errors on Windows.

There were also some minor improvements to the universal installer: in
particular, now it warns the user that the virtual environment must be
removed before reinstalling, or suggests to use to use the `--venv`
...
has no permissions to install a global DbServer.
