...
Assessment. There is a single codebases with three different interfaces
depending on the kind of user:

- level 0 users: can only see computations performed by other user and shared
- level 1 users: can only perform ShakeMap calculations determined by a ShakeMap ID
...
and at the time zone we are able to determine if `time_event` is "day", "night"
...
Level 2 users have a mosaic-model selector that can be used to select the model
to use in case there are multiple models close to the event.
...
Ilaria Oliveti contributed a fix to the GMM Tusa, Langer, Azzaro (2019),
since the coefficient table was inverting frequencies with periods.
...
consequence functions, with the final goal of making it possible
multi-peril calculations.
...
Multi-peril calculations are still not ready for the prime time and
...
which is a good value if you have 32 GB of available memory and wants to avoid
...
were failing on macOS with and HDF5 error. This is now fixed.
...
Finally, we removed many features related to the social vulnerability,
since they were not used anymore.
