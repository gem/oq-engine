...
The calculation IDs, output IDs, names, and exact set of outputs can change
between engine versions. After running a demo, use
``oq engine --list-outputs -1`` to see the outputs produced by the installed
version.
...
Several building-risk demos use Nepal as the region of interest. Their example
exposure contains 9,063 assets distributed among 2,221 locations. A map of
the number of buildings is presented in :ref:`the next figure
<exposure-nepal>`.
...
The building portfolio was organised into four classes for rural areas
(adobe, dressed stone, unreinforced fired brick, and wooden frames) and five
classes for urban areas (the same typologies plus reinforced concrete). The
vulnerability and fragility functions were collected from published
literature for the region. These input models are only demonstrative. For
information about building characteristics in Nepal, see the
`National Society for Earthquake Technology-Nepal
<https://www.nset.org.np/>`_.

The following sections explain how to run the risk calculations and produce
the necessary hazard inputs. Each demo includes configuration files, an
exposure model, risk functions, and the relevant hazard inputs or models.

In the commands below, ``--hc -1`` uses the current user's latest calculation
as the hazard input. This assumes that it is the calculation generated by the
preceding ``job_hazard.ini`` run. If another calculation has been run since
then, replace ``-1`` with the calculation ID produced by ``job_hazard.ini``.

***************
Scenario Damage
***************

This demo considers a magnitude Mw 7 rupture in central Nepal. Its geometry,
dip, rake, hypocentre, and seismogenic depths are defined in
``fault_rupture.xml``. The hazard and risk settings are specified in
``job_hazard.ini`` and ``job_risk.ini``.
...
The remaining risk demo directories illustrate extensions to the core
workflows:

- ``InfrastructureRiskRoadNetwork`` and
  ``InfrastructureRiskWaterSupplyNetwork`` demonstrate the connectivity
  analysis described in :ref:`infrastructure-risk-intro`.
- ``InfrastructureMultiPeril`` demonstrates the ground-shaking, landslide,
  and liquefaction workflow described in :ref:`multi-peril-risk-intro`.
- ``Reinsurance`` demonstrates the policies and treaties described in
  :ref:`reinsurance-intro`.

Each directory contains a README with its purpose, expected runtime, and
principal outputs. Run its ``job.ini`` and then list the actual outputs with
``oq engine --list-outputs -1``.
