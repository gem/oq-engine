...
*point source gridding*, based on the idea of using a raw grid for
sites distant from the point sources and a fine grid for close sites.
...
Finally, the engine can automatically download and run
...
over 100 GB of RAM on the master node now runs with less than 32 GB of
RAM on the master node. We also optimized the calculations of the
probability of exceedence by carefully generating arrays with a size
smaller than the CPU cache which can give a speedup of a factor 2 or 3.
...
event based but `random_seed` in scenario, quite confusing since
`random_seed` was also used for the logic tree sampling.

We changed the algorithm generating the rupture seeds,
so the engine will not produce identical GMFs as before, but they will
still be statistically equivalent. We refined the "minimum_intensity"
approximation, by making it more precise.
...
in data transfer, with a significant on the required RAM) while for
...
that you are running an `event_based_risk` calculation. The core calculations
are the same now.

We added the ability to compute aggregate losses to the scenario calculators
and aggregate loss curves to the `event_based_risk` calculator, even if they are
less efficient than the `ebrisk` calculator.

We optimized the case of many tags so that now it is possible to
aggregate by asset ID or by site ID with

  aggregate_by = id  # compute loss curves by asset
  aggregate_by = site_id  # compute loss curves by site
  
up to many thousands of assets/sites; before it was impossible due
to memory issues.
...
this is an alternative approach to the old `AvgGMPE`, which performs
gometric averages on the GMFs.
...
moment. Finally he introduced a KiteSurface class and and KiteFaultSource
...
We raise an early error if the parameter `soil_intensities` is set with an
...
Now we perform the disaggregation checks before starting the classical part of
...
We enhanced the command `oq compare`.

We improved a fixed a few `oq plot` subcommands.

We enhanced `oq plot sources` to plot point sources and to manage the IDL.
