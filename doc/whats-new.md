...
make the calculation slower than needed.
...
be split. It would never work anyway, because it would likely run out of memory.
...
table and it is exported in the file `sigma_epsilon.csv`.

In the `sigma_epsilon.csv` exported we renamed the field `eid` into
