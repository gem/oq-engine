This release features several major new features (completely revised
disaggregation, automatic optimization of duplicated sources,
...
The engine is now smart enough to recognize duplicate sources
appearing in different branches of the composite source model and to
avoid redundant computations. Because this
optimization is always on, the flag `optimize_same_id_sources` has
been removed, being now useless. There are several models in the
...
There was a substantial change in the way the tasks are distributed
...
much onger that the `task_duration` parameter may still appear: however,
the new approach is a drastic improvement and the situation was never better
than it is now.

We added a check on sources with a suspiciously large spatial extend
...
We added a check on sources with suspicious hypo depth and nodal plane
distributions (i.e. distributions with duplicated values) since they
make the calculation to be slower than needed/

In extra-large models saving some debugging information (the number of sites
affected by each source) was exceedingly slow, so now the information is
stored only if there are less than 100,000 relevant sources.
...
each GSIMs keeps its own minimum distance, but before they were all
...
The idea is that users can map the exposure taxonomies to one or
more vulnerability/fragility taxonomies, with different weights,
to take into account the uncertainty in the risk. The feature is
documented here:
...
correctly flagged as an error. Thanks to the change now internally
...
We introduced a parameter `max_sites_per_gmf` in the job.ini, with a
default of 65,536 sites. When an user tries to run a calculation with
too many sites (typically because of an incorrect grid spacing) now
she gets an early error instead of trashing her machine due to memory
swapping after hours of calculations. The limit can be raised,
...
4 billion events there is probably something wrong with it, and it has to
be split. It would never work anyway, because it would like run out of memory.
...
`code` field in the `ruptures` dataset is now an unique checksum
...
We fixed a small bug in the rupture XML exporter that was failing
...
by an event based calculation but not in the ground motion fields.
Not storing the GMFs reduces the data transfer and the memory occupation.
...
to export the site model information - that works for all calculators,
not only for event based -  is to use the command
``oq show sitecol > sitecol.csv``

The GMF import from CSV has been enhanced and now it does not require
anymore the field `rlzi`: before it was required, but it was assumed
to contain always the value `0`. On the other hand, now the GMF exporter
to CSV does not export the field `rlzi`, because it is redundant: the
association between events and realizations can be found in the events
table and it is exported in the file `sigma_epsilon.csv`. In the 
`sigma_epsilon.csv` exported we renamed the field `eid` into `event_id`
in order to avoid confusion with the naming used in the `gmf_data.csv` file
(`event_id` is the 64 bit event ID in the `events` table in the datastore,
`eid` is the 32 bit index to the event ID record).

Event based risk
----------------

There was a huge refactoring of all risk calculators. As a consequence
the event based risk calculation has become simpler and faster than before
...
Naturally this is only viable for small exposures.
...
Graeme Weatherill contributed a finite rupture option to the
...
Chris van Houtte contributed the Van Houtte et al. (2018) significant duration
model for New Zealand.

Robin Gee fixed a bug in the  GMPE Sharma (2009): there was
a key error if the intensity measure level specified in the job.ini included
periods that required interpolation.

Marco Pagani discovered a bug in ``calc_hazard_curves`` which was failing
with a cryptic
``AttributeError: 'NoneType' object has no attribute 'within_bbox'``. It
has been fixed.
...
of the instantiating the vulnerability functions.
...
Before the release 3.3 the engine had the ability to associate site
model parameters to hazard sites on a grid. Such feature was sometimes
...
associations only once and for all.
...
prepare_site_model`, since the same associations will be recomputed at
...
site model once and for all with the good grid spacing.
...
The progress report has been improved: before in large classical calculation
...
In this releases we updated some of our libraries (numpy from v 1.14 to 1.16 and
scipy from version 1.0.1 to 1.3.0) to make it possible to use the engine
with Python 3.7. We actually have a cluster using Python 3.7 in production.
