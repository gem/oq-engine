...
_devtest_innervm_run () {
    ...
    ssh $lxc_ip "sudo su postgres -c \"cd oq-engine ; export PYTHONPATH=\\\"\\\$PWD/oq-engine:\\\$PWD/oq-nrmllib:\\\$PWD/oq-hazardlib:\\\$PWD/oq-risklib\\\" ; bin/create_oq_schema --yes --db-user=\\\$USER --db-name=openquake --schema-path=\\\$(pwd)/openquake/engine/db/schema --load-fixtures=\\\$(pwd)/qa_tests/risk/fixtures.tar\""
