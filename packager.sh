...
_devtest_innervm_run () {
    ...
    ssh $lxc_ip "sudo su postgres -c \"cd oq-engine ; openquake/engine/bin/oq_create_db -v --yes --db-name=openquake --schema-path=openquake/engine/db/schema\""
...
_pkgtest_innervm_run () {
    ...
    ssh $lxc_ip "sudo -u postgres oq_create_db --yes --db-name=openquake --no-tab-spaces --schema-path=/usr/share/pyshared/openquake/engine/db/schema"
