...
class ExposureDBWriterTestCase(unittest.TestCase, helpers.DbTestMixin):
    ...
    """
    def setUp(self):
        # set up a dummy vulnerability model/function
        self.vulnerability_model = models.VulnerabilityModel(
            owner=self.default_user(), name="Test",
            imt="pga", imls=[], category='test')
        self.vulnerability_model.save()
        self.vulnerability_function = models.VulnerabilityFunction(
            vulnerability_model=self.vulnerability_model,
            vf_ref='RC/DMRF-D/LR', loss_ratios=[],
            covs=[])
        self.vulnerability_function.save()
    ...
    def test_read_exposure(self):
        ...
        self.assertEquals('RC/DMRF-D/LR',
                          assets[0].vulnerability_function.vf_ref)
        ...
        self.assertEquals('RC/DMRF-D/LR',
                          assets[1].vulnerability_function.vf_ref)
        ...
        self.assertEquals('RC/DMRF-D/LR',
                          assets[2].vulnerability_function.vf_ref)
