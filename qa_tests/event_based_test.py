...
TestData = collections.namedtuple(
    ...
    'expected_losses_lc expected_loss_map')
...
class EventBasedTestCase(unittest.TestCase):
    ...
    def check(self, asset_output, testdata, asset_id):
        self.assertAlmostEqual(
            testdata.expected_loss_map[asset_id],
            asset_output.conditional_losses[0.99],
            delta=0.05 * testdata.expected_loss_map[asset_id])

        self.assert_allclose(
            testdata.expected_poes[asset_id],
            asset_output.loss_ratio_curve.y_values)

        self.assert_allclose(
            testdata.expected_losses_lrc[asset_id], 
    ...
    def check_il(self, asset_output, i):
        ...
        self.assert_allclose(
            il.expected_losses_lc[i], asset_output.insured_loss_curve.x_values)
