...
class ScenarioTestCase(unittest.TestCase):
    ...
    def test_mean_based(self):
        epsilons = scientific.make_epsilons(2, 10, seed=37, correlation=0)

        [asset_output_a1, asset_output_a3] = \
            self.vulnerability_model_mean["RM"].apply_to(
                [self.hazard_mean["a1"], self.hazard_mean["a3"]], epsilons)
        ...
        epsilons = scientific.make_epsilons(1, 10, seed=37, correlation=0)
        [asset_output_a2] = self.vulnerability_model_mean["RC"].apply_to(
            [self.hazard_mean["a2"]], epsilons)
    ...
    def test_sample_based(self):
        ...
        epsilons = scientific.make_epsilons(2, 1000, seed=37, correlation=0)
        [asset_output_a1, asset_output_a3] = \
            vulnerability_model['RM'].apply_to(
                [gmv.a1, gmv.a3], epsilons)
        ...
        epsilons = scientific.make_epsilons(1, 1000, seed=37, correlation=0)
