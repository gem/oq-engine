...
class Afteran(BaseCatalogueDecluster):
    """ Afteran algorithm
    """
    def decluster(self, catalogue_matrix,
            time_dist_window=GardnerKnopoffWindow(), time_window=60.):

        # Convert time window from days to decimal years
        time_window = time_window / 364.75
        ...
        year_dec = decimal_year(catalogue_matrix[:, 0], 
                catalogue_matrix[:, 1], catalogue_matrix[:, 2])

        # Get space windows corresponding to each event
        sw_space = time_dist_window.calc(mag)[0]
